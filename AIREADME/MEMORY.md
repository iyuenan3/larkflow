...
## 2026-08-20 · 扁平 token 前驱会跨业务语义组污染搜索实体

- **现象，已复现**：真实旅行约束先列景点，再用分号开始交通要求。景点查询正确，交通查询却带入前一组的“伊犁那拉提”，同时漏掉复合短语“上海往返乌鲁木齐航班”中的乌鲁木齐。
- **根因**：交通实体抽取在按顿号、逗号和分号全局打平的 token 上工作。遇到任何交通词时，它把前一个短 token 当成枢纽；复合交通短语又在长度裁剪后丢失完整“航班”后缀，无法提取真正目的地。
- **修复**：先以分号、句号和换行保留业务语义组，只在包含交通词的同组内解析方向词与交通方式之间的有界中文实体；显式短枢纽仍可在本组内读取，跨组前驱完全不参与。真实约束必须作为回归输入，并回读 adapter 实际 query，而不是只测试人为拆开的字段。
- **避免**：任何按“前一个 token”补上下文的规则都必须先证明边界没有被 tokenizer 丢弃。字段授权解决可见范围，语义分组解决相关范围，两者缺一不可。
