...
| lark-cli (@larksuite/cli，飞书官方) | **入口 + 出口全包**：入口 = `event consume` 收 NDJSON 事件；出口 = 交付物读写（`markdown`/`docs`/`drive` 的 create/overwrite/fetch/upload/版本）+ 建任务 / 发卡（`task`/`im`）+ 节点内 AI 工具 + 运维 | 外部 CLI；dev 走独立测试组织（ADR-008）|
