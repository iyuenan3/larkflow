...
> 状态：Target System Boundary + Experimental Edge · 2026-08-05
...
员工电脑上的 Personal Agent Edge Proof 与中央 `lark-cli` 完全分离。Edge 使用一次性配对码取得可撤销设备凭据，再通过中央私有 HTTPS API 长轮询领取 `personal.readonly` 节点；它不登录飞书、不持有企业应用 profile，也不复用开发者电脑上的 `lark-cli`。用户可运行单次 `run-once`，或主动保持一个固定单工作区的前台 `serve` 会话；两者共享同一设备身份、中央租约和服务端授权，不会因为持续领取而扩大 capability。每次会话还必须同时通过目录级探针、确认模型外发并声明 `edge-data-v0.1` 允许的 `synthetic` 或 `public` 分类。安全卸载只处理本机受管文件，中央撤销才使设备凭据失效。受控 SSH 隧道已经完成干净员工 Mac 的安装、回滚、真实合成执行、撤销和清理，但非交互测试 Keychain 不能替代登录 Keychain 首次交互；公网设备链路仍受阿里云中国内地 ICP 接入备案阻断。员工若要在本机 Agent 内另行使用飞书能力，那是个人可选能力，不是 Edge 传输层或中央授权依据。
