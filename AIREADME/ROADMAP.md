...
- 在修复后的 Console 上继续同一次受控使用。由 Owner 不依赖聊天 `status` 命令或开发者代查，再次定位指定实例，使用拖动、缩放或适配查看完整 DAG，点选返工节点并读取 Attempt 和关键审计；记录是否成功、所需时间及是否仍需外部解释。取得这项证据后，再决定分页筛选、跨轮次对比或继续保持现状。任何可写前端仍必须复用既有预览确认、Owner 重授权和版本栅栏，不能直接改数据库。
...
- 暂停、流程级继续和版本绑定取消已经落码并部署，采用“停止新调度、允许已发出节点收口”的 drain 语义，取消会终止未完成 Node 与 Attempt、清除 claim 并拒绝迟到结果。真实 PostgreSQL 双连接竞争已通过；下一步完成真实飞书命令、Human Task 与决定卡收口验收，再补运维告警、可配置自动重试策略与人工接管运营视图。
- 投影缺失重建、重复事件与乱序事件验证。
