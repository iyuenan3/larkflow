...
- 已完成 Owner 中央只读控制台 v0：真实 PostgreSQL 可展示本人最近流程、运行中或草稿 DAG、跨轮次 Attempt 与追加型审计；非 Owner 真实实例返回 404。开发服务只监听 loopback，并已通过 SSH 隧道浏览器验收。当前静态 token 只适合开发试用，不是生产登录方案。
...
- 在下一项真实工作中安排一次受控 Console 使用：由 Owner 不依赖聊天 `status` 命令或开发者代查，自行定位指定实例、当前阻塞节点、返工 Attempt 和关键审计，再记录是否成功、所需时间及是否仍需外部解释。取得这项证据后，再决定分页筛选、跨轮次对比或继续保持现状。任何可写前端仍必须复用既有预览确认、Owner 重授权和版本栅栏，不能直接改数据库。
