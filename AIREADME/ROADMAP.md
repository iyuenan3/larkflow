...
- 内容提交 `3d438bb476ad9b9f98cd4c2873802a2894718fe4` 已关闭网页普通 Human 责任入口与待办转交的代码和开发部署门槛。任务参与者只看有界任务上下文，可以提交结果或转交给同租户活跃成员，不能读取完整协作者实例；决定节点继续使用飞书决定卡。转交保留冻结 Snapshot，只移动运行时 NodeInstance Owner，并追加审计与 outbox、同步既有飞书 Task 负责人。完整离线套件为 `1003 passed, 22 skipped`，真实 PostgreSQL 双连接竞争只有一路成功。开发服务器已完成 wheel、备份、migration、全部服务、静态资源、安全响应头与登录态 API 读回。用户刚才从页面确认两个草稿后，开发库现有两个已绑定飞书 Task 的普通 Human 节点处于 `waiting_human`；下一门槛是用真实浏览器完成一次页面提交和一次转交，并从飞书 Task 与 PostgreSQL 两侧读回一致终态。
...
- Owner 流程操作与普通 Human 任务页面均已提交、部署并通过真实 PostgreSQL 和鉴权 HTTP 边界验证。下一步等待自然产生的一条普通 Human Task，由当前负责人在真实登录浏览器提交一次结果，再创建另一条可转交任务并选择测试成员，回读旧负责人失权、新负责人可见、飞书 Task 负责人同步和最终完成。取得这项证据后，再建设受控流程输入框：输入先生成草稿与预览，用户明确确认后才启动，不把自由文本伪装成机器人消息，也不直接修改数据库。
