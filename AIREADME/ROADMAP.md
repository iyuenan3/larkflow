...
- 已完成自动节点失败恢复闭环：向节点 Owner 发送恢复卡，卡片回调进入耐久命令，领域侧重新校验 Owner 与精确 Instance / Node / Attempt 版本。重试和人工接管都创建新 Attempt，失败历史不覆盖。完整离线套件为 `779 passed, 14 skipped`；长期 PostgreSQL 十六份 migration、两个连续真实重试、人工接管、Human Task 完成、最终投影和可操作卡片即时反馈装配均已在开发环境验收。
