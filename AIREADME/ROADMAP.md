...
- Owner 流程操作与普通 Human 任务页面均已提交和部署。真实登录浏览器已完成一次普通 Human 结果提交和一次跨成员转交；PostgreSQL、outbox、Projection 与飞书 Task 负责人同步均已回读。下一步建设受控流程输入框：输入先生成草稿与预览，用户明确确认后才启动，不把自由文本伪装成机器人消息，也不直接修改数据库。
