...
- 飞书应用内员工工作台登录与 PostgreSQL 耐久会话已通过公网 IP HTTPS 开发部署。至少两名真实成员已完成 OAuth 登录、本人 Owner 可见性和跨 Owner 隔离验证，机器人能力继续可用；完成登录的真实会话已通过 Console 重启后直接刷新仍保持登录。下一门槛是定义只读优先的最小管理员后台、管理员级会话撤销与审计边界，再补正式域名、生产限流和安全响应头回归。公网 IP、静态开发 token 和单机部署都不作为正式员工交付方案。
