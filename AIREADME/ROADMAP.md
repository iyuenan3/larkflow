...
- Owner 流程操作、普通 Human 任务页面、受控流程输入与第一版受控 DAG 画板均已提交和部署。真实登录浏览器已完成普通 Human 提交、跨成员转交与网页草稿生成；本地真实浏览器已完成画板增改、取消删除和节点返工。下一步选取一个无风险开发实例，由真实登录 Owner 在公网完成未来节点增加或修改、图编辑确认和节点返工，再从 PostgreSQL 回读 revision、Attempt、旧历史与审计。该验收关闭后回到项目自身的真实工作，继续观察首次结果可用性、退回率、人工干预和重复副作用。
