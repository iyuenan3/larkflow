...
- 真实苏州样本已经关闭数据库调度、结构化 Human 提交、三个搜索 Tool、Agent 多上游消费、飞书 Task 完成投影、明确退回、局部返工、第二轮接受和两张决定卡收口。修复提交 `ba708724b5095f9185aa894ec381151f4305b91d` 只信任供应商结构化引用，并让消费搜索结果的 Agent 固定披露未独立核验边界。ADR-120 已冻结来源健康与 claim 支持的分层合同：生产默认不抓取任意返回 URL，health 只能保持 unknown；确定性支持只绑定当前搜索 Attempt 的规范化 URL 和 provider 原文片段。实现与测试尚待完成，供应商引用可访问性、摘要真实性和旅游事实正确性均未关闭，因此继续保持不可邀请测试。
...
- Phase 2A 已完成离线、真实 PostgreSQL、Caddy、开发部署和 Owner 作用域 HTTPS API 验收。Phase 2B 的 Agent Attempt 附件引用和可审计能力信封也已完成开发部署、真实 PostgreSQL 合同与 synthetic Runtime 技术探针。企业共享正文、服务器授权证明、规划与 Agent Attempt ContextBundle 合并现已完成真实 PostgreSQL、0026 migration、Caddy、开发部署和 synthetic Runtime 探针。下一关是真实管理员浏览器发布、真实内部资料受控验收，以及已有 Owner 项目附件交互的手工验收；生产对象存储、飞书消息附件、PDF/DOCX/OCR、语义检索、向量检索和 Tool Gateway 继续后置。
