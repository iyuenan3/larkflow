...
- 飞书应用内员工工作台登录基础已在内容提交 `c2e9db99f4b463a895450371dde9b176d6c31ef1` 本地实现：OAuth v3、PKCE、tenant 显式映射、服务端不透明会话和静态开发模式兼容均已有测试。下一步不是继续扩展页面，而是取得稳定公网 HTTPS，配置飞书应用主页与精确回调地址，部署后由至少两名真实成员验证自动登录、本人 Owner 可见性和跨 Owner 404。当前随机 Bearer token、SSH 隧道和进程内会话都不作为正式员工交付方案；管理员后台在员工身份和可见性稳定后再做。
