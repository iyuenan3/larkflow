...
- 内容提交 `432fea77c210e7a2cfa5344054eb30d01706bf87` 已关闭受控流程输入的代码、迁移与开发部署门槛。工作台填写的目标、背景和可选协作者先写入 PostgreSQL `DraftRequest`，无飞书凭据的中央 Worker 领取后生成并确定性校验候选 DAG，成功结果只创建草稿并复用现有“确认并启动”。重复提交幂等、候选冻结、租约接管和最多五次失败终止均已覆盖。完整离线套件为 `1015 passed, 23 skipped`；长期库应用第二十三份 migration，真实双 Worker 竞争只有一路领取，部署服务与公网边界正常。下一门槛是真实登录页面生成一个模型草稿并回读 `draft / 0 Attempt`，随后进入真实业务受控试用。
