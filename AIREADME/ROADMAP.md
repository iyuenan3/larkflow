...
P0 结构能力已由 `6d2d9fa22b7e6926cfe5a1bcf714ccccd073b6d3` 与 `c7e7d901237a3b72d3d265382e1e2239a0626abd` 提交、推送和部署。页面新增节点不再要求用户填写内部 key，可选择插入到既有节点之前；Human、Agent 与 Tool 都按交付物合同完成；飞书 Task 原生完成不能绕过必填输出；旅游规划必须经过需求确认、并行 `web.search` 研究、Agent 综合和 Human 决定。完整离线套件为 `1058 passed, 24 skipped`。

- 真实苏州样本已经关闭数据库调度、结构化 Human 提交、三个搜索 Tool、Agent 多上游消费和真实飞书 Task 完成投影门槛，但最终结果被真实 Human 退回。退回原因是来源 URL 的可访问性与正文绝对真实性声明不一致；网页决定后飞书决定卡也没有同步收口。下一步不是继续扩展画板，而是确定性区分供应商引用与正文裸 URL、建立安全的来源健康或证据状态、禁止无依据的绝对真实性声明，并让 Console 与卡片两个决定入口共享同一外部收口。上述缺口关闭前继续保持不可邀请测试。
