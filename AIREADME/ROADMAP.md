...
- 飞书应用内员工工作台登录已通过公网 IP HTTPS 开发部署。至少两名真实成员已从飞书工作台网页入口完成 OAuth 登录、本人 Owner 可见性和跨 Owner 隔离验证，机器人能力继续可用。下一门槛是把进程内会话迁移到 PostgreSQL，验证 Console 重启后登录态延续、过期清理和显式注销；随后再定义只读优先的最小管理员后台。公网 IP、随机开发 token 和单进程部署都不作为正式员工交付方案。
