...
- 飞书应用内员工工作台登录、PostgreSQL 耐久会话、最小管理员聚合和其他会话撤销已通过公网 IP HTTPS 开发部署。至少两名真实成员已完成 OAuth 登录、本人 Owner 可见性和跨 Owner 隔离验证，机器人能力继续可用；完成登录的真实会话已通过 Console 重启后直接刷新仍保持登录。管理员会话治理的真实 HTTP 与 PostgreSQL 竞争验收已完成，接下来先完成新面板的真实登录浏览器目视，再补公网入口限流、完整安全响应头、allowlist 变更运维流程和恢复演练。正式域名因近期不备案保持后置；公网 IP、静态开发 token 和单机部署都不作为正式员工交付方案。
