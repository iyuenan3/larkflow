...
- 先提交并部署 Owner 待处理中心，核对真实 PostgreSQL 查询仍限定 tenant、Owner 和最近实例上限，浏览器不出现人员 ID、原始错误或写 API。随后由 Owner 不依赖聊天 `status` 命令或开发者代查，从待处理卡判断失败恢复、本人 Human、暂停或草稿的下一步，复制命令或打开详情，并确认按钮点击即时反馈。取得这项证据后，再决定分页筛选、跨轮次对比或继续保持现状。任何可写前端仍必须复用既有预览确认、Owner 重授权和版本栅栏，不能直接改数据库。
