...
- 内容提交 `432fea77c210e7a2cfa5344054eb30d01706bf87` 已关闭受控流程输入的代码、迁移与开发部署门槛。工作台填写的目标、背景和可选协作者先写入 PostgreSQL `DraftRequest`，无飞书凭据的中央 Worker 领取后生成并确定性校验候选 DAG，成功结果只创建草稿并复用现有“确认并启动”。重复提交幂等、候选冻结、租约接管和最多五次失败终止均已覆盖。完整离线套件为 `1015 passed, 23 skipped`；长期库应用第二十三份 migration，真实双 Worker 竞争只有一路领取，部署服务与公网边界正常。同一真实成员主体的临时会话已用纯合成文本通过真实 API 和中央模型创建三节点草稿，数据库回读 `draft / 0 Attempt / 0 Projection`，会话随即注销。下一门槛缩小为真实浏览器完成飞书授权后的可见点击验收，随后进入真实业务受控试用。
...
- Owner 流程操作、普通 Human 任务页面与受控流程输入均已提交和部署。真实登录浏览器已完成一次普通 Human 结果提交和一次跨成员转交；PostgreSQL、outbox、Projection 与飞书 Task 负责人同步均已回读。受控输入坚持先生成草稿与预览、用户明确确认后才启动，不把自由文本伪装成机器人消息，也不直接修改数据库。下一步完成真实登录草稿生成验收，并用项目自身的真实工作连续观察首次结果可用性、退回率、人工干预和重复副作用。
