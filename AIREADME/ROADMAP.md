...
- 内容提交 `1a80b4035d0a5ad5c634af7be957f4b7d1ee37d7` 已把自然语言草稿的慢模型调用隔离到无飞书凭据的 Draft Generation Worker，并增加 `generating / repairing` 进度 revision、最终回复栅栏和覆盖两次模型调用的生成租约预算。代码已通过 `884 passed, 18 skipped`、四个隔离变异与 wheel 内容检查；下一门槛是开发服务器 migration 19、第九服务、真实 PostgreSQL 竞争和真实飞书卡片阶段变化回归。
