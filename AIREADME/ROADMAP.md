...
- 内容提交 `b60cbbd8beb98742cc80082df78ac185274e3a8a` 已关闭第一版受控 DAG 画板的代码和开发部署门槛。React Flow 与 ELK.js 提供真实依赖图、拖动、缩放与自动布局；个人节点位置只保存在浏览器。Owner 可从画板增加、修改或删除未开始节点，也可从选中节点发起返工；结构修改复用 GraphEditPreview，返工复用 RestartPreview，服务端继续重验冻结线、DAG、Owner、版本和影响集合。完整离线套件为 `1019 passed, 23 skipped`，wheel SHA-256 为 `e558f75a2e495d7d1e79e52a1b36458fb55c76ed6eca608e365dc94d43f97221`，已部署到 `/srv/larkflow/target/releases/20260809_175848_console_canvas_b60cbbd/`。本地真实浏览器已完成拖动持久化、节点增改、删除预览取消和四节点返工确认；公网静态资源与 401 边界已回读。真实登录 Owner 随后在服务器纯合成实例完成未来节点修改、三节点返工、末尾 Human 节点新增和循环依赖拒绝，Graph 从 r1 进入 r3，合法变更与审计均只出现一次。下一门槛回到小范围真实业务试用，不提前建设拖拽连边、协同编辑或通用自由白板。
...
- Owner 流程操作、普通 Human 任务页面、受控流程输入与第一版受控 DAG 画板均已提交、部署并完成真实登录浏览器验收。公网纯合成实例已覆盖未来节点修改与新增、图编辑确认、节点返工、旧 Attempt 保留、循环依赖拒绝和 PostgreSQL 审计回读。下一步回到项目自身的真实工作，继续观察首次结果可用性、退回率、人工干预和重复副作用。
...
- Edge 的 macOS 开发试用最小安装、升级、回滚与离线依赖交付已经落地，且没有增加隐藏后台常驻。内容提交 `00067f717ca8e0258b234e81c56e1388226bc471` 已完成安全评审 P0 的第一段：员工 artifact 只包含四个 Edge 模块和最小 initializer，schema v2 bundle 生成精确哈希 lock、SPDX SBOM 与构建证明，安装器逐项校验后使用 `--require-hashes` 断网安装。下一步先取得 Developer ID 身份，完成签名、公证与可信摘要发布，再在全新员工 Mac 验证首装、故障回滚、撤销和数据边界。目录级读取隔离与数据外发治理仍必须在正式分发前关闭。当前持久设备通过受控 SSH 隧道使用；若要形成可持续远程连接，仍需完成 ICP 接入备案或迁移合规地域。
