...
1. `Knowledge Context Service`：只支持管理员发布的企业共享资料与项目级上传，服务端在检索前完成 tenant、Instance、actor、数据分类和模型外发授权。企业资料的 tenant-first 目录、不可变正文 Blob、管理员全员授权证明、撤销门禁、规划和 Agent Attempt 重授权，以及两类来源的确定性 ContextBundle 合并已经落码。当前代码尚待真实 PostgreSQL、0026 migration、Caddy、开发部署和 synthetic Runtime 验证；生产对象存储、语义检索、部门 ACL 与个人知识库后置。
2. `PlannerRuntime Port`：Refactor Phase 1 基线已落码。当前 bounded adapter 只返回候选图与最小运行时摘要，不写草稿、不启动流程；`PlanningService` 会覆盖候选中的服务端输入并通过统一 validator 强制最终复验。Phase 2A 项目附件规划已部署；企业资料切片会在规划前选择当前 tenant 全部有效发布版本，重新授权正文并与项目附件共享预算和 fingerprint。类型化只读工具、显式企业资料 selection、A/B 与该切片的真库和部署验证仍未完成。
3. `AgentRuntime Port` 与 `Authorized Tool Gateway`：completion 基线端口与 Worker bridge 已落码，一次只执行一个节点的一个 Attempt，claim 和版本继续留在 Worker。Phase 2B 项目附件能力信封已部署；企业资料切片已增加显式 `instance_inputs.enterprise_knowledge`、Node/Attempt 重新授权和 `context.read.enterprise_knowledge`，但尚未部署。Tool Gateway 和候选 Runtime 仍未实现，所有业务写继续使用显式 Tool 节点。
...
**Refactor Phase 2B 开发部署证据：** 内容提交 `6e6e3895ac9bb355f40c017e4b5ffe395f4ddca4` 让附件支持的候选图获得服务端拥有的 `instance_inputs.project_attachments`，并在单个 Agent Attempt 开始前重新验证 tenant、Instance、Node、Attempt、原始 planning fingerprint、附件状态、origin、分级、外发、大小、哈希、UTF-8 和字符预算。Runtime 只获得有界正文与短时 `context.read.project_attachments` 能力信封，结果只保存安全 manifest、fingerprint 和运行证据，不持久化正文、object key、claim 或凭据。完整离线合并证据为 `1192 passed, 27 skipped`。测试修正提交 `3a93fd2af9cfecdc00cada822ab8705232018205` 让 PostgreSQL promotion fixture 先创建被外键引用的真实 Instance，没有放松生产约束；一次性 PostgreSQL 从空库运行 24 份 migration 并重入，仓储合同为 `27 passed`。安装态 synthetic Runtime 探针确认正文只在 Runtime 内可见，持久结果不含正文、object key 或 claim。开发环境十个服务为 `active / running / NRestarts=0`。真实 Owner 浏览器附件交互仍是独立产品验收门槛。

**2026-08-19 搜索与原生 Docx 开发验证：** 豆包 `SearchProvider` 已通过真实 synthetic/public 查询回读 10 条带 URL 的规范化来源、usage 和 provider request ID，证明当前开发配置可用，但不把来源数量等同于事实权威性。新建文本交付物已通过 bot 身份创建原生 Docx、用同一 document_id 覆盖并回读 revision 5；原生标题、列表和表格均存在。未配置文件夹 token 时 Target 与 legacy 使用同一 bot 个人空间根，历史 Markdown handle 仍沿兼容路径读写。两项证据都只属于开发测试环境。
...
- Phase 2A 已完成离线、真实 PostgreSQL、Caddy、开发部署和 Owner 作用域 HTTPS API 验收。Phase 2B 的 Agent Attempt 附件引用和可审计能力信封也已完成开发部署、真实 PostgreSQL 合同与 synthetic Runtime 技术探针。企业共享正文、服务器授权证明、规划与 Agent Attempt ContextBundle 合并已提交，下一关是真实 PostgreSQL、0026 migration、Caddy、开发部署和 synthetic Runtime 探针。真实 Owner 浏览器 collecting、txt/md 上传、列表与显式生成的可见交互仍待手工验收；生产对象存储、飞书消息附件、PDF/DOCX/OCR、语义检索、向量检索和 Tool Gateway 继续后置。
...
- 企业共享资料合同由 `ccf04c820b92cd74968ccb25ec7126990ba6fe17` 冻结，目录仓储与 `0025_enterprise_knowledge_catalog` 由 `a7f827f0f1bd4509d6c0f6cb69a8d3d404e3b33e` 落码，服务器管理员 metadata-only API 由 `fc2fc2e73ff0e4001d952f6cba7e4a20e6b04005` 落码并部署。ADR-118 由 `15f193f2be875e4d38783fcbec1de7f506aa1518` 冻结正文与授权证明边界，`88fd70ea5a29bdf7c9f84573fd45a951f73da88c` 增加不可变正文发布、0026 与安全 DTO，`d76a65faca027a558bfadc9b395e5807247705e3` 把重新授权的企业资料接入 Planner 和 Agent Attempt。当前完整离线合并证据为 `1248 passed, 30 skipped`；真库与部署验证尚未完成。部门和个人 ACL、语义检索、个人知识库与 Personal Edge 不在当前切片。
