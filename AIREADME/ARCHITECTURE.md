...
- `repository.py`：Instance 与 Template 仓储 Port，以及仅供测试的 copy-on-read 内存实现；两个 aggregate 分别使用版本拒绝丢失更新。
...
- `service.py`：提供只读草稿预览，并在一次仓储事务内协调草稿确认、调度、执行结果、授权、审计、outbox 与实例终态。
...
- `im_commands.py`：把 `im.message.receive_v1` 的原始 V2 信封和 lark-cli 拍平事件归一为耐久命令信号，按 message / event 去重；凭据侧先验证发送者为当前企业活跃成员，领域侧只接受 `/larkflow help`、`/larkflow start`、`/larkflow confirm`、`/larkflow status` 与 `/larkflow list`，并通过耐久回复队列发送结果。`start` 创建草稿但不自动确认，发送者成为 Instance Owner 与角色绑定来源；`status` 通过领域层 Owner 查询返回单实例有界摘要；`list` 通过独立摘要读模型返回本人最近十个实例，不读取完整聚合。两类查询都不暴露结果正文或人员 ID。
...
| 飞书集成 | PostgreSQL outbox / Inbox、幂等、服务端授权、对账 | Human Task 创建 / 完成、可靠轮询、可选事件、服务端详情回读、两阶段授权、启动对账、受控 Task 重建、`start / confirm / status / help`、自动节点消息、完成 Docx 与最终通知已完成开发真栈验收 | 需要更多命令、受控编辑入口和生产拓扑 |
