...
- `console_drafts.py`：工作台自然语言请求的耐久 Port、内存与 PostgreSQL 仓储、会话主体映射、目录校验、租约 Worker 和安全 DTO。草稿 Worker 依赖本地 `DraftGenerator` 合同，Target 装配通过 `PlanningService` 调用旅行模板与 bounded 组合基线，并传入服务端 tenant、当前 requester 与耐久 console request ID。联网旅行先确定性检查请求中的出发地、起止日期、人数和总预算，缺失字段直接映射为安全公开 DTO；字段完整且搜索 capability 可用时直接产生双 `web.search` 骨架，不进入规划模型 route 或其外层重试。其他候选在 bounded 内部经过生成校验，再由 `PlanningService` 通过同一 `GeneratedDraftValidator` 最终复验，之后才冻结到请求记录。含 Agent 的候选必须以 `accept_reject` Human 决策节点结束，且退回目标必须是它的直接上游 Agent。明确 no-web 且附件证据完整时允许 Human 来源交付、Agent 综合、Human 决定，附件不足则明确返回缺失证据。搜索节点只有在独立 provider 或模型路线显式声明带引用能力时才能进入候选；能力不可用会在生成前阻断。纯 Human 候选不被强制改成决定卡。实例创建使用稳定 ID，成功后只进入草稿，不自动运行。
...
- `search/`、`runner.py` 与 `executors.py`：保留只接受 `work.agent.kind=llm.generate` 的 completion 基线，以及按 `work.tool.kind` 路由的显式 `web.search`、`content.check / source_claims.check / source_decision.check / source_evidence.check`。薄 Python 豆包 adapter 继续只调用固定搜索端点，不接收数据库、飞书凭据、claim 或 DAG 修改权；Responses citation 路线继续兼容。typed 豆包查询只读取 NodeSpec `work.inputs` 明确声明的直接依赖，按节点任务从白名单旅行字段和有界约束词编译最多 100 字的实际 query；交通枢纽只从同一交通语义组的方向词与交通方式之间提取，不能跨分号借用前一个景点 token。`instance_inputs`、未声明依赖、任意字段和敏感标记不会进入查询。成功搜索结果保存 provider、实际查询、标题、摘要、规范 URL、发布时间、用量与安全错误，完成审计也保存同一 query；全部来源若与目的地或本节点关键实体无交集则以 `search_evidence_missing` 拒绝，并继续保留 URL 结构、health、freshness、authority 与 support 分层状态。默认 `DisabledSafeOutboundFetcher` 不含网络客户端，生产 health 明确为 unknown；未来只有满足 SSRF、DNS 与重定向边界的独立 adapter 才能启用。Runner 把直接依赖的 NodeSpec executor、服务端 tool kind 与 Attempt 标识冻结为独立 provenance；`source_evidence.check` 只接受 provenance 为真实 `tool / web.search` 的直接 `source_records`，结果自报、嵌套复制和缺失 provenance 均 fail closed。claim URL 仍须属于该搜索结果且 supporting excerpt 必须是 provider snippet 的有界原文片段；通过只证明片段支持，不证明页面、摘要或事实正确。额度、429、超时、协议、无 URL 和全部不可达使用稳定分类。Runtime 在 claim 前按 adapter 能力筛选具体节点，未接受的 kind 保持 ready。该 provenance 与 query 修复已完成开发部署和安装态 synthetic 探针，生产 URL 健康 adapter 继续保持 unavailable，不把 DSH、PTC 或 LangGraph 引入业务真相边界。
...
| PlannerRuntime | 运行时中立的候选图生成端口、只读工具和确定性复验 | 已有本地合同、`PlanningService`、`GeneratedDraftValidator`、bounded adapter 和 `deterministic_travel_v1`；联网旅行使用确定性五节点骨架，其他领域保留一次生成、最多一次修复，所有 Adapter 最终候选都由 larkflow 强制复验，真实入口携带服务端 tenant、actor 与耐久 request ID。企业资料切片会把重新授权的企业正文与项目附件合并为同一 planning ContextBundle，并已完成开发部署与 synthetic 技术探针 | 缺少更广的类型化只读工具、完整规划证据、持久 metadata、A/B harness，以及真实管理员浏览器验收 |
