...
| Knowledge Context | 企业共享资料加项目上传，服务端授权后形成 `ContextBundle` | Phase 2A 的 Console txt/md 项目上传、冻结 manifest、规划 bundle、默认 deny 外发与安全 Instance refs 已完成真实 PostgreSQL、Caddy、migration、开发部署和 Owner 作用域 API 验收。Phase 2B 的 Agent Attempt 显式附件输入、二次授权读取与正文隔离也已部署和开发验证。企业共享资料已有 provider-neutral 合同、tenant-first PostgreSQL 不可变版本目录、逻辑撤销、每来源单一 published 版本和追加审计 | 企业共享资料尚未接管理员授权、管理 API、正文读取、ContextBundle 合并或 Runtime；还缺生产对象存储、检索、持久运行策略、多机部署和真实浏览器附件交互 |
