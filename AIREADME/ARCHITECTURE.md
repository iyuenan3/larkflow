...
- `console.py`、`console_auth.py`、`console_http.py` 与 `console_cli.py`：Owner 范围的只读 DTO、双鉴权模式和 loopback-only 服务入口。`static` 模式把开发 Bearer token 映射到固定主体；`feishu` 模式使用 OAuth v3、浏览器绑定 state、PKCE S256、显式 `tenant_key` 映射和不透明 HttpOnly 会话，把飞书 `open_id` 映射为 person。用户 access token 只用于服务端读取一次用户身份，随后丢弃，不进入浏览器、日志或流程数据，也不复用 `lark-cli` 用户登录。完成登录后只把凭据 SHA-256 摘要、主体和有效期保存到 PostgreSQL；签发使用事务级 advisory lock 保持全局容量约束，认证、注销和过期清理都由服务端按摘要执行。短期 OAuth state 仍在单进程内存中，不承担已完成登录态。该模式现已通过公网 IP HTTPS、飞书网页应用入口和至少两名真实成员完成 Owner 可见性与跨 Owner 隔离验证，并通过真实 Console 重启后的浏览器登录态回归。业务 API 仍只接受 GET / HEAD；唯一 POST 是注销当前会话，不修改领域聚合。仓储查询同时限定 tenant 与 Instance Owner，详情读取再次校验 Owner。列表查询还在同一有界 Owner 实例集合上连接失败节点和本人 `waiting_human` 节点，由服务端派生失败恢复、Human 待办、暂停继续和草稿确认提示。响应不返回人员 ID、claim token、原始错误正文或 Audit payload，并对列表、审计和结果大小设上限。浏览器页面只用 `textContent` 渲染；DAG 视图依据 DTO 的 `deps` 计算拓扑层级并绘制依赖箭头，不把数组顺序伪装成执行链。待处理卡只能复制现有飞书命令并打开详情，不能直接写聚合。
...
| Owner 控制面 | 浏览流程、DAG、跨轮次 Attempt、审计与派生待处理提示，写操作保持预览确认 | Owner 只读 Console v0 已从真实 PostgreSQL 展示列表、详情、DAG、Attempt 与审计；待处理中心、飞书 OAuth v3、PKCE、tenant 映射和 PostgreSQL 耐久会话均已部署。至少两名真实成员已完成本人 Owner 可见性与跨 Owner 隔离验证，真实登录态已跨 Console 重启保持 | 仍需正式域名、生产限流、管理员后台与管理员级会话治理；还缺分页筛选、协作者可见性、跨轮次对比和任何可写控制面 |
