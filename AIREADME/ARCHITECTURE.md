...
| Project attachment / Context manifest | PostgreSQL + AttachmentBlobStore | Phase 2A 包含草稿请求级 txt/md 元数据、逻辑撤销、保留配额、冻结清单、规划 ContextBundle 与 Instance 安全引用，已完成真实 PostgreSQL、Caddy、migration 和开发部署验证；当前 filesystem adapter 只适合显式配置的单机开发环境，真实 Owner 浏览器验收仍待完成 |
...
- Phase 2A 已提交并部署到开发环境：附件模块为 `35 passed`，完整离线套件为 `1120 passed, 26 skipped`。一次性真实 PostgreSQL 从空库应用 24 份 migration 并重入，`tests/test_workflow_postgres.py` 为 `26 passed`；补充合同覆盖并发 tenant 配额、逻辑撤销保留、Owner 隔离、manifest round-trip、promotion 幂等、删除保护和 migration 事务回滚。Caddy 2.11.4 的 validate/adapt 与公网 64 KB / 256 KB 请求体边界实测通过。开发库为用户明确授权重建的空库，当前 ledger 为 `24 / 0024_console_project_attachments`；十个 Python 服务与 Caddy 均健康。主体提交为 `b2a13ff1eff796723774d42ca5d04556814a38c2`，合同收口提交为 `07de190db49839d8195cfa26967241fad7d975f6`。真实 Owner 浏览器上传与生成仍待手工验收，因此不是生产就绪。
...
| PlannerRuntime | 运行时中立的候选图生成端口、只读工具和确定性复验 | 已有本地合同、`PlanningService`、`GeneratedDraftValidator` 和 bounded adapter；默认保持一次生成、最多一次修复，所有 Adapter 最终候选都由 larkflow 强制复验，真实入口携带服务端 tenant、actor 与耐久 request ID | 缺少企业共享资料、类型化只读工具、完整规划证据、持久 metadata 和 A/B harness |
...
| Knowledge Context | 企业共享资料加项目上传，服务端授权后形成 `ContextBundle` | 已提交的 Phase 2A 包含 Console txt/md 项目上传、冻结 manifest、规划 bundle、默认 deny 外发与安全 Instance refs；只供 DAG 规划 | Agent Attempt 尚不读取附件；缺真实 PostgreSQL 与 Caddy 验证、migration 应用、部署、企业共享清单、生产对象存储、检索、持久运行策略和多机部署 |
