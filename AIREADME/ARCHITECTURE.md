...
| Owner 控制面 | 浏览流程、DAG、跨轮次 Attempt、审计与派生待处理提示，写操作保持预览确认 | Owner 只读 Console v0 已从真实 PostgreSQL 展示列表、详情、DAG、Attempt 与审计；内容提交 `30dc7ee` 与查询边界加固 `b6eda8c` 新增待处理中心。内容提交 `c2e9db99f4b463a895450371dde9b176d6c31ef1` 又在本地实现飞书 OAuth v3、PKCE、tenant 映射、不透明服务端会话、员工工作台登录与开发静态模式兼容，但尚未部署 | 仍需稳定公网 HTTPS、飞书应用回调与主页配置、真实应用内多用户 Owner 隔离验收、耐久会话和管理员后台；还缺分页筛选、协作者可见性、跨轮次对比和任何可写控制面 |
