...
- `console.py`、`console_auth.py`、`console_http.py`、`console_cli.py`、`console_admin.py`、`console_admin_sessions.py` 与 `console_rate_limit.py`：Owner 范围读模型、管理员聚合读模型、受限会话治理、双鉴权模式和 loopback-only 服务入口。`static` 模式把开发 Bearer token 映射到固定主体；`feishu` 模式使用 OAuth v3、浏览器绑定 state、PKCE S256、显式 `tenant_key` 映射和不透明 HttpOnly 会话，把飞书 `open_id` 映射为 person。用户 access token 只用于服务端读取一次用户身份，随后丢弃，不进入浏览器、日志或流程数据，也不复用 `lark-cli` 用户登录。完成登录后只把凭据 SHA-256 摘要、主体、安全 ID 和有效期保存到 PostgreSQL；签发使用事务级 advisory lock 保持全局容量约束，认证、注销和过期清理都由服务端按摘要执行。短期 OAuth state 仍在单进程内存中，不承担已完成登录态。该模式现已通过公网 IP HTTPS、飞书网页应用入口和至少两名真实成员完成 Owner 可见性与跨 Owner 隔离验证，并通过真实 Console 重启后的浏览器登录态回归。Owner 业务 API 仍只接受 GET / HEAD；注销当前会话不修改领域聚合。Owner 仓储查询同时限定 tenant 与 Instance Owner，详情读取再次校验 Owner；管理员资格只由服务端 `tenant + person` allowlist 计算，未授权成员和不存在的管理员路由统一返回 404。管理员聚合仓储只返回当前 tenant 的流程、会话、migration 与七条耐久队列汇总，不返回人员 ID、原始错误、payload、claim 或单条业务记录。会话治理服务只列安全 ID 与 `you / member` 关系，拒绝撤销当前会话；其他会话撤销必须先保存五分钟预览，再在事务中锁定预览与目标、删除会话、消费预览并追加不可变事件。竞争确认只有一路执行，重复回放返回相同结果。`feishu` 管理 POST 还要求精确同源 `Origin` 与专用动作头。Owner 列表查询继续在同一有界实例集合上连接失败节点和本人 `waiting_human` 节点，由服务端派生失败恢复、Human 待办、暂停继续和草稿确认提示。浏览器页面只用 `textContent` 渲染；DAG 视图依据 DTO 的 `deps` 计算拓扑层级并绘制依赖箭头，不把数组顺序伪装成执行链。待处理卡只能复制现有飞书命令并打开详情；会话撤销是当前唯一管理员写操作，不扩展到 allowlist、队列、配置或流程领域写操作。`feishu` 服务的请求边界先执行全局与分路由令牌桶，再进入鉴权和应用分发；来源地址使用进程随机密钥摘要化且不落库。只有 loopback peer 可以提供代理来源头，Caddy 必须覆盖客户端值，因此该地址绝不成为身份事实。LRU 上限控制来源 churn 的内存占用，全局桶控制持续更换来源的总量；该进程内策略是开发公网入口防护，不承担多副本一致限流。
...
| Owner 与管理员控制面 | Owner 浏览流程、DAG、跨轮次 Attempt、审计与派生待处理提示；管理员查看当前企业运行聚合，并经预览确认撤销其他浏览器会话 | Owner Console、待处理中心、飞书 OAuth v3、PKCE、tenant 映射和 PostgreSQL 耐久会话均已部署。至少两名真实成员已完成 Owner 隔离；服务端 allowlist 管理员聚合与会话治理面板均已完成真实浏览器目视，会话撤销已完成真实 PostgreSQL 竞争、管理员授权、普通成员 404、当前会话保护、幂等、审计与部署验收。Caddy 可信来源覆盖、连接边界、完整安全响应头和 Console 有界令牌桶也已完成真实公网回归 | 仍需 allowlist 变更运维流程、恢复演练、生产容量和分布式限流；还缺批量撤销、设备命名、分页筛选、协作者可见性、跨轮次对比和领域写控制面 |
