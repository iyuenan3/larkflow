...
- `executors.py`：包含只接受 `work.agent.kind=llm.generate` 的 `LLMAgentExecutor`、按 `work.tool.kind` 路由内部 adapter 的 `ToolExecutorRouter`、显式公开信息研究 `web.search`、确定性的 `content.check / source_claims.check / source_decision.check`，以及只用于开发验证的 `development.echo`。`web.search` 调用供应商托管搜索，固定限制为无登录、无表单、无预订和无购买的公开研究，输出正文和来源 URL，普通 Agent 不获得隐式联网能力。`source_claims.v1` 用于整理材料，要求区分来源事实、推断和开放问题；`source_decision.v1` 用于形成决定，要求一项优先级、逐一回答全部 Q、3 到 5 条完成标准、不做事项及重新评估条件，并把内容渲染为来源支持的建议推断。两个来源检查器都只检查结构、稳定引用覆盖与来源 URL 一致性，不访问网页或判断事实真伪；决策检查器也不判断建议在业务上是否正确。Runtime 在 claim 前按 adapter 能力筛选具体节点，未接受的 kind 保持 ready，不会先认领后失败。
