...
- Status addendum · 2026-08-19：`console_admin_knowledge.py` 已复用同一服务器管理员 allowlist 增加企业资料 metadata-only 列表、发布、版本审计和幂等撤销。管理员写面现在还包含安全元数据目录，但仍不允许浏览器修改 allowlist、队列、正文、存储位置或服务器配置。
...
| Knowledge Context | 企业共享资料加项目上传，服务端授权后形成 `ContextBundle` | Phase 2A 的 Console txt/md 项目上传、冻结 manifest、规划 bundle、默认 deny 外发与安全 Instance refs 已完成真实 PostgreSQL、Caddy、migration、开发部署和 Owner 作用域 API 验收。Phase 2B 的 Agent Attempt 显式附件输入、二次授权读取与正文隔离也已部署和开发验证。企业共享资料已有 provider-neutral 合同、tenant-first PostgreSQL 不可变版本目录、逻辑撤销、每来源单一 published 版本、追加审计和服务器管理员 metadata-only API | 企业共享资料尚未接正文读取、来源权限证明、ContextBundle 合并或 Runtime；还缺生产对象存储、检索、持久运行策略、多机部署和真实浏览器附件交互 |
