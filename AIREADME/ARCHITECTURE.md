...
- `executors.py`：保留只接受 `work.agent.kind=llm.generate` 的 `LLMAgentExecutor` 作为 completion 兼容基线，并包含按 `work.tool.kind` 路由业务 Tool adapter 的 `ToolExecutorRouter`、显式公开信息研究 `web.search`、确定性的 `content.check / source_claims.check / source_decision.check`，以及只用于开发验证的 `development.echo`。`web.search` 优先使用独立的本地 `SearchProvider` 合同与薄 Python 豆包 Custom API adapter；未配置该 provider 时才兼容显式声明 `responses_citations` 的既有托管 Responses 路线。两条路径都固定限制为无登录、无表单、无预订和无购买的公开研究，并保存 provider、原查询、标题、摘要、来源 URL、发布时间或时间不明、用量与安全错误分类。搜索只检索并规范化证据，信息综合由下游 Agent 完成；没有可验证 URL 的结果不能完成，普通 Agent 也不获得隐式联网能力。普通 Agent 消费 `web.search` 结果时必须披露引用未独立核验，并把时效信息标为执行前复核。`source_claims.v1` 用于整理材料，要求区分来源事实、推断和开放问题；`source_decision.v1` 用于形成决定，要求一项优先级、逐一回答全部 Q、3 到 5 条完成标准、不做事项及重新评估条件，并把内容渲染为来源支持的建议推断。两个来源检查器都只检查结构、稳定引用覆盖与来源 URL 一致性，不访问网页或判断事实真伪；决策检查器也不判断建议在业务上是否正确。Runtime 在 claim 前按 adapter 能力筛选具体节点，未接受的 kind 保持 ready，不会先认领后失败。豆包 adapter 不接收数据库连接、飞书凭据、claim token 或 DAG 修改权，也不把 DSH、PTC 或 LangGraph 引入业务真相边界。
