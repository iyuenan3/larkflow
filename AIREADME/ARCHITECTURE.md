...
- `migrations/` 与 `migrate.py`：PostgreSQL 14 schema、package-data migration 和 advisory lock migration runner；`0010_restart_previews` 保存短期重启授权并为未消费预览建立部分索引，`0011_restart_scope` 增加显式 scope、可空节点键和作用域约束，`0012_graph_edit_previews` 保存未来区域编辑的耐久预览、候选图哈希和消费结果，`0013_im_command_mentions` 为 IM 命令保存最小化 mention 数组。
...
| 责任 | 每节点唯一 Owner，执行器分离 | 新内核已强制 Owner 与 `human/agent/tool` 分离；IM mention 角色绑定与被绑定成员目录校验已落码并离线通过，发送者活跃成员校验已真栈通过，草稿 Owner 全量目录校验已落码但默认关闭 | 需要部署 mention migration、启用跨人员模板并完成真实正向验收 |
...
| 飞书集成 | PostgreSQL outbox / Inbox、幂等、服务端授权、对账 | Human Task 创建 / 完成、可靠轮询、可选事件、服务端详情回读、两阶段授权、启动对账、受控 Task 重建、十个窄命令、自动节点消息、两类重启、未来区域编辑、跨人员非 Owner 拒绝、完成 Docx 与最终通知已进入开发真栈；mention 角色绑定目前只完成离线实现 | 需要真实群聊跨人员正向验收、更多业务命令和生产拓扑 |
