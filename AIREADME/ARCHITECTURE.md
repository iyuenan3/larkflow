...
> 状态：Target + Gap · 既有架构简化版 · 2026-08-07
...
- Console：把一个服务端认证主体映射到 tenant 与 person，只返回该 Instance Owner 的有界流程摘要、DAG、Attempt、审计和待处理 DTO，不提供领域写命令。管理员可额外读取当前 tenant 聚合，并通过耐久预览、显式确认和追加型审计撤销其他浏览器会话；当前会话只能注销。待处理项从同一 PostgreSQL 聚合即时派生，不形成第二套状态。
...
- `migrations/` 与 `migrate.py`：PostgreSQL 14 schema、package-data migration 和 advisory lock migration runner；`0010_restart_previews` 保存短期重启授权，`0011_restart_scope` 增加显式 scope，`0012_graph_edit_previews` 保存未来区域编辑预览，`0013_im_command_mentions` 保存最小化 mention 数组，`0014_role_binding_cards` 保存人员选择卡状态，`0015_recovery_cards` 为失败恢复卡片增加耐久更新 token，`0016_role_card_single_action` 在不删除历史回调的前提下为每张人员选择卡保留一个 canonical 动作并建立部分唯一索引，`0017_card_feedback_metrics` 为人员选择与恢复动作保存完整的首个服务端反馈状态、耗时和完成时间，`0018_worker_wakeups` 为四类耐久工作表安装事务提交后通知触发器，`0019_draft_generation_progress` 为自然语言草稿增加独立生成、阶段进度与租约状态，`0020_console_sessions` 保存员工工作台会话凭据摘要、服务端主体和有效期，`0021_console_session_governance` 增加安全会话 ID、耐久撤销预览和追加型撤销事件。
- `postgres.py` 与 `serde.py`：模板版本、模板审计、JSONB 快照序列化、规范化运行态表、乐观并发仓储、追加型审计与 `FOR UPDATE SKIP LOCKED` outbox。
- `console.py`、`console_auth.py`、`console_http.py`、`console_cli.py`、`console_actions.py`、`console_admin.py`、`console_admin_sessions.py` 与 `console_rate_limit.py`：Owner 范围读模型、Owner 受控操作适配、管理员聚合读模型、受限会话治理、双鉴权模式和 loopback-only 服务入口。`static` 模式把开发 Bearer token 映射到固定主体；`feishu` 模式使用 OAuth v3、浏览器绑定 state、PKCE S256、显式 `tenant_key` 映射和不透明 HttpOnly 会话，把飞书 `open_id` 映射为 person。用户 access token 只用于服务端读取一次用户身份，随后丢弃，不进入浏览器、日志或流程数据，也不复用 `lark-cli` 用户登录。完成登录后只把凭据 SHA-256 摘要、主体、安全 ID 和有效期保存到 PostgreSQL；签发使用事务级 advisory lock 保持全局容量约束，认证、注销和过期清理都由服务端按摘要执行。短期 OAuth state 仍在单进程内存中，不承担已完成登录态。该模式现已通过公网 IP HTTPS、飞书网页应用入口和至少两名真实成员完成 Owner 可见性与跨 Owner 隔离验证，并通过真实 Console 重启后的浏览器登录态回归。Owner 读取 API 继续限制 tenant 与 Instance Owner；写入适配不实现新状态机，只把服务端鉴权主体传给既有 `WorkflowService`。确认草稿、暂停和继续可直接执行，取消与节点或完整实例重启必须先预览再确认；重复执行复用领域层幂等语义。Human 正文与决定继续在飞书入口完成。跨 Owner 与不存在实例统一返回 404，状态、版本或预览冲突返回 409。管理员资格只由服务端 `tenant + person` allowlist 计算，未授权成员和不存在的管理员路由统一返回 404。管理员聚合仓储只返回当前 tenant 的流程、会话、migration 与七条耐久队列汇总，不返回人员 ID、原始错误、payload、claim 或单条业务记录。会话治理服务只列安全 ID 与 `you / member` 关系，拒绝撤销当前会话；其他会话撤销必须先保存五分钟预览，再在事务中锁定预览与目标、删除会话、消费预览并追加不可变事件。竞争确认只有一路执行，重复回放返回相同结果。`feishu` 管理和工作流 POST 都要求精确同源 `Origin`、各自的专用动作头、空 query 与空 body。Owner 列表查询继续在同一有界实例集合上连接失败节点和本人 `waiting_human` 节点，由服务端派生失败恢复、Human 待办、暂停继续和草稿确认提示。浏览器页面只用 `textContent` 渲染；DAG 视图依据 DTO 的 `deps` 计算拓扑层级并绘制依赖箭头，不把数组顺序伪装成执行链。待处理卡和详情操作栏在请求发出前立即显示处理中；高风险操作在同页展示服务端影响预览并要求显式确认。管理员写面仍只包含其他会话撤销，不扩展到 allowlist、队列或配置。`feishu` 服务的请求边界先执行读取、认证、管理员写入、Owner 工作流写入和全局令牌桶，再进入鉴权和应用分发；来源地址使用进程随机密钥摘要化且不落库。只有 loopback peer 可以提供代理来源头，Caddy 必须覆盖客户端值，因此该地址绝不成为身份事实。LRU 上限控制来源 churn 的内存占用，全局桶控制持续更换来源的总量；该进程内策略是开发公网入口防护，不承担多副本一致限流。
...
- `cli.py`：独立 `larkflow-target` 运维入口，提供模板全生命周期、从模板创建草稿、预览、确认、状态、Human 提交，以及 Runtime、Projection、Interactive、Draft Generation、入站校验和领域入站的单步 / 常驻服务；`generate-drafts-once / generate-drafts` 运行无凭据草稿生成车道，`reconcile-instance-completion` 可显式修复一个已完成实例缺失的完成文档或最终通知。独立 `larkflow-console` 装配 Owner 读仓储、受控操作适配与 HTTP 页面；操作适配复用 `WorkflowService`，不复用 CLI 文本解析，也不建立浏览器专属领域命令。
- `executors.py`：包含只接受 `work.agent.kind=llm.generate` 的 `LLMAgentExecutor`、按 `work.tool.kind` 路由内部 adapter 的 `ToolExecutorRouter`、确定性的 `content.check / source_claims.check / source_decision.check`，以及只用于开发验证的 `development.echo`。`source_claims.v1` 用于整理材料，要求区分来源事实、推断和开放问题；`source_decision.v1` 用于形成决定，要求一项优先级、逐一回答全部 Q、3 到 5 条完成标准、不做事项及重新评估条件，并把内容渲染为来源支持的建议推断。两个来源检查器都只检查结构、稳定引用覆盖与来源 URL 一致性，不访问网页或判断事实真伪；决策检查器也不判断建议在业务上是否正确。Runtime 在 claim 前按 adapter 能力筛选具体节点，未接受的 kind 保持 ready，不会先认领后失败。
...
- `edge_client.py`、`edge_agent.py` 与 `edge_cli.py`：用户设备提供 `pair`、离线 `doctor`、单次 `run-once` 和用户主动保持的前台 `serve`。`doctor` 只校验本机凭据、Codex 命令和传输形状，不连接中央节点或领取工作。常驻循环固定显式单工作区，使用有界长轮询、退避、应用心跳和结构化日志；同一凭据只能由一个本机 Worker 使用。Codex adapter 使用只读、临时会话和忽略用户配置模式，清除 Edge、Target 与飞书凭据环境变量，并在超时、服务停止或租约续期失败时按进程组终止子进程。
- `deploy/larkflow-edge-manager.py` 与 `deploy/build-larkflow-edge-bundle.py`：macOS 当前用户的独立安装控制面和发布方离线构建器。builder 为一个明确的 Mac 架构与 Python 次版本下载 binary-only wheelhouse，记录 source commit、目标、主 artifact、全部文件哈希和 wheel 包清单，并加入已修复的 bootstrap pip。manager 先验证独立 manifest SHA-256、精确文件集、目标和 wheel metadata，再断网安装；它在最终 `releases/<version>-<sha12>/venv` 路径创建虚拟环境，完成 `pip check` 与已安装 CLI 启动校验后才原子切换 `current`，并保留一个 `previous` 回滚点。`~/.local/bin` 只链接到稳定入口；manager 不修改系统 Python、Keychain 或 launchd，也不连接中央节点。
...
| 草稿与模板可选 | 草稿预览、确认、模板或无模板实例 | 新内核支持直接 Snapshot 草稿，以及模板参数和角色绑定生成的冻结草稿；Owner 可只读预览并独立确认；飞书 IM 已提供模板 `start`、自然语言引导 `draft` 和结构化高级 `draft`，三条路径均已完成开发真栈闭环。自然语言验收还覆盖即时处理中、非法首候选的有界修复、唯一草稿、独立确认、Agent 与 Human 执行、耐久 Task 入站、最终无控件卡片、完成 Docx 和最终通知；公开软件需求材料已完成来源约束型接受和退回重启恢复路径 | 需要更完整的模板管理入口、图形化无模板编辑体验和受控内部试用的产品价值证据 |
...
| Owner 与管理员控制面 | Owner 浏览流程、DAG、跨轮次 Attempt、审计与派生待处理提示，并执行本人实例的受控流程操作；管理员查看当前企业运行聚合，并经预览确认撤销其他浏览器会话 | Owner Console、待处理中心、飞书 OAuth v3、PKCE、tenant 映射和 PostgreSQL 耐久会话均已部署。至少两名真实成员已完成 Owner 隔离；确认、暂停和继续直接复用领域服务，取消与重启保留预览确认、版本栅栏、幂等和历史。真实登录 Owner 已从公网工作台直接确认并启动三节点实例，实例最终完成，Human Task、Agent 结果、完成文档和通知均有外部绑定。服务端 allowlist 管理员聚合与会话治理面板均已完成真实浏览器目视，会话撤销已完成真实 PostgreSQL 竞争、管理员授权、普通成员 404、当前会话保护、幂等、审计与部署验收。Caddy 可信来源覆盖、连接边界、完整安全响应头和 Console 有界令牌桶也已完成真实公网回归 | 仍需网页 Human 责任入口、待办转交、暂停继续取消重启的逐项页面验收、生产容量和分布式限流；还缺批量撤销、设备命名、分页筛选、协作者可见性、跨轮次对比与运行中图编辑 |
