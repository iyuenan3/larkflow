...
| Owner 控制面 | 浏览流程、DAG、跨轮次 Attempt、审计与派生待处理提示，写操作保持预览确认 | Owner 只读 Console v0 已从真实 PostgreSQL 展示列表、详情、DAG、Attempt 与审计；待处理中心、飞书 OAuth v3、PKCE、tenant 映射和不透明服务端会话均已部署。至少两名真实成员已从飞书工作台网页入口完成本人 Owner 可见性与跨 Owner 隔离验证 | 仍需 PostgreSQL 耐久会话、重启后登录态回归、正式域名与管理员后台；还缺分页筛选、协作者可见性、跨轮次对比和任何可写控制面 |
