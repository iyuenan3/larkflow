...
| Owner 与管理员控制面 | Owner 浏览流程、DAG、跨轮次 Attempt、审计与派生待处理提示；管理员查看当前企业运行聚合，并经预览确认撤销其他浏览器会话 | Owner Console、待处理中心、飞书 OAuth v3、PKCE、tenant 映射和 PostgreSQL 耐久会话均已部署。至少两名真实成员已完成 Owner 隔离；服务端 allowlist 管理员聚合已完成真实浏览器目视，会话撤销已完成真实 PostgreSQL 竞争、管理员授权、普通成员 404、当前会话保护、幂等、审计与部署验收 | 仍需会话治理面板真实登录浏览器目视、生产限流、安全响应头和 allowlist 变更运维流程；还缺批量撤销、设备命名、分页筛选、协作者可见性、跨轮次对比和领域写控制面 |
