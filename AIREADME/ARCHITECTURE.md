...
| Project attachment / Context manifest | PostgreSQL + AttachmentBlobStore | Phase 2A 未提交候选包含草稿请求级 txt/md 元数据、逻辑撤销、保留配额、冻结清单、规划 ContextBundle 与 Instance 安全引用；当前 filesystem adapter 只适合显式配置的单机开发环境，仍待真实 PostgreSQL 与 Caddy 验证 |
...
- `agent_runtime/`：提供纯本地 `AgentRunRequest / AgentRunResult / AgentRuntime` 合同、包住 `LLMAgentExecutor` 的 `CompletionAgentRuntime`，以及连接 `WorkflowWorker` 的 `AgentRuntimeExecutor`。桥接只下发节点合同、冻结输入和稳定 `tenant:attempt` 幂等键；claim token、租约到期时间和 expected node version 留在 Worker 内，Runtime 不能提交业务状态。
...
- `console_drafts.py`：工作台自然语言请求的耐久 Port、内存与 PostgreSQL 仓储、会话主体映射、目录校验、租约 Worker 和安全 DTO。草稿 Worker 依赖本地 `DraftGenerator` 合同，Target 装配通过 `PlanningService` 调用 bounded 基线，并传入服务端 tenant、当前 requester 与耐久 console request ID。候选先在 bounded 内部经过生成校验，再由 `PlanningService` 通过同一 `GeneratedDraftValidator` 最终复验，之后才冻结到请求记录。含 Agent 的候选必须以 `accept_reject` Human 决策节点结束，且退回目标必须是它的直接上游 Agent；旅游规划还必须先收集四类核心出行需求，把景点和交通拆成独立 `web.search` 节点，再由 Agent 汇总。该约束由服务端确定性校验，不依赖模型自律；联网能力未开启时直接拒绝旅游候选。纯 Human 候选不被强制改成决定卡。实例创建使用稳定 ID，成功后只进入草稿，不自动运行。
- `executors.py`：保留只接受 `work.agent.kind=llm.generate` 的 `LLMAgentExecutor` 作为 completion 兼容基线，并包含按 `work.tool.kind` 路由业务 Tool adapter 的 `ToolExecutorRouter`、显式公开信息研究 `web.search`、确定性的 `content.check / source_claims.check / source_decision.check`，以及只用于开发验证的 `development.echo`。`web.search` 调用供应商托管搜索，固定限制为无登录、无表单、无预订和无购买的公开研究，输出正文和来源 URL，普通 Agent 不获得隐式联网能力。普通 Agent 消费 `web.search` 结果时必须披露引用未独立核验，并把时效信息标为执行前复核。`source_claims.v1` 用于整理材料，要求区分来源事实、推断和开放问题；`source_decision.v1` 用于形成决定，要求一项优先级、逐一回答全部 Q、3 到 5 条完成标准、不做事项及重新评估条件，并把内容渲染为来源支持的建议推断。两个来源检查器都只检查结构、稳定引用覆盖与来源 URL 一致性，不访问网页或判断事实真伪；决策检查器也不判断建议在业务上是否正确。Runtime 在 claim 前按 adapter 能力筛选具体节点，未接受的 kind 保持 ready，不会先认领后失败。
...
| PlannerRuntime | 运行时中立的候选图生成端口、只读工具和确定性复验 | 已有本地合同、`PlanningService`、`GeneratedDraftValidator` 和 bounded adapter；默认保持一次生成、最多一次修复，所有 Adapter 最终候选都由 larkflow 强制复验，真实入口携带服务端 tenant、actor 与耐久 request ID | 缺少授权知识上下文、类型化只读工具、完整规划证据、持久 metadata 和 A/B harness |
| AgentRuntime | 单节点、单 Attempt 的可替换执行端口 | 已有本地合同、completion adapter 和 Worker bridge；默认仍只接受声明式 `llm.generate`，一次执行一次 completion，claim 与版本不下发 | 缺少能力信封、只读 Tool Gateway、统一 trace、持久 policy 快照和候选适配器 |
| Knowledge Context | 企业共享资料加项目上传，服务端授权后形成 `ContextBundle` | Phase 2A 未提交候选包含 Console txt/md 项目上传、冻结 manifest、规划 bundle、默认 deny 外发与安全 Instance refs；只供 DAG 规划 | Agent Attempt 尚不读取附件；缺真实 PostgreSQL 与 Caddy 验证、企业共享清单、生产对象存储、检索、持久运行策略和多机部署 |
