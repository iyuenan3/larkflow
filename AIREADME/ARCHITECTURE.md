...
| Owner 与管理员控制面 | Owner 浏览流程、DAG、跨轮次 Attempt、审计与派生待处理提示，并执行本人实例的受控流程操作；管理员查看当前企业运行聚合，并经预览确认撤销其他浏览器会话 | Owner Console、待处理中心、飞书 OAuth v3、PKCE、tenant 映射和 PostgreSQL 耐久会话均已部署。至少两名真实成员已完成 Owner 隔离；确认、暂停和继续直接复用领域服务，取消与重启保留预览确认、版本栅栏、幂等和历史。服务端 allowlist 管理员聚合与会话治理面板均已完成真实浏览器目视，会话撤销已完成真实 PostgreSQL 竞争、管理员授权、普通成员 404、当前会话保护、幂等、审计与部署验收。Caddy 可信来源覆盖、连接边界、完整安全响应头和 Console 有界令牌桶也已完成真实公网回归 | 仍需真实登录浏览器完成首轮 Owner 写操作验收、生产容量和分布式限流；还缺批量撤销、设备命名、分页筛选、协作者可见性、跨轮次对比、运行中图编辑与 Human 正文提交 |
