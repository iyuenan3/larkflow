...
| Project attachment / Context manifest | PostgreSQL + AttachmentBlobStore | 已提交的 Phase 2A 包含草稿请求级 txt/md 元数据、逻辑撤销、保留配额、冻结清单、规划 ContextBundle 与 Instance 安全引用；当前 filesystem adapter 只适合显式配置的单机开发环境，仍待真实 PostgreSQL 与 Caddy 验证、部署和 migration 应用 |
...
- Phase 2A 已提交验证：附件模块为 `35 passed`；完整套件清空六个代理变量后为 `1119 passed, 26 skipped`，唯一因沙箱禁止 `ps` 的既有测试在允许读取进程树后单独 `1 passed`，合并结果为 `1120 passed, 26 skipped`。真实 PostgreSQL 合同测试已扩展 retained 配额、resolve、撤销和 promotion，但本机没有配置一次性测试 DSN；本机也没有 Caddy 二进制，所以本轮不声称 `0024` 真库或 `caddy validate` 已通过。代码已纳入内容提交 `b2a13ff1eff796723774d42ca5d04556814a38c2` 并推送，但尚未部署或应用 migration，不能把 Phase 2A 标记为完成。
