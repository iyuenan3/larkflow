...
| 飞书集成 | PostgreSQL outbox / Inbox、幂等、服务端授权、对账 | Human Task 创建 / 完成、可靠轮询、可选事件、服务端详情回读、两阶段授权、启动对账、受控 Task 重建、十个窄命令、自动节点消息、两类重启、未来区域编辑、跨人员非 Owner 拒绝、完成 Docx 与最终通知已进入开发真栈 | 需要从已发布版本重做最小权限回归、更多业务命令和生产拓扑 |
