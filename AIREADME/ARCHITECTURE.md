...
- `role_bindings.py`：把需要跨人员分工但未显式 mention 的 `start` 转成人员选择卡，也承载裸 `draft` 的自然语言引导卡。两者复用候选人快照、卡片发送、回调事件、目录再验证、领域创建、卡片回写和文本回复的耐久链路，并用请求 `kind` 区分语义。操作人、候选集合、角色集合、表单字段和实例 ID 都由服务端重算；普通人员分工 Worker 明确排除自然语言草稿动作，后者由独立生成进程认领。回调动作耐久插入后立即排队 `generating` 进度，确定性校验拒绝首个候选时再排队 `repairing`，成功后同一原卡片冻结为人员确认结果或图预览。同一 message 只有一个 canonical 动作，重复回调不创建第二个草稿，也不重复调用模型；迁移前的重复行保留为非 canonical 历史。
