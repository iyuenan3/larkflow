...
| Project attachment / Context manifest | PostgreSQL + AttachmentBlobStore | Phase 2A 包含草稿请求级 txt/md 元数据、逻辑撤销、保留配额、冻结清单、规划 ContextBundle 与 Instance 安全引用，已完成真实 PostgreSQL、Caddy、migration、开发部署和 Owner 作用域 HTTPS API 验收；当前 filesystem adapter 只适合显式配置的单机开发环境，真实 Owner 浏览器附件交互仍待完成 |
...
- `executors.py`：保留只接受 `work.agent.kind=llm.generate` 的 `LLMAgentExecutor` 作为 completion 兼容基线，并包含按 `work.tool.kind` 路由业务 Tool adapter 的 `ToolExecutorRouter`、显式公开信息研究 `web.search`、确定性的 `content.check / source_claims.check / source_decision.check`，以及只用于开发验证的 `development.echo`。`web.search` 只选择显式声明 `responses_citations` 的供应商路线，固定限制为无登录、无表单、无预订和无购买的公开研究，输出正文和供应商结构化引用 URL，普通 Agent 不获得隐式联网能力。普通 Agent 消费 `web.search` 结果时必须披露引用未独立核验，并把时效信息标为执行前复核。`source_claims.v1` 用于整理材料，要求区分来源事实、推断和开放问题；`source_decision.v1` 用于形成决定，要求一项优先级、逐一回答全部 Q、3 到 5 条完成标准、不做事项及重新评估条件，并把内容渲染为来源支持的建议推断。两个来源检查器都只检查结构、稳定引用覆盖与来源 URL 一致性，不访问网页或判断事实真伪；决策检查器也不判断建议在业务上是否正确。Runtime 在 claim 前按 adapter 能力筛选具体节点，未接受的 kind 保持 ready，不会先认领后失败。
...
| AgentRuntime | 单节点、单 Attempt 的可替换执行端口 | 已有本地合同、completion adapter 和 Worker bridge；默认仍只接受声明式 `llm.generate`，一次执行一次 completion，支持 metadata 的回复必须通过结束原因、完成 envelope、验收 ID 与正文短锚点校验，claim 与版本不下发 | 缺少能力信封、只读 Tool Gateway、统一 trace、持久 policy 快照和候选适配器 |
| Knowledge Context | 企业共享资料加项目上传，服务端授权后形成 `ContextBundle` | Phase 2A 的 Console txt/md 项目上传、冻结 manifest、规划 bundle、默认 deny 外发与安全 Instance refs 已完成真实 PostgreSQL、Caddy、migration、开发部署和 Owner 作用域 API 验收；只供 DAG 规划 | Agent Attempt 尚不直接读取附件，当前 no-web 路径由首个 Human 节点交付完整已确认来源；还缺企业共享清单、生产对象存储、检索、持久运行策略、多机部署和真实浏览器附件交互 |
