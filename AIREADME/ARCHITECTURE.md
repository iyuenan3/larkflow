...
| Project attachment / Context manifest | PostgreSQL + AttachmentBlobStore | Phase 2A 包含草稿请求级 txt/md 元数据、逻辑撤销、保留配额、冻结清单、规划 ContextBundle 与 Instance 安全引用，已完成真实 PostgreSQL、Caddy、migration、开发部署和 Owner 作用域 HTTPS API 验收；Phase 2B 已提交显式 Agent 输入、Node/Attempt 绑定的执行 bundle 与短时只读能力信封，但尚未部署或真库验证；当前 filesystem adapter 只适合显式配置的单机开发环境，真实 Owner 浏览器附件交互仍待完成 |
...
| AgentRuntime | 单节点、单 Attempt 的可替换执行端口 | 已有本地合同、completion adapter 和 Worker bridge；默认仍只接受声明式 `llm.generate`，一次执行一次 completion，支持 metadata 的回复必须通过结束原因、完成 envelope、验收 ID 与正文短锚点校验，claim 与版本不下发。Phase 2B 已提交 Node/Attempt 绑定的只读附件 capability envelope 与安全运行证据 | 尚未部署和真库验证 Phase 2B；缺少只读 Tool Gateway、统一 trace、持久 policy 快照和候选适配器 |
| Knowledge Context | 企业共享资料加项目上传，服务端授权后形成 `ContextBundle` | Phase 2A 的 Console txt/md 项目上传、冻结 manifest、规划 bundle、默认 deny 外发与安全 Instance refs 已完成真实 PostgreSQL、Caddy、migration、开发部署和 Owner 作用域 API 验收。Phase 2B 已提交 Agent Attempt 的显式附件输入、二次授权读取与正文隔离 | Phase 2B 尚未部署和真库验证；还缺企业共享清单、生产对象存储、检索、持久运行策略、多机部署和真实浏览器附件交互 |
