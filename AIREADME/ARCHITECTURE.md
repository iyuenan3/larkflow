...
- `knowledge_context.py`：把当前 tenant 中带服务端全员授权证明的 published 企业资料组装为 planning ContextBundle，并在单个 Agent Attempt 前按冻结 ref 重新授权。服务先校验版本、授权、分级与外发，再读取并校验正文大小、SHA-256、UTF-8、字符预算和 TTL；发行前必须通过仓储原子最终授权复验。PostgreSQL 仓储在与撤销相同的 source advisory transaction lock 下重读精确 ref、published 状态、proof 与外发决定，以事务完成点作为 ContextBundle 发行线性化点。撤销先完成则本次 fail closed，最终授权先完成则 bundle 视为已发行且撤销只阻断后续发行。项目附件保持冻结 manifest 顺序，企业资料按 source/version 排序后追加；Runtime 不获得 BlobStore、repository、object key、管理员身份或原始授权声明，旧 Attempt 的安全 manifest 与 fingerprint 不被改写。
