...
migration `0023_console_draft_requests` 保存请求、输入、生成状态、尝试次数、可用时间、短租约、冻结候选定义、实例绑定、安全错误码与完成时间；已提交但尚未应用的 migration `0024_console_project_attachments` 增加 collecting、冻结 manifest 和 tenant-first 附件元数据。独立 Draft Generation Worker 在同一 PostgreSQL 事务中通过 `FOR UPDATE SKIP LOCKED` 领取；候选一旦通过确定性校验，就必须先在当前 claim 下保存，再用 `console_draft_<request_id>` 作为稳定实例 ID 创建未启动 Snapshot。Worker 在保存候选后崩溃时，接管者复用同一冻结候选，不能再次调用模型。基础设施失败采用有界退避，默认最多五次后进入 `exhausted`；确定性拒绝直接进入 `rejected`。成功请求只创建 `draft / 0 Attempt`，仍由既有确认接口启动。
