...
macOS 开发试用安装由独立 `larkflow-edge-manager` 管理。`install --wheel <path> --sha256 <hex>` 要求标准、非符号链接 wheel 和完整 64 位 SHA-256，读取 wheel metadata 后只接受包名 `larkflow`，并选择 Python 3.10 或更高版本。release ID 固定为 `<package-version>-<sha12>`；虚拟环境必须直接创建在最终 release 路径，验证成功前不得切换。`status` 只输出非敏感安装状态；`rollback` 交换 `current / previous`，保留 release 内容。稳定命令为 `~/.local/bin/larkflow-edge` 与 `~/.local/bin/larkflow-edge-manager`。manager 拒绝 root、符号链接目录和已有的无关同名命令，不读取或迁移 Keychain，不注册后台服务，也不提供联网自动更新。
