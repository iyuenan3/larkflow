...
企业资料目录管理复用 `LARKFLOW_CONSOLE_ADMIN_PERSON_IDS` 与当前 Target tenant，不新增浏览器可声明的管理员身份。`GET /console/api/v1/admin/knowledge?limit=<1..100>` 返回当前 tenant 的 published 与 revoked 版本安全元数据；`GET /console/api/v1/admin/knowledge/sources/<source_id>/versions/<version_id>/audit?limit=<1..100>` 返回追加审计，但 actor 只表示 `you / member`。`POST /console/api/v1/admin/knowledge/publications` 只接受 `source_id / version_id / display_label / media_type / size_bytes / content_sha256 / egress_decision` 七个字段，tenant、publisher、发布时间、分级与状态由服务端产生。`POST /console/api/v1/admin/knowledge/sources/<source_id>/versions/<version_id>/revoke` 不接受 query 或正文，重复撤销保持幂等且不新增审计。两个写端点都要求 `X-Larkflow-Console-Action: knowledge-governance-v1`；带正文的发布还要求严格 JSON、精确 `Content-Length`、不超过 65536 字节和正确 `Content-Type`，`feishu` 模式继续要求精确同源 `Origin`。非管理员与跨 tenant 统一 404，版本冲突为 409 `knowledge_conflict`。响应不含 tenant、person ID、正文、object key、源系统 locator 或凭据。`GET /console/api/v1/auth` 只对已认证管理员返回 `capabilities.enterprise_knowledge_catalog=true`。该 API 当前只登记安全元数据，不证明正文存在、来源有全员使用权或 Runtime 已获授权；正文与来源权限证明接入前不能把目录版本当成可用企业知识。
