...
- `/larkflow draft` 与 `/larkflow draft <JSON定义> [role=@成员 ...]`：两种形式都不查找模板版本，只创建 `template_version_id=NULL`、`locked=false` 的 Instance Snapshot 草稿，仍需独立 `confirm` 才启动。带 JSON 的高级入口沿用严格解析、100 节点上限、`human / agent / tool` 完整 work 契约与 mention Owner 验证，拒绝重复键、非有限数、模型服务配置和 `personal.readonly`。裸命令打开 Card 2.0 引导，必填目标、可选背景，并从凭据侧冻结的活跃人员快照中选择一名协作者。回调只接受原发起人、原消息与卡片，以及服务端允许的表单字段；协作者在领域处理前再次验证。中央 Agent 只能生成最多八个 `human / agent` 节点，Owner 角色只能是 `requester / collaborator`，Agent 节点后必须直接有人类复核。模型输出按严格 JSON 解码，服务端覆盖 `schema_version` 与原始输入并重新校验完整 Snapshot，不能借生成结果引入 Tool、服务配置或 Personal Edge capability。首个候选未通过相同确定性校验时，服务端最多让中央 Agent 重生成一次；第二个候选仍不合法就拒绝。动作落库后由独立无凭据 Draft Generation Worker 认领，凭据侧按 revision 先显示无按钮“正在生成”，需要第二次模型调用时显示“正在修复”，成功后同一卡片显示节点、Owner、依赖和独立确认命令。普通人员分工 Worker 不认领该动作；最终回复必须等待当前进度更新结算，旧 revision 不能覆盖终态。旧拓扑已完成真实飞书验收；独立生成拓扑在内容提交 `1a80b4035d0a5ad5c634af7be957f4b7d1ee37d7` 落码，本段提交时仍待开发部署与真实卡片回归。
