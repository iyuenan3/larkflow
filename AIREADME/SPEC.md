...
节点和完整实例重启都已实现。可重启实例状态为 `running / done / failed`。节点 scope 的目标节点状态必须为 `running / waiting_human / done / failed`，且目标节点直接依赖必须完成；失败实例若存在影响集合之外的失败节点会拒绝节点重启，避免把仍有未处理失败的实例错误恢复为 running。完整实例 scope 覆盖全图，因此不需要外部失败节点例外。运行中图编辑仍未实现。
