...
migration `0025_enterprise_knowledge_catalog` 增加 tenant-first 企业共享资料版本目录与追加审计。目录只保存安全元数据，不保存正文或对象 key；同一 tenant 与 source 同时最多一个 `published` 版本，既有版本除一次 `published -> revoked` 外不可修改，版本和审计都禁止物理删除。仓储对 tenant 与 source 使用事务级 advisory lock，使重复发布幂等、竞争版本只有一个成功；撤销重复调用不追加第二条审计。仓储参数中的 publisher 仍是服务端调用参数，不构成管理员授权边界；在管理员服务接入前没有公开 HTTP 或飞书入口。
