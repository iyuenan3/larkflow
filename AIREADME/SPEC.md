...
该开发入口没有正式域名，进程内令牌桶也不是多副本或分布式生产限流。当前仍没有通用流程输入框、allowlist 自助管理、批量撤销、设备命名、跨区域容灾、生产容量证明或正式员工交付方案。工作台只提供参与者任务面，不提供完整协作者实例视图、分页游标、筛选、跨轮次 diff 或运行中图编辑。
