...
- 当前 Target 应用 wheel 对应内容提交 `ed118e7b3a9eeb5b5daed52e3d7b0296896f12f1`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。统一重启后九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / running / NRestarts=0`。公网与 loopback `/console/` 均返回 200，未认证任务 API 返回 401。飞书 OAuth 应用凭证与管理员 allowlist 继续只保存在受限 env。
- 当前发布件为 `/srv/larkflow/target/releases/20260809_0201_outbox_exhaustion_ed118e7/larkflow-0.0.2-py3-none-any.whl`，权限为 `0640 root:lf_target_dev`，SHA-256 为 `a9f68581294ac65e71b2eae5f97940618289194eedd77c5943c40f539e4f6245`。wheel 内 `projection.py` 与共享 venv 安装文件 SHA-256 均为 `79cd77ced73f86033f666ec96a6c92e974562af949d149b8e1d8120c09700eb3`；服务器 `pip check` 无断裂依赖。
...
- 当前部署的参与者任务 API 已在真实登录态会话中返回 200，成员目录返回 4 名应用可见成员。验收主体当时没有自然等待中的普通 Human Task，因此列表为 0；本轮不伪造长期库任务，也不把 API 读回外推为真实浏览器提交或真实飞书 Task 转交已通过。
...
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，鉴权模式为 `feishu`。App secret 与 OAuth token 不进入命令行、浏览器或日志；静态 token 只保留为 loopback 开发回退。未认证业务 API 返回 401。Owner 完整实例视图继续按 tenant 与 Instance Owner 隔离；普通 Human Task 参与者只能读取任务有界上下文并提交或转交，决定节点继续使用飞书卡片。既有标签页不会自动替换已加载的旧 JavaScript，部署后验收必须显式刷新。下一项真实验收是自然任务的页面提交与飞书 Task 负责人转交，不需要继续为纯计时反复点击。
