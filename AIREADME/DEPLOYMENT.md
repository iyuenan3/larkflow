...
- 当前 Target 开发发布件对应内容提交 `e3bd98d155a446a66bdb2c947e124f7ba7fc9c31`，只重构 Owner 工作台的信息架构、字号和浅色、深色主题，不改变上一版来源约束决策契约或服务端领域接口。从该提交构建的干净 wheel SHA-256 为 `7b7b5318c4f94b096210629f5c94db1d2369ee2cb94f5e66cfc146ab8a2a5178`，位于 `/srv/larkflow/target/releases/20260808_231502_console_ui_e3bd98d/`，权限为 `0640 root:lf_target_dev`。完整离线等价结果为 `988 passed, 21 skipped`，服务器 `pip check` 无断裂依赖；源码、wheel、临时安装和服务器安装后的三项静态资源哈希逐项一致。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260808T231502+0800.dump` 为 259027 bytes，SHA-256 为 `3018163deb374eb4a7376e56c26eb381e6b78772cd267451adc9dfa742e5ff3a`，权限为 `0600 lf_target_dev:lf_target_dev`；备份服务回读 `Result=success / ExecMainStatus=0`。本次没有 schema 变更，migration runner 返回空集，ledger 仍为 `21 / 0001_workflow / 0021_console_session_governance`。
- 本次只重启 `larkflow-target-console.service`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / running`，Console 为 `NRestarts=0`，部署窗口 warning 为 0。5432、8765 与 8780 只监听 `127.0.0.1`，Caddy 管理口只监听 `127.0.0.1:2019`。Edge 公网链路仍保持关闭，不能把 Console 入口外推为 Personal Agent Edge 已具备公网 E2E。
...
- 当前 Target 开发发布件对应内容提交 `e3bd98d155a446a66bdb2c947e124f7ba7fc9c31`，在既有自然语言草稿、来源约束复核、返工上下文、生命周期、Owner Console、待处理中心、PostgreSQL 耐久会话、管理员聚合、受审计会话撤销和公网请求边界基础上重构本人工作台。长期库仍为二十一份 migration，本次没有 schema 变更。干净 wheel 位于 `/srv/larkflow/target/releases/20260808_231502_console_ui_e3bd98d/`，SHA-256 为 `7b7b5318c4f94b096210629f5c94db1d2369ee2cb94f5e66cfc146ab8a2a5178`；上一版来源决策发布件继续保留为可恢复回滚点。本次只重启 Console，九个 Target 服务、legacy 消费者与 Caddy 均回读 active，Console `NRestarts=0`，部署窗口 warning 为 0。该版本尚待真实登录浏览器刷新后完成新版视觉复验。
