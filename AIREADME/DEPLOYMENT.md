...
- 当前开发发布件对应内容提交 `a506e7df078cbb6d8fa5f359272505ba62cac241`，包含 PostgreSQL Worker 通知唤醒、轮询兜底、所有可操作卡片的耐久后即时视觉反馈、人员选择卡单动作约束、首个服务端反馈耐久指标、批次 Worker 逐项完成计时，以及既有失败恢复能力。完整离线套件为 `788 passed, 16 skipped`；长期库第十八份 migration、六服务部署、四条真实监听连接、一次性 PostgreSQL 逐项时间持久化和五次人员选择卡真栈验收均已通过。
...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、十个 IM 窄命令、人员选择卡、失败恢复卡、未来区域编辑、节点与完整实例安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询、PostgreSQL Worker 通知唤醒、批次 Worker 逐项完成计时和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前代码发布 wheel SHA-256 为 `385e9b2a272246b683d49b7232dcbbdd788aa1ee2efa051ee7cdff95bca46b6b`，保存在 `releases/20260804_185244_item_completion_a506e7d/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `a506e7df078cbb6d8fa5f359272505ba62cac241`；升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260804T185325+0800.dump` 已读回为成功、`0600`、`lf_target_dev` 所有且 136056 bytes，长期库已应用十八份 migration。代码发布后的六个 Python 服务均回读 `active / running / NRestarts=0`，部署窗口 warning 级日志数为 0。安装使用现有 root 管理的虚拟环境；Target 与 legacy 在同一停机窗口统一安装同一 wheel，migration 成功后才统一启动，不存在混合运行窗口。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
...
- 开发交互延迟配置仍保留 Runtime 与 Projection 的 1 秒空闲退避上限，原 env 备份保持不变。内容提交 `72d2e28` 为 Runtime、Projection、凭据侧入站和领域侧入站增加 PostgreSQL `LISTEN/NOTIFY` 唤醒；四个服务各自使用独立监听连接，触发器只在可认领状态事务提交后发送空 payload。通知连接建立或等待失败时，循环只等待当前退避区间的剩余时间并继续扫描，所以队列表与轮询仍承担可靠性，通知只改善延迟。开发服务器已回读四个触发器、四条监听连接和四条监听启动日志；重复 migration 返回空版本集。
