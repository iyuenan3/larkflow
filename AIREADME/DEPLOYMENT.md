...
- 当前 Target 应用 wheel 对应内容提交 `e6106e5f218f9c520928bef0293899ace7a2395f`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260809_2203_edge_policy_e6106e5/larkflow-0.0.2-py3-none-any.whl`，目录权限为 `0750 root:lf_target_dev`，文件权限为 `0640 root:lf_target_dev`，大小为 992995 bytes，SHA-256 为 `e163cb8a118c26f74d2543b2102564a841e356caa64692a3e4cbf1a489932b26`。升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260809T220248+0800.dump` 为 348510 bytes、权限 `0600 lf_target_dev:lf_target_dev`，SHA-256 为 `dc556788996afe7f3f9faf650ccea15a9ae2dce9375c5b6cbd767370b6f898a5`，且 `pg_restore --list` 可读取。migration runner 返回 `versions=[]`，ledger 保持 `23 / 0001_workflow / 0023_console_draft_requests`；服务器 `pip check` 无断裂依赖。统一重启后十个 Python 服务与 Caddy 均为 `active / NRestarts=0`；公网与 loopback Console 为 200，未认证 Console API 与 Edge claim 为 401，5432、8765 和 8780 继续只监听 loopback，部署窗口 warning 为 0。
