...
- 当前开发发布件对应内容提交 `f6125331aa541e824675e25f9cd2d756cd4c6b56`，包含 `0dc5359e990635c7b6aa16ec0bcd798eb8df39d0` 的具体退回意见与目标 Attempt 返工上下文，并修正原生 Card 2.0 表单提交不携带 `action_value` 时的服务端绑定。wheel SHA-256 为 `f7c909a4844fa69ef7c5387d20f0fa8fa3e43863c807392c3b26d37cb9e45c61`，位于 `/srv/larkflow/target/releases/20260806_145900_decision_form_f612533/`。完整离线套件为 `910 passed, 18 skipped`，新增契约复用既有 JSONB，不增加 migration。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260806T150649+0800.dump` 为 207307 bytes、`0600 lf_target_dev:lf_target_dev`，`pg_restore --list` 成功读取 134 个 TOC 条目。候选已同时安装到 Target 与 legacy 虚拟环境；migration runner 返回无待应用版本，长期库保持 `19 / 0019_draft_generation_progress`。
...
- 当前开发发布件对应内容提交 `f6125331aa541e824675e25f9cd2d756cd4c6b56`，包含裸 `/larkflow draft` 的 Card 2.0 引导、有界重生成、独立 Draft Generation Worker、阶段 revision 栅栏、按消息 ID 的多阶段卡片更新、来源约束型材料复核、必填退回意见与目标 Attempt 返工上下文。长期库为十九份 migration，八个 Target 服务与一个 legacy 事件消费者均回读 `active / running / NRestarts=0`。
...
- 当前 Target wheel 为 `larkflow 0.0.2`，SHA-256 为 `0dcccb7f674135dde8b44ab08d437ba397b92397b8456ede8a064f66f1eb2af1`，保存在 `releases/20260805_233701_source_review_b7e589b/larkflow-0.0.2-py3-none-any.whl`，对应内容提交 `b7e589ba4af0398573ec995254dd61e9b1a4508c`。决定卡回调需要 legacy 事件桥接进入耐久命令队列，因此同一候选已同时替换 `/srv/larkflow/target/venv` 与 `/srv/larkflow/dev/venv`。八个 Target 服务与一个 legacy 服务均回读 `active / running / NRestarts=0`。Gateway 与 PostgreSQL 继续只监听 loopback，Caddy 保持 `disabled / inactive`。
