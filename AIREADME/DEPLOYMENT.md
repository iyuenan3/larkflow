...
- 当前开发发布件对应内容提交 `fd6933a186bf115fe83adc5ac7d3a3b6153b0436`，新增员工设备前台 `larkflow-edge serve`，并保留内容提交 `5312f6c026453ac6d9e2e62679b755f271c114f3` 的双 Interactive 副本拓扑。完整离线套件为 `807 passed, 17 skipped`；长期库保持十八份 migration，八服务与六条真实监听连接已回读。`serve` 已在员工 Mac 上用同一候选 wheel 的临时安装态完成真实 Codex、应用心跳、续租、单设备锁、SIGTERM 停止和设备撤销验收。该员工端安装与凭据均为一次性验收资产，不是正式分发。
...
- 当前 Target wheel SHA-256 为 `e4c0a60588969202ffacf57b660f39e7811d2cdb564016756584ddc0ecc2ea13`，保存在 `releases/20260805_003605_edge_serve_fd6933a/candidate/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `fd6933a186bf115fe83adc5ac7d3a3b6153b0436`。同一发布目录的 `rollback/` 保存前一版 `5312f6c` wheel。升级前即时 PostgreSQL 备份 `larkflow_target_dev-20260805T003635+0800.dump` 为 139942 bytes、权限 `0600`；migration runner 返回空版本集，长期库保持十八份 migration。安装使用现有 root 管理的虚拟环境并通过 `pip check`；八个 Python 服务均回读 `active / running / NRestarts=0`，部署窗口 warning 级日志为 0。Gateway 与 PostgreSQL 继续只监听 `127.0.0.1`，Caddy 保持 `disabled / inactive`，备份 timer 保持 `enabled / active`。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
