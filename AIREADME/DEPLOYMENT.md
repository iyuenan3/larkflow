...
- 当前 Target 应用 wheel 对应内容提交 `432fea77c210e7a2cfa5344054eb30d01706bf87`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`。公网 `/console/` 返回 200，未认证草稿 API 返回 401。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260809_0357_console_drafts_432fea7/larkflow-0.0.2-py3-none-any.whl`，权限为 `0640 root:lf_target_dev`，大小为 427657 bytes，SHA-256 为 `6b320b22804c02eaa2840d9a101bcf1b4ffe75287509816486727588ccdc0198`。服务器安装的 `console_drafts.py`、`0023_console_draft_requests.sql`、`index.html`、`app.js` 与 `styles.css` SHA-256 均已和源码逐项回读一致；服务器 `pip check` 无断裂依赖。
- 本轮升级前 custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260809T035600+0800.dump` 为 285292 bytes，SHA-256 为 `ffb091224db79ab6bab92b9f42ad0b3af8963c1380f940b5fad52329777b1b61`，并通过 `pg_restore --list`。migration runner 应用 `0023_console_draft_requests`，ledger 为 `23 / 0001_workflow / 0023_console_draft_requests`。真实 PostgreSQL 双连接竞争只得到一个 `generating / attempt 1` claim，测试记录精确清理一条。本次重启 Console 与 Draft Generation Worker；十个 Python 服务与 Caddy 均为 `active / NRestarts=0`，公网静态资源哈希与源码一致，安全响应头齐全，部署窗口 warning 为 0。
