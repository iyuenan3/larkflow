...
- 当前 Target 应用 wheel 对应内容提交 `d879a280d49e584d2d7e5927a498e7947544bb63`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260809_1929_generated_decision_d879a28/larkflow-0.0.2-py3-none-any.whl`，权限为 `0640 root:lf_target_dev`，大小为 990765 bytes，SHA-256 为 `95149af078a67cc36f87ae832d1f9764cd40f3b7378fe6a408cd90eae59074c7`。升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260809T192947+0800.dump` 为 315393 bytes，SHA-256 为 `220237fd448e10d5d7a86251ab2bcc837ca2ebbb980eefff7ef85cf7bd7a23de`，且 `pg_restore --list` 可读取。migration runner 返回 `versions=[]`，ledger 保持 `23 / 0023_console_draft_requests`；服务器 `pip check` 无断裂依赖。统一重启后十个 Python 服务与 Caddy 均为 `active / NRestarts=0`；公网与 loopback Console 为 200，未认证 API 为 401，5432、8765 和 8780 继续只监听 loopback，部署窗口 warning 为 0。上一版受控画板发布件继续保留为可恢复回滚点。
