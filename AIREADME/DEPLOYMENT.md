...
- 当前 Target 应用 wheel 对应内容提交 `ba708724b5095f9185aa894ec381151f4305b91d`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读健康；Runtime、Projection、Draft Generator、Console、Edge、两条 Inbound 和两个 Interactive 实例均为 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260810_0535_p0_source_cards_ba70872/larkflow-0.0.2-py3-none-any.whl`，文件权限为 `0640 root:lf_target_dev`，大小为 1014356 bytes，SHA-256 为 `48abee97574fba4e157c7f15782c3c78fa2e5055fb1309219a3ec3c9f5cec90e`。升级前 custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260810T053541+0800.dump` 为 126043 bytes，权限为 `0600 lf_target_dev:lf_target_dev`，SHA-256 为 `f25b6b8b2433fc8fef167a721157f055b20cc3bf1a97b2c67edc9b94b9307b`，且 `pg_restore --list` 可读取。migration runner 保持 `23 / 0001_workflow / 0023_console_draft_requests`，`pip check` 无断裂依赖；公网 Console 200，未认证 API 401，安装态回读确认只接受供应商结构化引用、强制来源提示和决定卡收口逻辑均已加载，部署窗口 warning 为 0。
- Phase 2A 附件规划已纳入内容提交 `b2a13ff1eff796723774d42ca5d04556814a38c2` 并推送，但尚未应用 migration、完成真实 PostgreSQL 与 Caddy 验证或部署。已提交配置要求 Console 与 Draft Generation Worker 使用同一个显式绝对 `LARKFLOW_TARGET_ATTACHMENT_ROOT`，并同时把 `LARKFLOW_TARGET_ATTACHMENT_MODEL_EGRESS` 设为 `allow`；任一条件缺失时，Console 能力标志为 false，服务端在写入 collecting 请求前拒绝 defer，原有无附件生成继续可用。已提交 Caddy 模板只对精确附件上传 POST 放宽为 262144 字节，其他路径继续限制为 65536 字节。部署前必须在一次性 PostgreSQL 应用 `0024`，运行元数据 resolve、撤销、promotion、并发配额与幂等合同，使用 `caddy validate` 检查模板，并回读安装 wheel、loopback 与公网的能力标志和两种 body limit；这些检查目前都不能标记为已通过。
...
- 联网研究开关：Target Runtime 设置 `LARKFLOW_TARGET_ENABLE_WEB_SEARCH_EXECUTOR=true`，Draft Generation Worker 设置 `LARKFLOW_TARGET_DRAFT_ENABLE_WEB_SEARCH=true`，两者必须成对启用。`web.search` 共用服务端 LLM 路由，通过供应商 Responses API 执行公开网络研究；启用前必须确认模型支持、账号已开通及计费边界。`LARKFLOW_TARGET_WEB_SEARCH_MAX_PROMPT_CHARS / MAX_RESULT_CHARS` 分别限制研究输入和正文。只开草稿侧会生成无法调度的节点，只开 Runtime 侧不会让生成器使用联网能力。
