...
- 当前 Target 应用 wheel 对应内容提交 `482c280cf9007951fb117b835086a4b19eb1f932`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260809_164706_task_desc_482c280/larkflow-0.0.2-py3-none-any.whl`，权限为 `0640 root:lf_target_dev`，大小为 428067 bytes，SHA-256 为 `9d1cbf7cd1a0880632cdabb2dc31a757e29230d63bf89afc24ccfa3e5e2f08af`。服务器 `pip check` 无断裂依赖。升级前 custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260809T164816+0800.dump` 为 305530 bytes，权限为 `0600 lf_target_dev:lf_target_dev`，SHA-256 为 `93165424dc05e8c38d4bcf25184c19515ac5cef726be28cd6c21396b6bfd8b85`。本次无 schema 变更，十个 Python 服务统一重启后均为 `active / NRestarts=0`，部署窗口 warning 为 0。
