...
- 当前 Target 开发发布件对应内容提交 `c3e23fcbf3bf9e66eeb9cf97bf8bbbc1bb2eefc3`。前一内容提交 `b153c5311771eaa5b98d964fe6ffd448b62cf49d` 为 Owner Console 增加空白区域拖动平移、50% 到 160% 缩放、100% 重置、适配、键盘操作和点击后视口保持；当前提交把节点卡片排除在平移手势启动范围之外，避免细微指针位移吞掉节点点击。最终 wheel SHA-256 为 `5b1009c95fa493b1f583ea9ee63ee4a61190840b700efb151802f283e7b67dec`，位于 `/srv/larkflow/target/releases/20260806_194700_console_nodeclick_c3e23fc/`。完整离线套件为 `922 passed, 18 skipped`，Console 聚焦套件为 `10 passed`，JavaScript 语法检查通过；服务器安装资源、wheel 与本地源码的 `app.js` SHA-256 均为 `399cb70a72d2291ede7532cc6421f0d31a95d8c018b8b57a8d6309b67f0cb482`。legacy 事件消费者不需要本次只读页面代码，继续使用原虚拟环境。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260806T193753+0800.dump` 为 222118 bytes、`0600 lf_target_dev:lf_target_dev`，备份服务回读 `Result=success / ExecMainStatus=0`。两次候选安装的 migration runner 都返回 `versions=[]`，长期库保持 `19 / 0019_draft_generation_progress`，本次不增加 schema。
- 本次只重启 `larkflow-target-console.service`。Runtime、Draft Generation Worker、Projection、两个 Interactive、凭据侧入站、领域侧入站、Edge、Console 和 legacy 消费者共十个 Python 服务均回读 `active / NRestarts=0`；部署窗口 Console warning 为 0。5432、8765 与 8780 都只监听 `127.0.0.1`。Caddy 因 ICP 边界继续保持 `disabled / inactive`。
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，静态 token 为 64 字符且不进入命令行或日志。未认证 API 返回 401；认证后的真实页面加载 Owner 实例。真实 Chrome 在 `source_grounded_reject_20260806_001940` 上先以空白区域拖动把阶段 02 到 04 移入视口，再用鼠标选中 Tool 节点并立即切换右侧 Attempt；视口没有被节点点击重置。按钮验收覆盖 100% 到 90% 缩小和 57% 适配。既有标签页不会自动替换已加载的旧 JavaScript，部署后验收必须显式刷新。此前 SSH 隧道浏览器对 Owner 列表、运行中、草稿、Attempt、审计、跨 Owner 404 和锁定的验收仍有效；服务保持 loopback 常驻。Console 仍是只读观察面，文字回复发生在当前 Agent 对话，不在流程图中输入。
...
- 当前 Target 开发发布件对应内容提交 `c3e23fcbf3bf9e66eeb9cf97bf8bbbc1bb2eefc3`，包含裸 `/larkflow draft` 的 Card 2.0 引导、有界重生成、独立 Draft Generation Worker、阶段 revision 栅栏、按消息 ID 的多阶段卡片更新、来源约束型材料复核、必填退回意见、目标 Attempt 返工上下文，以及按真实依赖分层并可拖动缩放的 Owner 只读中央控制台。长期库为十九份 migration，九个 Target 服务与一个 legacy 事件消费者均回读 `active / NRestarts=0`。
