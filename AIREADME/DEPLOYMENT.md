...
- 当前 Target 应用 wheel 对应 `23437d15499df9182beeb823e0a1d7780fc69f5f`，包含 Phase 2A 附件、Agent 完成性、旅游 evidence policy、搜索能力预检、Runtime 结果规范化、公开错误说明、飞书安全富文本投影、no-web 直接来源依赖、正向证据复验、业务字段绑定和日期赋值边界。发布件位于 `/srv/larkflow/target/releases/date_boundary_20260815_225530_23437d1/larkflow-0.0.2-py3-none-any.whl`，大小为 1048882 bytes，SHA-256 为 `99075d335e64cf9d43851e447d8b4b7a6cd27e5d61ba7e08ba25d677ab73b6ce`。Target 与 legacy 安装态的 `draft_validation.py` SHA-256 都是 `7394bcca832dd4fb4e3a84a504c9d5206b0d7038da97dbab2a593f61e7ef6e7d`，两个 venv 的 `pip check` 通过。十个 larkflow Python 服务均为 `active / running / NRestarts=0`，部署窗口 journald warning 及以上为 0。
...
- Console 与 Draft Generation Worker 使用同一个持久绝对附件目录，且当前部署只对这两个服务启用 `LARKFLOW_TARGET_ATTACHMENT_MODEL_EGRESS=allow`；systemd `ProtectSystem=strict` 通过精确 `ReadWritePaths` 只放行该目录。Caddy 2.11.4 候选经过真实 validate/adapt，live 配置 SHA-256 为 `748175bb5f85551fdae805f1f78b7c8d2e42e76ef00ffea5c5d510e334f70989`。公网实测普通 70000 字节 POST 返回 413，精确附件 70000 字节 POST 穿透到应用授权层返回 401，270000 字节附件 POST 返回 413；loopback 与公网 Console 为 200，受保护实例 API 为 401，附件能力标志为 true，公网 JavaScript 与 CSS 哈希和安装态一致。Owner 作用域 HTTPS API 已完成 collecting、上传、显式生成、启动、Human 提交、Agent 执行和最终决定闭环；真实 Owner 浏览器附件交互仍待 Maxwell 手工验收。本条只证明开发环境，不代表生产就绪。Phase 2B 部署后，Target Runtime 也必须读取同一附件目录，并显式设置相同外发策略与 `LARKFLOW_TARGET_AGENT_CONTEXT_MAX_CHARS`；在完成部署回读前，不得声称 Agent Attempt 已在开发环境直接读取附件。
...
- 需要联网资料的负向草稿请求 `c79be5e550b70e9d6a40fe2b3cabcb17` 在 2.4 秒内进入 rejected，没有创建 Instance。Runtime 与 Draft Generator 的运行开关虽保持启用，当前已部署 provider/model 路线均明确配置为 unavailable，所以 PlanningService 在模型生成前返回可操作阻断，不再让 `web.search` 节点运行约四分钟后以无引用失败。豆包 Custom `SearchProvider` 已在内容提交 `807f51a71417669f8fd52852b99a1ae7eafa1ff7` 落码，但尚未部署或执行真实 API 请求，不能用离线 stub 证据覆盖这条开发环境现状。
...
- Agent 附件上下文：Phase 2B 要求 Runtime 与 Console、Draft Generation Worker 指向同一持久绝对 `LARKFLOW_TARGET_ATTACHMENT_ROOT`，并显式设置 `LARKFLOW_TARGET_ATTACHMENT_MODEL_EGRESS=allow` 与正整数 `LARKFLOW_TARGET_AGENT_CONTEXT_MAX_CHARS`。Runtime 服务只需读取附件目录，不应获得 Console 的上传写权限；systemd 沙箱应使用精确 `ReadOnlyPaths` 或等价只读授权。配置缺失时，声明了 `instance_inputs.project_attachments` 的 Agent Attempt 必须以 `agent_context_unavailable` 失败，禁止静默跳过附件继续生成。当前代码已提交，但本条尚未在开发服务器安装态验证。
