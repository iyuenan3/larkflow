...
> 当前开发状态：企业共享正文与 ContextBundle 已部署到阿里云开发环境。内容由 `88fd70ea5a29bdf7c9f84573fd45a951f73da88c` 和 `d76a65faca027a558bfadc9b395e5807247705e3` 实现，竞态修复 `17aba5ae35261bc915d2dd6d6f4c272273d2da89` 已部署。完整离线合并证据为 `1255 passed, 31 skipped`，一次性真实 PostgreSQL 为 `31 passed`，安装态 synthetic 竞态探针已回读。真实管理员浏览器验收尚未完成，本状态不代表生产就绪。
...
- 企业资料上下文配置：Console、Draft Generation Worker 与 Target Runtime 必须共享同一持久绝对 `LARKFLOW_TARGET_ENTERPRISE_KNOWLEDGE_ROOT`，并显式设置 `LARKFLOW_TARGET_ENTERPRISE_KNOWLEDGE_MODEL_EGRESS=allow` 才能发布或读取正文。缺失、deny、撤销、版本漂移、proof 不匹配、哈希或大小异常、预算超限和临时 Blob 故障都不能静默忽略资料继续运行。正文与重新授权能力已部署；显式 Console source 选择和 `0027_console_enterprise_knowledge_selection` 已提交但尚未部署。部署时必须确认 Console、Draft Generation Worker 与 Runtime 对同一目录的最小权限，并回读 realpath、owner、mode 与非 symlink 边界。
...
- 来源健康边界：来源质量切片已提交但尚未部署。当前没有启用的生产 `SafeOutboundFetcher`，部署后 `source_health.available` 必须回读为 false，所有来源 health 必须为 unknown，不能把既有豆包调用成功误写成来源网页健康。该默认端口不发起 URL 请求；未来启用真实 adapter 前必须单独完成 localhost、私网、链路本地、凭据 URL、DNS 漂移、重定向、响应上限和无登录态验证。`source_evidence.check` 不依赖网络，只验证当前已提交搜索 Attempt 的规范 URL 与 provider 原文片段。
