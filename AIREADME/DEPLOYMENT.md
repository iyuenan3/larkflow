...
- Owner Console 当前对应内容提交 `a6f5babb07623590e9be2a2b8c523857cce56ff7`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS，Console 继续只监听 `127.0.0.1:8780`；两项服务均为 `active / NRestarts=0`，公网与 loopback `/console/` 均返回 200。飞书 OAuth 应用凭证已由官方令牌端点验证，受限 env 保持 `0640 root:lf_target_dev`。
- 飞书应用已发布网页应用与机器人两种能力，网页应用作为工作台默认入口。至少两名真实成员已完成网页授权登录、本人 Owner 数据读取与跨 Owner 隔离验证，机器人命令入口继续可用。完成登录后的会话摘要已保存到 PostgreSQL；真实会话在 Console 重启后仍有效，用户直接刷新保持登录。该证据关闭开发环境单进程会话缺口，不等于生产发布；入口仍使用公网 IP，也没有管理员级集中撤销、正式域名或跨区域容灾。
- 当前 Target 开发发布件对应内容提交 `a6f5babb07623590e9be2a2b8c523857cce56ff7`，在既有生命周期、来源约束复核、Owner Console 与待处理中心上增加 PostgreSQL 耐久会话。wheel SHA-256 为 `a3b680c0a76545ab25a6c62ad500c9a2db0e24b2aac890eb4a1b708bc5fea729`，位于 `/srv/larkflow/target/releases/20260807_195154_console_sessions_a6f5bab/`。完整等价离线结果为 `955 passed, 19 skipped`；新增一次性真实 PostgreSQL 验收覆盖认证器重建、原始凭据不落库、注销和过期清理。Target 虚拟环境已替换为该 wheel；legacy 事件消费者未因本次 Console 会话变化重装。
...
- 当前 Target 开发发布件对应内容提交 `a6f5babb07623590e9be2a2b8c523857cce56ff7`，在既有自然语言草稿、来源约束复核、返工上下文、生命周期、Owner 只读 Console 与待处理中心基础上增加 PostgreSQL 耐久会话。长期库为二十份 migration；本次只重启 Console，Console 与 Caddy 均回读 `active / running / NRestarts=0`。真实 PostgreSQL 会话摘要、认证器重建、注销、过期清理和浏览器登录态跨 Console 重启已经通过；既有待处理中心查询、生命周期竞争与真实飞书命令证据继续有效。
