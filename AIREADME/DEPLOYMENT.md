...
- Owner Console 当前对应内容提交 `66b2c12d3ea27a61e5a1cdc21332ed03adb516ac`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS，覆盖 `X-Larkflow-Client-IP` 并限制请求体、请求头与连接时间；Console 继续只监听 `127.0.0.1:8780`。两项服务均为 `active / NRestarts=0`，公网与 loopback `/console/` 均返回 200，未认证管理员 API 返回 401。飞书 OAuth 应用凭证已由官方令牌端点验证，受限 env 保持 `0640 root:lf_target_dev`，管理员 allowlist 只保存于该受限 env。
...
- 当前 Target 开发发布件对应内容提交 `66b2c12d3ea27a61e5a1cdc21332ed03adb516ac`，在既有生命周期、来源约束复核、Owner Console、待处理中心、PostgreSQL 耐久会话、管理员聚合和受审计的其他会话撤销上增加可信代理来源、有界令牌桶与完整浏览器安全响应头。wheel SHA-256 为 `3ff1d97317bf4c72e4040622e747bc16d7ca98709ecf2525371f894b9fa1b9df`，位于 `/srv/larkflow/target/releases/20260808_004500_console_public_66b2c12/`。完整离线结果为 `972 passed, 21 skipped`。真实公网并发验收让 31 个请求携带不同伪造来源值，仍只得到同一预算下的 30 次 200 和 1 次 429，429 携带 `Retry-After: 2`；安全响应头、Caddy 运行时超时、原有一条真实登录会话和部署窗口零 warning 均已回读。Target 虚拟环境已替换为该 wheel；legacy 事件消费者未因本次 Console 边界变化重启。
...
- 本次公网边界发布只重启 `larkflow-target-console.service` 并 reload Caddy。Runtime、Draft Generation Worker、Projection、两个 Interactive、凭据侧入站、领域侧入站、Edge、Console 和 legacy 消费者共十个 Python 服务，以及 Caddy，均回读 `active / running / NRestarts=0`。5432、8765 与 8780 只监听 `127.0.0.1`，Caddy 管理口只监听 `127.0.0.1:2019`。Edge 公网链路仍保持关闭，不能把 Console 入口外推为 Personal Agent Edge 已具备公网 E2E。
...
- 当前 Target 开发发布件对应内容提交 `66b2c12d3ea27a61e5a1cdc21332ed03adb516ac`，在既有自然语言草稿、来源约束复核、返工上下文、生命周期、Owner Console、待处理中心、PostgreSQL 耐久会话、管理员聚合和受审计会话撤销基础上增加公网请求边界。长期库仍为二十一份 migration，本次没有 schema 变更。切换前创建 `/etc/larkflow-target-console.env.bak.20260808_004500_console_public_66b2c12`、`/etc/caddy/Caddyfile.bak.20260808_004500_console_public_66b2c12` 和 systemd unit 备份；上一版 wheel `/srv/larkflow/target/releases/20260807_212230_session_gov_8ba0ab9/larkflow-0.0.2-py3-none-any.whl` 保留为回滚点。本次只重启 Console 并 reload Caddy，十个 Python 服务与 Caddy 均回读 `active / running / NRestarts=0`。公网、loopback、安全响应头、Caddy 运行时配置、伪造来源限流和 429 已真实验收，原有真实登录会话仍为一条。既有待处理中心查询、会话治理、生命周期竞争与真实飞书命令证据继续有效。
