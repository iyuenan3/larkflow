...
- 当前开发发布件对应内容提交 `2ed644e640f3c3834f82c464e05fe0b4c3a241cc`，包含裸 `/larkflow draft` 的 Card 2.0 引导、有界重生成、独立 Draft Generation Worker、阶段 revision 栅栏和按消息 ID 的多阶段卡片更新。长期库为十九份 migration，八个 Target 服务与一个 legacy 事件消费者均回读 `active / running / NRestarts=0`。
...
- 当前 Target wheel 为 `larkflow 0.0.2`，SHA-256 为 `56787f1f3e8298a831c80dc65d60e3a116d34f0c1aef9324c50448e4d15ee4b7`，保存在 `releases/20260805_214200_cardsettle_2ed644e/larkflow-0.0.2-py3-none-any.whl`，对应内容提交 `2ed644e640f3c3834f82c464e05fe0b4c3a241cc`。本轮只修改 Target Interactive 与消息 adapter 路径，不改变 legacy 回调动作或事件归一化，因此只替换 `/srv/larkflow/target/venv`；`larkflow@dev` 继续使用上一份已验证 wheel。八个 Target 服务与一个 legacy 服务均回读 `active / running / NRestarts=0`。Gateway 与 PostgreSQL 继续只监听 loopback，Caddy 保持 `disabled / inactive`。
