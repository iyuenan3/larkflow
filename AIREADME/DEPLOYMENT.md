...
- 当前 Target wheel 为 `larkflow 0.0.2`，SHA-256 为 `e8b82659cb03a42892480164ef0541ed512b4dde4dbf259b0892e32d02e8d78e`，保存在 `releases/20260805_191842_draftretry_282ea51/candidate/larkflow-0.0.2-py3-none-any.whl`，对应内容提交 `282ea515aeb463896133b4b3a60d9d42733d555c`。同一发布目录的 `rollback/` 保存上一版 `6ff0af2` wheel，回滚件 SHA-256 为 `b822f349fe1411490689067751b96eb8b619bc6d3020e328fb05ee5296e852ce`。升级前即时 PostgreSQL 备份 `larkflow_target_dev-20260805T191903+0800.dump` 为 150907 bytes；migration runner 返回空版本集，长期库保持 `18|0001_workflow|0018_worker_wakeups`。同一候选 wheel 已安装到 `/srv/larkflow/target/venv` 与 `/srv/larkflow/dev/venv`，两处 `draft_generation.py`、`role_bindings.py` 源码哈希与本地提交一致。前者运行 Target 八服务中的七个，后者供 `larkflow@dev` 长连接事件桥接使用；新增飞书动作若只升级 Target 虚拟环境，旧桥接会静默忽略回调。八个 Python 服务均回读 `active / running / NRestarts=0`，部署窗口 warning 级日志为 0。Gateway 与 PostgreSQL 继续只监听 `127.0.0.1`，Caddy 保持 `disabled / inactive`。`/etc/larkflow-target.env` 保持权限收紧，路由真值不进入仓库或日志。
