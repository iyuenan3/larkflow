...
- 当前开发发布件对应内容提交 `5113a59aacc8b0a97481411e581b9d52f6462073`，新增 `/larkflow draft` 结构化无模板草稿入口，并保留内容提交 `5312f6c026453ac6d9e2e62679b755f271c114f3` 的双 Interactive 副本拓扑与既有 Edge Gateway。完整离线套件为 `858 passed, 17 skipped`；长期库保持十八份 migration，八服务全部回读 `active / running / NRestarts=0`，部署窗口 warning 级日志为 0。
...
- 当前 Target wheel 为 `larkflow 0.0.2`，SHA-256 为 `d5b0964ce3bcb817a6ec22c3346bb4ff47aaae64bffdd0b0cd67027ee3dd4d2a`，保存在 `releases/20260805_162906_inline_5113a59/candidate/larkflow-0.0.2-py3-none-any.whl`，对应内容提交 `5113a59aacc8b0a97481411e581b9d52f6462073`。同一发布目录的 `rollback/` 保存上一版 `fd6933a` wheel，回滚件 SHA-256 为 `e4c0a60588969202ffacf57b660f39e7811d2cdb564016756584ddc0ecc2ea13`。升级前即时 PostgreSQL 备份 `larkflow_target_dev-20260805T163019+0800.dump` 为 143443 bytes；备份服务回读 `Result=success / ExecMainStatus=0`。migration runner 返回空版本集，长期库保持 `18|0001_workflow|0018_worker_wakeups`。安装使用现有 root 管理的虚拟环境并通过 `pip check`；已安装源码回读包含 `/larkflow draft` 与严格 JSON decoder。八个 Python 服务均回读 `active / running / NRestarts=0`，部署窗口 warning 级日志为 0。Gateway 与 PostgreSQL 继续只监听 `127.0.0.1`，Caddy 保持 `disabled / inactive`。`/etc/larkflow-target.env` 保持权限收紧，路由真值不进入仓库或日志。
...
- 开发交互延迟配置保留 Runtime、Projection 与两个 Interactive 副本的 1 秒空闲退避上限。内容提交 `72d2e28` 最初为 Runtime、Projection、凭据侧入站和领域侧入站增加 PostgreSQL `LISTEN/NOTIFY` 唤醒；内容提交 `5312f6c` 又让两个 Interactive 副本各持有独立监听连接，当前共六条。触发器只在可认领状态事务提交后发送空 payload。通知连接建立或等待失败时，循环只等待当前退避区间的剩余时间并继续扫描，所以队列表与轮询仍承担可靠性，通知只改善延迟。开发服务器已回读四个触发器、六条监听连接和六条监听启动日志；重复 migration 返回空版本集。
...
- 完成投影最初缺失的根因是一次部署只重启了 Runtime、Projection 和 legacy，遗漏两个 Target 入站服务；旧领域入站进程在代码升级前启动，因此只提交完成审计，没有生成新版本的完成投影 outbox。`deploy/restart-development-services` 现统一重启 Runtime、Projection、两个 Interactive、凭据侧入站、领域侧入站、Edge 和 legacy 共八个服务，并逐个回读启动时间与 `NRestarts`。`reconcile-instance-completion` 只修复一个已知完成实例，不批量补发历史通知。
...
- `larkflow-target.service`、`larkflow-target-projection.service`、两个 `larkflow-target-interactive@.service` 实例、`larkflow-target-inbound-adapter.service`、`larkflow-target-inbound.service`、`larkflow-target-edge.service` 与 legacy `larkflow@dev` 均 enabled / active，统一重启后 `NRestarts=0`。legacy 消费 Task 与 IM EventKey，只把原始 Task 提示或 `/larkflow` 命令写入 Target Inbox，不写 Target 领域表；Task 状态轮询是 Human 完成的可靠入口，Task 事件只是可选低延迟信号。
