...
- 当前 Target 应用 wheel 对应内容提交 `6d2d9fa22b7e6926cfe5a1bcf714ccccd073b6d3` 与装配修复 `c7e7d901237a3b72d3d265382e1e2239a0626abd`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读健康；本轮重点回读的 Runtime、Draft Generator、Projection 与两个 Interactive 实例均为 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260810_0440_p0_hotfix_c7e7d90/larkflow-0.0.2-py3-none-any.whl`，文件权限为 `0640 root:lf_target_dev`，大小为 1013741 bytes，SHA-256 为 `3b738c85376380a1b1040bb06a9fadf748058971c1779af934c5b59ec1c9d605`。升级前 custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260810T043650+0800.dump` 为 98019 bytes，SHA-256 为 `b6df588e795eef2671604492ba08e102327d612e57211e35ba6f2cc7fb95348c`，且 `pg_restore --list` 可读取。migration runner 保持 `23 / 0001_workflow / 0023_console_draft_requests`，`pip check` 无断裂依赖；公网 Console 200，未认证 API 401。wheel 中的交付物模块和 `WebSearchToolExecutor` 已从安装态回读。
