...
- **事件入口**：引擎 spawn `lark-cli event consume <EventKey>` 子进程收 NDJSON，出站长连接。原设计「无需任何入站端口」（ADR-007）；但前端（ADR-019）要引擎暴露读 / 命令 API，会引入一个入站端口 / 域名 / 证书，此点待妙搭原型后定（引擎前置一层，或前端后端代理）。
