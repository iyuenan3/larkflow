...
- 当前 Target 应用 wheel 对应内容提交 `1063b29eece4b0dea05467982e5b581af3e79ff2`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260810_020108_first_use_1063b29/larkflow-0.0.2-py3-none-any.whl`，目录权限为 `0750 root:lf_target_dev`，文件权限为 `0640 root:lf_target_dev`，大小为 998688 bytes，SHA-256 为 `80d3f53c0d9e666cd35eda6250ea1273c52825c48f40707528566d3b6b1f1efc`。升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260810T020226+0800.dump` 为 349074 bytes、权限 `0600 lf_target_dev:lf_target_dev`，SHA-256 为 `95b56e7c76b98dfd25f8d8f70a3c70c03d4cdfb5c8e4b56a4aef9e9bde68499e`，且 `pg_restore --list` 可读取。migration runner 返回 `versions=[]`，ledger 保持 `23 / 0001_workflow / 0023_console_draft_requests`；Target 与 legacy 的 `pip check` 无断裂依赖。统一重启后十个 Python 服务与 Caddy 均为 `active / NRestarts=0`；公网与 loopback Console 为 200，未认证实例 API 为 401，5432、8765 和 8780 继续只监听 loopback，安装态与公网 `app.js / index.html / styles.css` 哈希一致，安全响应头保持有效，部署窗口 warning 为 0。浏览器控制在打开公网真实登录标签页时超时，本轮没有创建或推进流程，真实可见验收仍需后续单独关闭。
