...
- 当前 Target 应用 wheel 对应内容提交 `86189216a0b67cf258daa4027d368257eee7491a`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260810_0233_workflow_mainline_8618921/larkflow-0.0.2-py3-none-any.whl`，目录权限为 `0750 root:lf_target_dev`，文件权限为 `0640 root:lf_target_dev`，大小为 1001173 bytes，SHA-256 为 `5851a5bc7d296d0a40e74552aeae631175c50953b59adb7f82c3431063ec802f`。升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260810T023250+0800.dump` 为 349074 bytes、权限 `0600 lf_target_dev:lf_target_dev`，SHA-256 为 `fc97e29d7c0fc1df26d530d22efe52df7a12a2bfea82d63873ac6eaa45e9e54c`，且 `pg_restore --list` 可读取。migration runner 返回 `versions=[]`，ledger 保持 `23 / 0001_workflow / 0023_console_draft_requests`；Target 与 legacy 的 `pip check` 无断裂依赖。统一重启后十个 Python 服务与 Caddy 均为 `active / NRestarts=0`；公网与 loopback Console 为 200，未认证实例 API 为 401，5432、8765 和 8780 继续只监听 loopback，安装态与公网 `app.js / index.html / styles.css` 哈希一致，安全响应头保持有效，部署窗口 warning 为 0。两个精确临时上传文件已经删除并回读不存在。真实飞书 OAuth 连续路径仍需单独关闭，不能由部署回读代替。
