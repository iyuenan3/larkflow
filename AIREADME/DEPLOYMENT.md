...
- 当前 Target 应用 wheel 对应 `0ff4272a10abe85d2afab36f65a2edd3e4d50a41`，包含 Phase 2A 附件、Agent 完成性、旅游 evidence policy、搜索能力预检、Runtime 结果规范化、公开错误说明和飞书安全富文本投影。发布件位于 `/srv/larkflow/target/releases/search_guidance_20260815_0ff4272/larkflow-0.0.2-py3-none-any.whl`，大小为 1046839 bytes，SHA-256 为 `e36f36ccf73f7366c59cc6d5c671aeec4b1f45f6001e7c161be3f9d6f3ef0e76`，安装态 AgentRuntime 与 PlannerRuntime 文件哈希和本地提交一致，`pip check` 无断裂依赖。十个 larkflow Python 服务均为 `active / running / NRestarts=0`，部署窗口 warning 为 0。
...
- 部署版本的截断探针使用纯内存、无网络、无数据库和无飞书副作用，得到 `Attempt=failed / error_code=agent_result_incomplete / instance=failed / result_persisted=false`。最终完整离线套件为 `1145 passed, 27 skipped`；唯一因沙箱禁止读取进程树的既有测试在允许环境单独 `1 passed`，合并证据为 `1146 passed, 27 skipped`。
