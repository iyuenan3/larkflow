...
- 当前 Target 应用 wheel 对应内容提交 `3fd42df8740825482eb3bbebd5cf69715f37df5b`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / running / NRestarts=0`。公网与 loopback `/console/` 均返回 200，未认证任务 API 返回 401。飞书 OAuth 应用凭证与管理员 allowlist 继续只保存在受限 env。
- 当前发布件为 `/srv/larkflow/target/releases/20260809_0259_transfer_sync_3fd42df/larkflow-0.0.2-py3-none-any.whl`，权限为 `0640 root:lf_target_dev`，大小为 414709 bytes，SHA-256 为 `79ac572f4feb160db835d8a26b25d77b84e57916542367c347f0df0b65426ee1`。服务器安装的 `app.js`、`index.html` 与 `console_tasks.py` SHA-256 分别为 `458c5e8e5b77fb1591c24cb87d5441d37e2ecdf89f9c69525684d7ce20ee5cec`、`712b94976c35bb468718b7456960da3c762636f79c2ea13d7148e3b9dbfaf477` 与 `076cdd0ba58a260a317010bcbc5440ab681be5911cc2e5b4433cf0661e70ee66`，和源码及 wheel 一致；服务器 `pip check` 无断裂依赖。
- 本轮升级前 custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260809T030013+0800.dump` 为 285292 bytes，SHA-256 为 `f3593021322de5b87aace25b61b8b3edf711dbb42c9726efae054f2d7438357d`，权限为 `0600`，归档目录可正常列举。migration runner 返回空集，ledger 保持 `22 / 0001_workflow / 0022_outbox_exhaustion`。本次只重启 Console；十个 Python 服务与 Caddy 均为 `active / NRestarts=0`，公网静态资源哈希与源码一致，安全响应头齐全，部署窗口 warning 为 0。
...
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，鉴权模式为 `feishu`。App secret 与 OAuth token 不进入命令行、浏览器或日志；静态 token 只保留为 loopback 开发回退。未认证业务 API 返回 401。Owner 完整实例视图继续按 tenant 与 Instance Owner 隔离；普通 Human Task 参与者只能读取任务有界上下文并提交或转交，决定节点继续使用飞书卡片。既有标签页不会自动替换已加载的旧 JavaScript，部署后验收必须显式刷新。自然任务的页面提交与飞书 Task 负责人转交已经通过，下一项产品工作是受控流程输入框。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的二十二份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
