...
- Owner Console 当前对应内容提交 `e15f47942fcc01bc85ecbbfa822acd00558c06f0`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS，Console 继续只监听 `127.0.0.1:8780`；两项服务均为 `active / NRestarts=0`，公网与 loopback `/console/` 均返回 200。飞书 OAuth 应用凭证已由官方令牌端点验证，受限 env 保持 `0640 root:lf_target_dev`，管理员 allowlist 只保存于该受限 env。
- 飞书应用已发布网页应用与机器人两种能力，网页应用作为工作台默认入口。至少两名真实成员已完成网页授权登录、本人 Owner 数据读取与跨 Owner 隔离验证，机器人命令入口继续可用。完成登录后的会话摘要已保存到 PostgreSQL；真实会话在 Console 重启后仍有效，用户直接刷新保持登录。服务端 allowlist 命中的成员可读取当前企业管理员聚合，普通成员访问同一路由返回 404。该证据关闭开发环境单进程会话和最小管理员观察面缺口，不等于生产发布；入口仍使用公网 IP，也没有管理员级集中撤销、allowlist 管理、正式域名或跨区域容灾。
- 当前 Target 开发发布件对应内容提交 `e15f47942fcc01bc85ecbbfa822acd00558c06f0`，在既有生命周期、来源约束复核、Owner Console、待处理中心和 PostgreSQL 耐久会话上增加管理员只读概览。wheel SHA-256 为 `fbdd2e325d57fb595362c4aac8c32b10ae734843014c4bbef2da71480bbe418b`，位于 `/srv/larkflow/target/releases/20260807_204031_admin_e15f479/`。完整离线结果为 `960 passed, 20 skipped`；真实 PostgreSQL 与 HTTP 验收覆盖七条队列、55 个流程、二十份 migration、管理员 200、普通成员 404 和聚合响应隐私。Target 虚拟环境已替换为该 wheel；legacy 事件消费者未因本次 Console 管理概览变化重装。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260807T195221+0800.dump` 为 227704 bytes，备份服务回读 `Result=success / ExecMainStatus=0`。migration runner 应用 `0020_console_sessions`，ledger 回读 `20 / 0001_workflow / 0020_console_sessions`；会话表所有者为 `lf_target_dev`，主键、过期和主体索引均存在。
- 待处理中心发布时只重启 `larkflow-target-console.service`。Runtime、Draft Generation Worker、Projection、两个 Interactive、凭据侧入站、领域侧入站、Edge、Console 和 legacy 消费者共十个 Python 服务均回读 `active / running / NRestarts=0`。5432、8765 与 8780 都只监听 `127.0.0.1`。后续为员工工作台 OAuth 单独启用 Caddy 的公网 IP 80 / 443 入口；Edge 公网链路仍保持关闭，不能把 Console 入口外推为 Personal Agent Edge 已具备公网 E2E。
...
## Target PostgreSQL 开发验证状态（2026-08-07）

- 当前 Target 开发发布件对应内容提交 `e15f47942fcc01bc85ecbbfa822acd00558c06f0`，在既有自然语言草稿、来源约束复核、返工上下文、生命周期、Owner Console、待处理中心与 PostgreSQL 耐久会话基础上增加管理员只读概览。长期库为二十份 migration；升级前数据库备份服务返回 `Result=success / ExecMainStatus=0`，env 备份为 `/etc/larkflow-target-console.env.bak.20260807_204031_admin_e15f479`。本次只重启 Console，十个 Python 服务与 Caddy 均回读 `active / running / NRestarts=0`。真实 HTTP 验收回读 `read_only=true / scope=current_tenant / admin=200 / non_admin=404 / queue_lanes=7 / workflow_total=55 / migrations_up_to_date=true`；两条短期验收会话已撤销，原有真实登录会话仍为一条。真实登录浏览器中的管理员页签视觉验收仍待完成；既有待处理中心查询、生命周期竞争与真实飞书命令证据继续有效。
...
- 长期开发库已应用 `0001_workflow` 到 `0020_console_sessions`。第十九份 migration 增加自然语言草稿的独立生成租约、阶段进度 revision 与最终回复栅栏；第二十份保存 Console 会话凭据 SHA-256 摘要、服务端主体与有效期，并为过期清理和主体查询建立索引。前十八份继续覆盖领域状态、投影、Inbox、模板、Edge、IM 命令、重启、编辑、人员分工、恢复卡、canonical 动作、首反馈指标和事务后通知。通知不携带业务状态，凭据侧默认验证上限仍为 24 次，结构化日志中的非零 `exhausted` 必须告警。
...
- `larkflow-target-backup.timer` 每天北京时间 03:20 后随机延迟不超过 15 分钟执行 custom-format `pg_dump`，本机保留约 7 天。备份目录权限为 `0700 lf_target_dev:lf_target_dev`，备份文件为 `0600`。本次升级前备份 `larkflow_target_dev-20260805T214140+0800.dump` 已回读 `Result=success / ExecMainStatus=0`，大小 155532 bytes，所有者为 `lf_target_dev:lf_target_dev`。十九份 migration 已进入备份范围；新版异库恢复演练仍待重跑。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的二十份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
