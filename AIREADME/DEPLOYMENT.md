...
- Phase 2A 附件规划仍只存在于未提交工作树，尚未应用 migration 或部署。候选配置要求 Console 与 Draft Generation Worker 使用同一个显式绝对 `LARKFLOW_TARGET_ATTACHMENT_ROOT`，并同时把 `LARKFLOW_TARGET_ATTACHMENT_MODEL_EGRESS` 设为 `allow`；任一条件缺失时，Console 能力标志为 false，服务端在写入 collecting 请求前拒绝 defer，原有无附件生成继续可用。候选 Caddy 模板只对精确附件上传 POST 放宽为 262144 字节，其他路径继续限制为 65536 字节。部署前必须在一次性 PostgreSQL 应用 `0024`，运行元数据 resolve、撤销、promotion、并发配额与幂等合同，使用 `caddy validate` 检查模板，并回读安装 wheel、loopback 与公网的能力标志和两种 body limit；这些检查目前都不能标记为已通过。
