...
- 节点重启候选件另在一次性 PostgreSQL 14 数据库应用十份 migration，并用两个真实连接同时确认同一 RestartPreview。结果恰好一路执行、一路幂等回放，aggregate version 只增加 1，Attempt 从 1 增至 2，旧结果保留且重启审计只有 1 条；跨 tenant 读取预览被拒绝。测试库、安装目录和上传件随后删除并回读为 0。
...
- 长期开发库已应用 `0001_workflow` 到 `0010_restart_previews`。第六份 migration 允许 Inbox 进入不可再认领的 `exhausted` 终态；第七份增加一次性配对、可撤销设备和追加型 Edge 审计；第八份增加耐久 IM 命令、发送者验证和回复状态；第九份增加 Owner 最近实例列表所需的复合索引；第十份增加短期 RestartPreview、消费版本和未消费预览部分索引。凭据侧默认验证上限为 24 次，结构化日志中的非零 `exhausted` 必须告警。
...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、七个 IM 窄命令、节点安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前发布 wheel SHA-256 为 `093c20fb9d1936f3060fbc3153e8805cee278acaef88eaefbdf2eac40740f358`，保存在 `releases/20260803_162724_node_restart/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `b319494`；五个 Target 服务和 legacy 服务已统一重启并回读 active、`NRestarts=0`，部署与真实验收窗口无错误级日志。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
...
- `larkflow-target-backup.timer` 每天北京时间 03:20 后随机延迟不超过 15 分钟执行 custom-format `pg_dump`，本机保留约 7 天。备份目录权限为 `0700 lf_target_dev:lf_target_dev`，备份文件为 `0600`，backup service 使用 systemd 文件系统与权限沙箱。本次升级前的北京时间 16:27 备份已回读 `Result=success / ExecMainStatus=0` 且文件非空。最近一次恢复演练在当时只有两份 migration 时完成，已回读表所有权和收紧的 schema 权限并删除恢复库。十份 migration 已进入备份范围，但新版恢复演练尚未重跑。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的十份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
...
**已在真栈实测通过（测试组织）**：2026-07-26 的交互消息与卡片回调；2026-08-02 的 Target Human Task 创建、完成和详情读取；2026-08-03 的 `im.message.receive_v1` 窄命令、当前企业活跃成员验证、Owner 专属状态查询、文本回执、Agent / Tool 结果消息、完成 Docx 与最终通知。文档权限在新增 `docx:document + docx:document:create` 并发布后生效。六个 Python 服务统一重启后均为 active 且 `NRestarts=0`。Task 完成轮询已完成真实闭环，Task 事件路径本轮仍为零事件，不能标记为通过。权限台账记录已知运行所需能力，不等于当前控制台完整权限清单；上线前仍需从已发布版本重新导出并做最小权限回归。
