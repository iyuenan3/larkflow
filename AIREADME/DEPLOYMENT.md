...
- 2026-08-20 Agent 运行时恢复已部署。最终内容提交为 `b43a8cb230bfa123dfcc0638de4589c4708da2f5`，wheel SHA-256 为 `a87f154666aaa631002d6ef2e0f4d7816d90168aa285e54d792834ed37dde5ce`，保存在 `/srv/larkflow/target/releases/b43a8cb_tokens_20260820_221201/`。Target Agent 路线为 `deepseek-v4-flash`，thinking 为 disabled，显式 completion 上限为 16384 tokens，正文仍受 12000 字符持久化合同约束。安装态 `llm/client.py`、`workflow/config.py`、`workflow/cli.py` 和 `workflow/executors.py` 哈希与本地一致；`pip check` 通过，migration 重入为空，长期库保持 `29 / 0029_console_draft_cancellation`。
- 同一部署窗口精确停止并重启全部十个共享 wheel 服务，启动时间为北京时间 22:15:23 至 22:15:31，最终均为 `active / running / NRestarts=0`，严重异常日志匹配为 0。正式 restart 创建的 Agent Attempt 7 使用实际 provider model `deepseek-v4-flash-ga-260731`，首轮与格式 repair 均在 16384 token 配置下正常 stop，Agent 以 5768 字符正文完成，下游 Human Attempt 7 为 `waiting_human`。旧 Attempt 2 至 6、重启审计和失败原因均保留；没有直接改写数据库状态，也没有修改 Caddy 或 schema。Human Gate 仍需真实参与者接受或退回。

- 2026-08-20 有界旅行 Planner 已部署。内容提交为 `90b14189cc43c67e919c480f1e4fa8f70cc934b5` 与 `a5f39ded62a7ae880ac0a554f2f71a287e821715`，最终 wheel SHA-256 为 `dccce10ec4f0922dec51829943af919405faa0b6e1811f04a0785e5609d7e46c`，保存在 `/srv/larkflow/target/releases/travel_planner_20260820_194100_a5f39de/`。Target venv 的 `pip check` 通过，`planning/travel.py`、`draft_validation.py` 和 `workflow/cli.py` 安装态哈希与本地一致。长期库 ledger 保持 `29 / 0029_console_draft_cancellation`，本轮没有 schema 或 Caddy 变更；只重启 Draft Generation Worker，最终十个 Python 服务与 Caddy 均为 `active / running / NRestarts=0`，部署窗口 warning/error 为 0，Console loopback 与公网静态资源均为 200 且 app.js 哈希一致。
- 同一版本的 synthetic 请求 `1b42e59f4d634f39b773900e24949e37` 冻结一份企业资料，在 0.58 秒内形成 `Human / Tool / Tool / Agent / Human` 五节点 ready 草稿，两个 Tool 均为独立 `web.search`，Agent 显式消费企业资料。独立豆包公开查询在 0.59 秒返回 10 条带 URL 来源。取消探针保持 `canceled / definition null / instance null`，迟到 claim 写回被拒绝；缺预算请求返回 `missing_required_fields / 预算` 且未泄漏原始异常。草稿没有确认启动，本轮没有飞书 Task、文档或消息写入。DeepSeek v4 Flash 与 qwen3.7-plus 对同一复杂冻结输入的 120 秒诊断均失败，故没有把模型 fallback 误记为通过；真实飞书浏览器端到端仍待独立验收。

> 当前开发状态：企业共享正文、ContextBundle 与 Console 显式资料选择均已部署到阿里云开发环境。内容由 `88fd70ea5a29bdf7c9f84573fd45a951f73da88c`、`d76a65faca027a558bfadc9b395e5807247705e3` 和 `e663edfa2c54230b3b58bfc9e26ad046f95a3b51` 实现，竞态修复 `17aba5ae35261bc915d2dd6d6f4c272273d2da89` 已部署。搜索来源质量 `fba57583af164d5a39077d7979b751e604cc3382` 也已部署；完整离线合并证据为 `1280 passed, 32 skipped`，一次性真实 PostgreSQL 为 `32 passed`，安装态 synthetic 选择、撤销和搜索证据探针已回读。真实管理员浏览器验收尚未完成，本状态不代表生产就绪。
...
- 2026-08-20 飞书 Console 重新授权恢复已部署。内容提交为 `bc416252747963ba955568c8f5ff09aa33b13e65`，wheel SHA-256 为 `c583d1e729944086dffbd0c3922b8c5bfbfdb55768b120f86bc9890e1ba56182`，保存在 `/srv/larkflow/target/releases/console_login_recovery_20260820_165646_7d94879/`。Target venv 的 `pip check` 通过；本地、wheel、安装态、loopback 与公网 `app.js` SHA-256 均为 `4511fa84e050708874484f46b7990d8f78cfff4458faea9ee8947c0bdad2b7ff`。本轮只重启 Console，服务为 `active / running / NRestarts=0`，页面为 200，未登录业务 API 为 401，部署窗口 warning 为 0；OAuth 登录入口仍以安全 state cookie 302 到 `accounts.feishu.cn`。没有运行 migration、修改 Caddy 或写入飞书与业务数据。真实浏览器重新授权点击仍待人工验收。

- 2026-08-19 provenance 与撤销选择恢复已部署。内容提交为 `cceed8d3093c28e3687f748fe872b6fd5f59f2ee`，文档同步基线为 `b6c52e3becb332825f63dd5816f8aad256dbc73a`；wheel SHA-256 为 `75eac62b39295e255560c9c5be46d4c64203fb858eeca15d73f4d347b5a8289c`，保存在 `/srv/larkflow/target/releases/provenance_recovery_20260819_141913_b6c52e3/`。一次性空库完整应用 28 份 migration 并重入；重复 ID 被数据库约束拒绝，撤销墓碑清除后选择版本为 2，并发更新只有一路成功。故障库中的重复历史行使 0028 整体回滚，ledger 保持 `27 / 0027` 且数据未被自动修改；两座一次性库均精确删除并回读不存在。长期库增量应用并重入后为 `28 / 0028_console_enterprise_source_uniqueness`。Target 与 legacy 的 `pip check`、五个关键安装态源码哈希和在线 app.js 哈希均通过；十个服务为 `active / running / NRestarts=0`，部署窗口 warning 为 0，Console loopback 为 200，未认证资料目录为 401，Caddy 保持 active 且未修改。安装态 synthetic 探针确认普通 Agent 伪造 provenance 被拒绝、真实 `web.search` 通过、撤销墓碑只含安全字段并可由 Owner 显式清除。本轮没有飞书写入，真实浏览器人工验收仍未完成。

- 2026-08-19 显式企业资料选择与搜索证据质量已部署。内容提交为 `e663edfa2c54230b3b58bfc9e26ad046f95a3b51` 与 `fba57583af164d5a39077d7979b751e604cc3382`，wheel SHA-256 为 `3615e455d5a7750981897cb00ada7e4ec7c41823c963d3315a15087fbc731800`，保存在 `/srv/larkflow/target/releases/explicit_knowledge_search_20260819_044726_591476c/`。一次性真实 PostgreSQL 从空库运行 27 份 migration 与重入，合同为 `32 passed`；强制失败回滚保持 `26 / 0026` 且没有 0027 字段，两个测试库随后精确删除并回读不存在。长期库增量应用并重入后为 `27 / 0027_console_enterprise_knowledge_selection`。Target 与 legacy 的 `pip check` 和关键安装态源码哈希均通过；十个 larkflow Python 服务与 Caddy 为 `active / running / NRestarts=0`，部署窗口 warning 为 0。Caddy live 配置未修改，SHA-256 保持 `fc24674c12ebe132c4fc10206f24ac71f6c510d32c0ba69c8cf6a6cf40527b19` 且再次通过 validate。安装态 synthetic 探针确认选择冻结、重试版本稳定、撤销门禁、Planner 与 Agent Context、来源质量、claim 片段绑定和安全错误分类；`SafeOutboundFetcher` 仍为 unavailable，没有访问任意来源 URL。本轮没有飞书写入，真实浏览器显式选择和人工产品验收仍未完成。
>
> 除修正事实错误外，不再给这套部署增加新的产品领域能力。个人端不得复用下文的企业 bot 全局凭证；中央端和个人端必须使用不同身份、权限与生命周期。

## 当前发布状态 · 2026-08-19

- 当前 Target 与 legacy 已安装包含企业知识撤销竞态修复的 wheel，SHA-256 为 `b2aa7941e393111d011c2b5351b7df1bd052f85aef5d6cb71b682fd18b911d95`。两个 venv 的 `pip check` 通过，安装态 `knowledge_context.py` 和 `knowledge/repository.py` 的 SHA-256 与内容提交 `17aba5ae35261bc915d2dd6d6f4c272273d2da89` 一致。长期库 migration 重入没有新增版本，ledger 保持 `26 / 0026_enterprise_knowledge_content_authorization`。一次性真实 PostgreSQL 从完整 26 份 migration 启动，包含撤销先于最终授权与最终授权先于撤销两种受控屏障的合同为 `31 passed`，测试库已精确删除并回读不存在。
- Caddy 2.11.4 候选通过 validate 与 adapt，企业资料发布和项目附件两条精确路由均使用 262144 字节限制，通用请求保持 65536 字节，并位于通用路由之前；三个 upstream 均覆盖客户端 IP 头，admin 保持 loopback，access log 未启用。live 配置 SHA-256 为 `fc24674c12ebe132c4fc10206f24ac71f6c510d32c0ba69c8cf6a6cf40527b19`。公网 synthetic 请求回读为通用 70 KB 得到 413、企业资料发布 70 KB 穿过代理后在应用授权层得到 401、企业资料发布 270 KB 得到 413。十个 larkflow Python 服务和 Caddy 均为 `active / running / NRestarts=0`，部署窗口 warning 为 0。
- 开发环境使用持久、非 symlink 的企业资料目录，并只为 Console、Draft Generation Worker 与 Target Runtime 配置相同正文根和显式 allow 外发策略。安装态 synthetic/public 探针在 Blob `get` 中触发撤销，planning 与 Agent 两条路径均回读 `rejected=true / bundle_returned=false / status=revoked`。十个 larkflow Python 服务均为 `active / running / NRestarts=0`，部署窗口 warning 为 0；Console loopback 返回 200，Caddy 配置 SHA 未变化。本轮未创建或修改飞书资源，真实管理员浏览器发布与真实内部资料验收仍待手工完成。
- 本轮直接应用回滚点为上一版完整企业正文 wheel `/srv/larkflow/target/releases/phase2_enterprise_knowledge_20260819_023650_588899f/larkflow-0.0.2-py3-none-any.whl`，SHA-256 为 `16e1833fe05579661bfec72ccbd2fcfeee6fa8b86f7fe48eeb80fa3e60be00c4`。更早的 metadata-only wheel 和 release 继续保留。本轮没有创建数据库备份或变更 Caddy；0026 为已存在的追加型事务 migration，旧 wheel 与旧 release 继续作为应用回滚点。
- 用户明确选择不备份旧测试数据，开发数据库 `larkflow_target_dev` 曾按同名、同 owner 重建为空库。企业目录部署前另建 custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-before-0025-20260819_003122.dump`，大小 153317 bytes，SHA-256 为 `e712511ab51d7383b01248f8db8a4c86797d4d25f6d482672942242d084ee7a2`，`pg_restore --list` 可读。长期库已增量应用 `0025_enterprise_knowledge_catalog` 并重入，当前 ledger 为 `25 / 0025`。一次性真实 PostgreSQL 从空库运行 25 份 migration 并重入，`tests/test_workflow_postgres.py` 为 `28 passed`，测试库与临时目录已精确删除并回读不存在。
- 企业目录仓储 synthetic 探针保留 tenant `synthetic_knowledge_catalog_20260819_003122` 的 2 个不可变版本、1 个当前 published 版本和 3 条追加审计。服务器管理员 API synthetic 探针另保留 tenant `synthetic_knowledge_api_20260819_005038` 的 1 个 revoked 版本和 2 条追加审计；发布、列表、审计、撤销、重复撤销、非管理员 404 与安全 DTO 均经开发服务回读。该记录只验证开发环境的 metadata-only 目录管理，不代表真实企业正文、来源权限证明、ContextBundle 合并或知识读取已经完成。
- 企业资料目录 API 聚焦离线套件为 `153 passed`；完整离线套件在沙箱中为 `1223 passed, 28 skipped, 1 failed`，唯一失败是 macOS 进程树权限，同一测试在允许环境单独 `1 passed`，合并证据为 `1224 passed, 28 skipped`。一次性 PostgreSQL 从空库运行 25 份 migration 并重入，`tests/test_workflow_postgres.py` 为 `28 passed`，数据库回读 3 个版本和 4 条审计后精确删除测试库。长期库未增加 migration，仍为 `25 / 0025`。
- Console、Draft Generation Worker 与 Target Runtime 使用同一个持久绝对附件目录，并显式启用相同模型外发策略；Runtime 的 `LARKFLOW_TARGET_AGENT_CONTEXT_MAX_CHARS` 为 12000。systemd `ProtectSystem=strict` 继续通过精确 `ReadWritePaths` 只放行该目录。Caddy 2.11.4 live 配置未改变，SHA-256 仍为 `748175bb5f85551fdae805f1f78b7c8d2e42e76ef00ffea5c5d510e334f70989`。loopback 与公网 Console 为 200，公网受保护实例 API 为 401，JavaScript 哈希与本地源码一致。真实 Owner 浏览器附件交互仍待 Maxwell 手工验收，本条只证明开发环境，不代表生产就绪。
...
- 字段绑定反例 `NO_WEB_FIELD_COUNTEREXAMPLE_20260815_195116_9A70F03` 对应请求 `ae384b2064761d15781e319703a21aae`，正文同时包含“出行日期待定”和资料更新时间、“出行人数未知”和酒店限住 2 人、“旅行总预算未确认”和酒店预算 12000 元。开发 Worker 第一次领取后即以 `DraftGenerationRejected` 进入 rejected，定义为空且没有 Instance，错误只列出缺失的出行日期、出行人数和预算；同一代码路径的回归测试证明 Planner 调用数为 0。合法标记 `NO_WEB_FIELD_VALID_20260815_195116_9A70F03` 生成实例 `console_draft_93099995e3a27f0722fd2db6d49a7610`，最终为 `done / version 7 / graph_revision 1`，三个节点均为 Attempt 1。数据库逐对象比较确认 Human 结果与 Agent `input_snapshot.dependencies.confirm_source_and_requirements` 完全相等，两侧均为 2691 bytes。飞书 Task `393dca72-4583-4c25-9ac4-46ce93514f1a` 由 bot 原生接口回读为完成；完成文档 `OzGCdthQkoI3NLxQmu3ccaLunud` 为 revision 4，包含验收标记、1 个 title、33 个 heading、21 个 list 和 1 个原生 table。决定卡已收口为 `settled=true / accepted`，完成通知和四类节点、实例投影均已落库。旧实例和外部历史均未清理或覆盖。
- 日期边界反例 `DATE_BOUNDARY_COUNTEREXAMPLE_20260815_2302_23437D1` 对应请求 `880ef7341d0cd73bb6adf4dfa76bf9ea`。真实 Console 依次返回创建 202、上传 201、开始生成 202，Draft Worker 随后以 `DraftGenerationRejected` 进入 rejected；PostgreSQL 回读定义为空、Instance 为空，错误只列出缺失的出行日期。部署版探针确认“出行日期资料更新时间”和“出行日期说明”均为 false，中文合法日期、起止日期和英文范围均为 true。临时 Console 会话和远端 `/tmp` 制品已撤销或删除；本轮没有新建合法飞书 E2E，也没有修改或清理旧实例和飞书历史。
- 需要联网资料的历史负向草稿请求 `c79be5e550b70e9d6a40fe2b3cabcb17` 曾在 2.4 秒内因能力不可用进入 rejected，没有创建 Instance。豆包 Custom `SearchProvider` 内容提交 `807f51a71417669f8fd52852b99a1ae7eafa1ff7` 现已部署，Target 与 Draft Generation Worker 的 capability preflight 都回读为 configured。一次 synthetic/public 真实查询返回 10 条带合法 URL 的规范化来源、usage 和 provider request ID，错误为空。该证据证明当前开发配置可调用并保留来源，不证明来源权威性或业务事实正确性。
- 原生 Docx 交付物通过 bot 身份创建文档 `SxmedQ4R9oIcUdxAejEc23m8npr`，同一幂等键和覆盖更新都保持同一 document_id。原生接口回读 revision 5、1 个标题、1 个列表、1 个表格和 2 条版本历史，最终正文包含 synthetic marker。当前未设置规范或兼容文件夹 token，Target completion 与 legacy deliverable 因此使用同一 bot 个人空间根；旧 Markdown handle 仍沿兼容命令读写。本条保留该测试文档，不清理外部历史。
- 部署版本的确定性探针确认：来源 Human 根节点未进入 Agent 直接 `deps` 时候选图被拒绝；全字段否定资料会列出出发地、出行日期、出行人数、预算、景点资料和交通资料六项缺口；超过 `max_result_chars` 的真实 `WorkflowWorker` 路径得到 `Attempt=failed / error_code=agent_result_incomplete / instance=failed / result_persisted=false`。字段绑定聚焦及关联套件为 `154 passed`。本轮完整离线套件为 `1160 passed, 27 skipped, 1 failed`，唯一失败是沙箱禁止读取进程树；同一测试在允许环境单独 `1 passed`，合并证据为 `1161 passed, 27 skipped`。
...
- Agent 附件上下文：Phase 2B 已让 Runtime 与 Console、Draft Generation Worker 指向同一持久绝对 `LARKFLOW_TARGET_ATTACHMENT_ROOT`，并显式设置 `LARKFLOW_TARGET_ATTACHMENT_MODEL_EGRESS=allow` 与 `LARKFLOW_TARGET_AGENT_CONTEXT_MAX_CHARS=12000`。配置缺失时，声明了 `instance_inputs.project_attachments` 的 Agent Attempt 必须以 `agent_context_unavailable` 失败，禁止静默跳过附件继续生成。安装态 synthetic Runtime 探针已确认正文只进入当前 ContextBundle，安全运行证据不含正文、object key 或 claim。后续生产装配仍应把 Runtime 的文件系统权限收紧为只读授权。
- 企业资料上下文配置：Console、Draft Generation Worker 与 Target Runtime 必须共享同一持久绝对 `LARKFLOW_TARGET_ENTERPRISE_KNOWLEDGE_ROOT`，并显式设置 `LARKFLOW_TARGET_ENTERPRISE_KNOWLEDGE_MODEL_EGRESS=allow` 才能发布或读取正文。缺失、deny、撤销、版本漂移、proof 不匹配、哈希或大小异常、预算超限和临时 Blob 故障都不能静默忽略资料继续运行。正文、重新授权、显式 Console source 选择和 `0027_console_enterprise_knowledge_selection` 均已部署。后续变更仍必须确认 Console、Draft Generation Worker 与 Runtime 对同一目录的最小权限，并回读 realpath、owner、mode 与非 symlink 边界。
...
- 联网研究开关：Target Runtime 的 `LARKFLOW_TARGET_ENABLE_WEB_SEARCH_EXECUTOR` 与 Draft Generation Worker 的 `LARKFLOW_TARGET_DRAFT_ENABLE_WEB_SEARCH` 仍需成对启用，但两个布尔值不代表 provider 已可用。专用豆包路线读取同名 `LARKFLOW_DOUBAO_SEARCH_BOT_ID / API_ID / API_KEY`，其中 Bot ID 与 API ID 只作为控制台资源标签，直连请求只使用 API Key；任一服务发现资源标签存在但 API Key 缺失时 fail closed，不会静默回退。没有配置专用路线时，既有主线路和备用线路仍必须逐条声明 `LLM[_ROLE][_BACKUP_N]_WEB_SEARCH_CAPABILITY=responses_citations`，改变 base URL 或 model 后不会继承能力。当前开发环境两个服务的安装态 preflight 均为 configured，真实 synthetic/public 查询回读 10 条带 URL 来源、usage 和 provider request ID。该证据不替代额度耗尽、远端错误分类或来源权威性验收。`LARKFLOW_TARGET_WEB_SEARCH_MAX_PROMPT_CHARS / MAX_RESULT_CHARS` 分别限制研究输入和正文，豆包查询另有 100 字符硬上限，Runtime claim TTL 必须大于 30 秒搜索超时与安全余量之和。
- 来源健康边界：来源质量切片已部署。当前没有启用的生产 `SafeOutboundFetcher`，安装态 `source_health.available` 回读为 false，来源 health 保持 unknown，不能把既有豆包调用成功误写成来源网页健康。该默认端口不发起 URL 请求；未来启用真实 adapter 前必须单独完成 localhost、私网、链路本地、凭据 URL、DNS 漂移、重定向、响应上限和无登录态验证。`source_evidence.check` 不依赖网络，只验证当前已提交搜索 Attempt 的规范 URL 与 provider 原文片段。
