...
- 当前 Target 开发发布件对应内容提交 `623b9b6228caa52b4680eb30ad2fee723e8921b6`。该提交让 Owner Console 依据真实 `deps` 计算拓扑层级、绘制 SVG 依赖箭头和直接依赖标签，不再把节点数组误画成线性链。wheel SHA-256 为 `6b8faed6eb5a4f32d695e40fdc495480585e53d9058e28e7ca7d2ece32421f8d`，位于 `/srv/larkflow/target/releases/20260806_183749_console_dag_623b9b6/`。完整离线套件为 `922 passed, 18 skipped`，Console 与部署相关聚焦套件为 `22 passed`，JavaScript 语法检查通过；服务器安装资源、wheel 与本地源码的 `app.js` SHA-256 均为 `a17afe0badc483d009b2b6049eb94c77a82c2f3cb1cbccd8045e7d50a08c2cc1`。legacy 事件消费者不需要本次只读页面代码，继续使用原虚拟环境。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260806T183926+0800.dump` 为 222114 bytes、`0600 lf_target_dev:lf_target_dev`，备份服务回读 `Result=success / ExecMainStatus=0`。migration runner 返回 `versions=[]`，长期库保持 `19 / 0019_draft_generation_progress`，本次不增加 schema。
...
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，静态 token 为 64 字符且不进入命令行或日志。未认证 API 返回 401，认证后的真实详情返回 200；静态资源包含新拓扑分层实现且不再包含旧线性连接器。真实 Chrome 标签页经刷新加载当前静态资源后，页面回读 4 条依赖边和 4 条依赖标签；分叉、汇合、关联高亮和横向滚动均已目视通过。既有标签页不会自动替换已加载的旧 JavaScript，部署后验收必须显式刷新。此前 SSH 隧道浏览器对 Owner 列表、运行中、草稿、Attempt、审计、跨 Owner 404 和锁定的验收仍有效；服务保持 loopback 常驻。
...
- 当前 Target 开发发布件对应内容提交 `623b9b6228caa52b4680eb30ad2fee723e8921b6`，包含裸 `/larkflow draft` 的 Card 2.0 引导、有界重生成、独立 Draft Generation Worker、阶段 revision 栅栏、按消息 ID 的多阶段卡片更新、来源约束型材料复核、必填退回意见、目标 Attempt 返工上下文，以及按真实依赖分层的 Owner 只读中央控制台。长期库为十九份 migration，九个 Target 服务与一个 legacy 事件消费者均回读 `active / NRestarts=0`。
