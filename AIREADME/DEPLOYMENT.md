...
- Owner Console 代码现对应内容提交 `3fe8cd5`，systemd 出站边界对应 `bc961b6`。Caddy 已重新启用并通过公网 IP 的 80 / 443 端口提供 HTTPS，Console 继续只监听 `127.0.0.1:8780`；两项服务均为 `active / NRestarts=0`，公网 `/console/` 返回 200。飞书 OAuth 应用凭证已由官方令牌端点验证，受限 env 保持 `0640 root:lf_target_dev`。
- 飞书应用已发布网页应用与机器人两种能力，网页应用作为工作台默认入口。至少两名真实成员已完成网页授权登录、本人 Owner 数据读取与跨 Owner 隔离验证，机器人命令入口继续可用。该证据关闭开发环境身份与可见性门槛，不等于生产发布；入口仍使用公网 IP，Console 会话仍保存在单进程内存中，重启后需要重新登录。
- 当前 Target 开发发布件对应内容提交 `b6eda8caaa06d338de8c5aa0283c3d787a8affe7`，在既有生命周期、来源约束复核和 Owner Console 上增加 Owner 待处理中心与查询边界加固。wheel SHA-256 为 `14cdbcfc5f343dc16d4985f62752ef7ab302cb6f20e8e1410eae7f7420befa3c`，位于 `/srv/larkflow/target/releases/20260807_010810_attention_b6eda8c/`。完整等价离线结果为 `944 passed, 18 skipped`，Console 聚焦套件为 `16 passed`。安装态 `app.js`、`index.html` 与 `styles.css` 的 SHA-256 均与本地源码和 wheel 一致。Target 虚拟环境已替换为该 wheel；legacy 事件消费者未因本次只读 Console 变化重装。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260807T010925+0800.dump` 为 227702 bytes、`0600`，备份服务回读 `Result=success / ExecMainStatus=0`。migration runner 返回 `versions=[]`，ledger 回读 `19 / 0001_workflow / 0019_draft_generation_progress`，本次不增加 schema。
...
- 当前 Target 开发发布件对应内容提交 `b6eda8caaa06d338de8c5aa0283c3d787a8affe7`，在既有自然语言草稿、来源约束复核、返工上下文、生命周期和 Owner 只读 Console 基础上增加待处理中心。长期库为十九份 migration，九个 Target 服务与一个 legacy 事件消费者均回读 `active / running / NRestarts=0`；待处理中心真实 PostgreSQL 查询、认证 API 与隐私边界已经通过，生命周期 PostgreSQL 双连接竞争与真实飞书命令证据继续有效。
