...
- 当前开发发布件对应内容提交 `72d2e286c4a44b7893896939acf93aa97662db83`，包含 PostgreSQL Worker 通知唤醒、轮询兜底、所有可操作卡片的耐久后即时视觉反馈、人员选择卡单动作约束、首个服务端反馈耐久指标，以及既有失败恢复能力。完整离线套件为 `786 passed, 15 skipped`；长期库第十八份 migration、六服务部署、四条真实监听连接和通知版本人员选择卡的反馈耗时及终态读回均已验收。
...
- 长期开发库已应用 `0001_workflow` 到 `0018_worker_wakeups`。第六份 migration 允许 Inbox 进入不可再认领的 `exhausted` 终态；第七份增加一次性配对、可撤销设备和追加型 Edge 审计；第八份增加耐久 IM 命令、发送者验证和回复状态；第九份增加 Owner 最近实例列表所需的复合索引；第十份增加短期 RestartPreview、消费版本和未消费预览部分索引；第十一份为 RestartPreview 增加显式 scope、可空节点键和作用域约束；第十二份增加耐久 GraphEditPreview、候选 Snapshot 哈希、消费版本与未消费预览索引；第十三份保存认证 mention；第十四份保存人员选择卡候选快照、回调、处理和回复状态；第十五份为恢复卡片增加耐久更新 token；第十六份为同卡回调增加 canonical 标记与部分唯一索引；第十七份增加卡片反馈完整指标；第十八份为 Outbox、Inbox、IM 命令和人员分工动作的可认领状态安装事务后空通知触发器。通知不携带业务状态，凭据侧默认验证上限仍为 24 次，结构化日志中的非零 `exhausted` 必须告警。
...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、十个 IM 窄命令、人员选择卡、失败恢复卡、未来区域编辑、节点与完整实例安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询、PostgreSQL Worker 通知唤醒和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前代码发布 wheel SHA-256 为 `f2fe2072f98ef1ba371d97b18e4ef2d070fa1bfa21b657f9c12876d314aa89bb`，保存在 `releases/20260804_171500_worker_wakeup_72d2e28/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `72d2e286c4a44b7893896939acf93aa97662db83`；升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260804T171129+0800.dump` 已读回为成功、`0600`、`lf_target_dev` 所有且 129748 bytes，长期库已应用十八份 migration。代码发布后的六个 Python 服务均回读 `active / running / NRestarts=0`，部署窗口 warning 级日志数为 0。安装使用现有 root 管理的虚拟环境；Target 与 legacy 在同一停机窗口统一安装同一 wheel，migration 成功后才统一启动，不存在混合运行窗口。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
...
- 开发交互延迟配置仍保留 Runtime 与 Projection 的 1 秒空闲退避上限，原 env 备份保持不变。内容提交 `72d2e28` 进一步为 Runtime、Projection、凭据侧入站和领域侧入站增加 PostgreSQL `LISTEN/NOTIFY` 唤醒；四个服务各自使用独立监听连接，触发器只在可认领状态事务提交后发送空 payload。通知连接建立或等待失败时，循环只等待当前退避区间的剩余时间并继续扫描，所以队列表与轮询仍承担可靠性，通知只改善延迟。开发服务器已回读四个触发器、四条监听连接和四条监听启动日志；重复 migration 返回空版本集。
- 指标版本部署后，Owner 在测试组织分别提交新的 `collaborative_agent_review` 人员选择卡和失败恢复卡。人员选择动作进入 `processed / draft_created`，服务端首个反馈耗时 1.264 秒，领域处理与最终回复分别在入站后 4.484 秒和 5.030 秒完成；机器人身份读回原消息为 `interactive / updated`，内容包含“人员分工已确认”，不含“处理中”或提交控件。恢复动作进入 `processed / automated_retry_started`，服务端首个反馈耗时 0.990 秒，领域处理与最终回复分别在入站后 2.844 秒和 3.213 秒完成；机器人身份读回原消息为 `interactive / updated`，标题为“恢复操作已处理”，不含“处理中”或按钮。两条动作的 `feedback_status` 均为 `updated`，完成时间与耗时字段完整落库；该验收确认服务端反馈区间和最终卡片，不宣称客户端在同一毫秒完成渲染。
- 通知版本部署后，Owner 再次提交 `collaborative_agent_review` 人员选择卡。动作只产生一个 canonical 回调和一个草稿，进入 `processed / draft_created / reply sent`；服务端首个反馈为 0.868 秒，凭据验证、领域处理和最终回复分别在入站后 0.959 秒、1.912 秒和 2.200 秒完成。应用 bot 从飞书服务端回读原消息为 `interactive`，包含已确认状态且不含按钮或 `role_binding_submit` 动作。该单次结果只证明开发服务器与测试组织中的服务端区间，不包含客户端渲染，也不外推到生产容量。
...
- 完成投影最初缺失的根因是上一次部署只重启了 Runtime、Projection 和 legacy，遗漏两个 Target 入站服务；旧领域入站进程在代码升级前启动，因此只提交完成审计，没有生成新版本的完成投影 outbox。`deploy/restart-development-services` 现统一重启五个 Target 服务和 legacy，并逐个回读启动时间与 `NRestarts`。`reconcile-instance-completion` 只修复一个已知完成实例，不批量补发历史通知。
...
- `larkflow-target.service`、`larkflow-target-projection.service`、`larkflow-target-inbound-adapter.service`、`larkflow-target-inbound.service`、`larkflow-target-edge.service` 与 legacy `larkflow@dev` 均 enabled / active，统一重启后 `NRestarts=0`。legacy 消费 Task 与 IM EventKey，只把原始 Task 提示或 `/larkflow` 命令写入 Target Inbox，不写 Target 领域表；Task 状态轮询是 Human 完成的可靠入口，Task 事件只是可选低延迟信号。
...
- `larkflow-target-backup.timer` 每天北京时间 03:20 后随机延迟不超过 15 分钟执行 custom-format `pg_dump`，本机保留约 7 天。备份目录权限为 `0700 lf_target_dev:lf_target_dev`，备份文件为 `0600`，backup service 使用 systemd 文件系统与权限沙箱。本次升级前备份已回读 `Result=success / ExecMainStatus=0`，文件 `larkflow_target_dev-20260804T171129+0800.dump` 非空、129748 bytes 且为 `0600 lf_target_dev:lf_target_dev`。最近一次恢复演练在当时只有两份 migration 时完成，已回读表所有权和收紧的 schema 权限并删除恢复库。十八份 migration 已进入备份范围，但新版恢复演练尚未重跑。
...
- Target CLI：`/srv/larkflow/target/venv/bin/larkflow-target --env-file /etc/larkflow-target.env <command>`；模板控制面支持 template-create、template-add-version、template-enable、template-disable、template-delete、template-list、template-show、create-from-template 与 preview，并保留实例和四类 Worker 命令。Projection 身份使用 `/etc/larkflow-target-projection.env reconcile-projections` 显式执行启动对账，使用同一 env 的 `reconcile-completions` 立即扫描完成状态；`reconcile-instance-completion <instance_id>` 只补齐一个已完成实例缺失的文档或最终通知。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的十八份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
