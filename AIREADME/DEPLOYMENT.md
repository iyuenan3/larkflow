...
- 长期开发库已应用 `0001_workflow` 到 `0011_restart_scope`。第六份 migration 允许 Inbox 进入不可再认领的 `exhausted` 终态；第七份增加一次性配对、可撤销设备和追加型 Edge 审计；第八份增加耐久 IM 命令、发送者验证和回复状态；第九份增加 Owner 最近实例列表所需的复合索引；第十份增加短期 RestartPreview、消费版本和未消费预览部分索引；第十一份为 RestartPreview 增加显式 scope、可空节点键和作用域约束。凭据侧默认验证上限为 24 次，结构化日志中的非零 `exhausted` 必须告警。
...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、八个 IM 窄命令、节点与完整实例安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前发布 wheel SHA-256 为 `c1aa5b65eaba977e53175889d64332114a822b64c80349da4160abea01747751`，保存在 `releases/20260803_174919_full_restart/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `e66f6ab`；五个 Target 服务和 legacy 服务已统一重启并回读 active、`NRestarts=0`，部署与真实验收窗口无错误级日志。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
...
- `larkflow-target-backup.timer` 每天北京时间 03:20 后随机延迟不超过 15 分钟执行 custom-format `pg_dump`，本机保留约 7 天。备份目录权限为 `0700 lf_target_dev:lf_target_dev`，备份文件为 `0600`，backup service 使用 systemd 文件系统与权限沙箱。本次升级前的北京时间 17:49 备份已回读 `Result=success / ExecMainStatus=0`，文件 `larkflow_target_dev-20260803T174935+0800.dump` 非空且为 `0600 lf_target_dev:lf_target_dev`。最近一次恢复演练在当时只有两份 migration 时完成，已回读表所有权和收紧的 schema 权限并删除恢复库。十一份 migration 已进入备份范围，但新版恢复演练尚未重跑。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的十一份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
