...
- 当前 Target 应用 wheel 对应内容提交 `da94891f5e6d01ecee6082a98bab6148abba12ee`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS，覆盖 `X-Larkflow-Client-IP` 并限制请求体、请求头与连接时间；Console 继续只监听 `127.0.0.1:8780`。本次只重启 Console；九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / running`，全部为 `NRestarts=0`。公网与 loopback `/console/` 均返回 200，未认证读取与工作流 POST 返回 401。飞书 OAuth 应用凭证继续只保存在受限 env，管理员 allowlist 也只保存于该受限 env。
...
- 当前 Target 开发发布件从内容提交 `da94891f5e6d01ecee6082a98bab6148abba12ee` 构建。它在既有双主题工作台上增加 Owner 受控流程操作，不改变来源约束型决策契约或 Human 飞书责任入口。干净 wheel SHA-256 为 `fca2eee16d3af57dcfb4bb78409a0b6f9e23b7d3d29aa7d7435cc1f26dd3063a`，位于 `/srv/larkflow/target/releases/20260808_235309_console_actions_da94891/`，权限为 `0640 root:lf_target_dev`。完整离线结果为 `995 passed, 21 skipped`，服务器 `pip check` 无断裂依赖；源码、wheel、临时安装、服务器安装和公网下载后的 `console_actions.py`、HTML、JavaScript 与 CSS 哈希逐项一致。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260808T235220+0800.dump` 为 259098 bytes，SHA-256 为 `459ecac443e2eb6cad9034691fa07b21dec947b7944213ed6e88f5cdbca4db7e`，权限为 `0600 lf_target_dev:lf_target_dev`；备份服务回读 `Result=success / ExecMainStatus=0`。本次没有 schema 变更，migration runner 返回空集，ledger 仍为 `21 / 0001_workflow / 0021_console_session_governance`。
- 本次只重启 `larkflow-target-console.service`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / running / NRestarts=0`，部署窗口 warning 为 0。公网与 loopback `/console/` 返回 200，未认证读取与工作流 POST 返回 401；公网响应包含 HSTS、CSP、Permissions-Policy、COOP、CORP、拒绝 framing、`nosniff` 与 `no-referrer`。Edge 公网链路仍保持关闭，不能把 Console 入口外推为 Personal Agent Edge 已具备公网 E2E。
...
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，鉴权模式现为 `feishu`。App secret 与 OAuth token 不进入命令行、浏览器或日志；静态 token 只保留为 loopback 开发回退。未认证业务 API 返回 401；授权后的真实页面加载当前 Owner 实例，非 Owner 与不存在实例继续统一返回 404。确认草稿、暂停和继续直接调用中央领域服务；取消与重启先在同页展示影响预览，再显式确认。Human 正文与决定仍在飞书入口完成。早期 Chrome 在 `source_grounded_reject_20260806_001940` 回读“已完成 4/4、实例版本 16”、Agent、Tool 与最终 Human 三个 Attempt 2 节点，以及 00:53:27 由当前 Owner 发起且影响三节点的受控节点重启。`source_grounded_20260805_234517` 同时回读“未发现多轮执行节点”和“最近审计中未发现受控重启”。既有标签页不会自动替换已加载的旧 JavaScript，部署后验收必须显式刷新。当前版本的真实登录 Owner 已从公网工作台直接确认并启动 `internal_trial_20260808_155244`，最终三节点全部完成；其他受控操作仍需逐项页面验收。
...
- 当前 Target 开发发布件对应内容提交 `da94891f5e6d01ecee6082a98bab6148abba12ee`，在既有自然语言草稿、来源约束复核、返工上下文、生命周期、Owner Console、待处理中心、PostgreSQL 耐久会话、管理员聚合、受审计会话撤销和公网请求边界基础上增加 Owner 受控流程操作。长期库仍为二十一份 migration，本次没有 schema 变更。干净 wheel 位于 `/srv/larkflow/target/releases/20260808_235309_console_actions_da94891/`，SHA-256 为 `fca2eee16d3af57dcfb4bb78409a0b6f9e23b7d3d29aa7d7435cc1f26dd3063a`；上一版双主题工作台发布件继续保留为可恢复回滚点。本次只重启 Console，九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`，部署窗口 warning 为 0。真实登录 Owner 随后已直接确认并启动 `internal_trial_20260808_155244`；实例完成于 `version 7`，三个节点均为 Attempt 1，两个飞书 Task、Agent 结果消息、完成文档与最终通知均有外部绑定。该证据不覆盖暂停、继续、取消或重启的页面操作。
