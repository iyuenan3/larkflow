...
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，静态 token 为 64 字符且不进入命令行或日志。未认证 API 返回 401，认证后的真实详情返回 200；静态资源包含新拓扑分层实现且不再包含旧线性连接器。自动浏览器受回环地址安全策略限制，本次尚未完成人工目视确认新版图形。此前 SSH 隧道浏览器对 Owner 列表、运行中、草稿、Attempt、审计、跨 Owner 404 和锁定的验收仍有效；服务保持 loopback 常驻。
