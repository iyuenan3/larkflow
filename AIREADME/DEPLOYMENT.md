...
- 当前 Target 开发发布件对应内容提交 `770243a02b116e12583ceebdb8362fd40b7fe0a7`，增加 Owner 范围的暂停、继续和版本绑定取消，并在取消时收口普通 Human Task 与决定卡。wheel SHA-256 为 `04b76ac0b1cbe14c410c739be0279d74e60127b9cd3f68eeb4f8a07e0ba2b8af`，位于 `/srv/larkflow/target/releases/20260806_221621_lifecycle_770243a/`。完整离线套件为 `939 passed, 18 skipped`，生命周期聚焦套件为 `178 passed`；安装态 lifecycle 模块与本地源码 SHA-256 均为 `d2e7e81a44e06393768af08a293bdba579b23bab884bcee270e294d416680025`。Target 与 legacy 虚拟环境均已替换为该 wheel。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260806T221621+0800.dump` 为 222114 bytes、`0600 lf_target_dev:lf_target_dev`，备份服务回读 `Result=success / ExecMainStatus=0`。migration runner 返回 `versions=[]`，ledger 回读 `19 / 0001_workflow / 0019_draft_generation_progress`，本次不增加 schema。
- Runtime、Draft Generation Worker、Projection、两个 Interactive、凭据侧入站、领域侧入站、Edge、Console 和 legacy 消费者共十个 Python 服务统一重启并回读 `active / NRestarts=0`；启动时间集中在北京时间 22:18:45 至 22:19:02，部署窗口 warning 为 0。5432、8765 与 8780 都只监听 `127.0.0.1`。Caddy 因 ICP 边界继续保持 `disabled / inactive`。
- 一次性 PostgreSQL 数据库执行两组真实双连接竞争。相同版本的两个取消确认只有一路执行，另一路幂等回放；aggregate version 只增加 1，取消审计恰好 1 条。暂停与 Human dispatch 竞争只允许一路成功，本轮结果为 dispatch 成功、pause 冲突，最终保持 `running / waiting_human`，没有产生“已经返回暂停但仍新增派单”的非法组合。测试库与三个远端临时上传件已删除并回读为 0，正式 release wheel 保留且哈希一致。真实飞书命令、普通 Human Task 和决定卡取消收口尚未验收。
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，静态 token 为 64 字符且不进入命令行或日志。未认证 API 返回 401；认证后的真实页面加载 Owner 实例。真实 Chrome 在 `source_grounded_reject_20260806_001940` 回读“已完成 4/4、实例版本 16”、Agent、Tool 与最终 Human 三个 Attempt 2 节点，以及 00:53:27 由当前 Owner 发起且影响三节点的受控节点重启。`source_grounded_20260805_234517` 同时回读“未发现多轮执行节点”和“最近审计中未发现受控重启”。页面明确 Console 只读，回复与操作仍在 Agent 对话或飞书入口完成。既有标签页不会自动替换已加载的旧 JavaScript，部署后验收必须显式刷新。
...
- 当前 Target 开发发布件对应内容提交 `770243a02b116e12583ceebdb8362fd40b7fe0a7`，在既有自然语言草稿、来源约束复核、返工上下文和 Owner 只读 Console 基础上增加暂停、继续与安全取消。长期库为十九份 migration，九个 Target 服务与一个 legacy 事件消费者均回读 `active / NRestarts=0`；生命周期 PostgreSQL 双连接竞争与真实飞书命令均已通过。普通 Human Task 在实例取消后由飞书服务端回读为 `done`，Human 决定卡在取消后原位更新为无控件“复核已取消”，对应 PostgreSQL Instance、Node、Attempt、Projection 和审计终态一致。
...
- 长期开发库已应用 `0001_workflow` 到 `0019_draft_generation_progress`。第十九份 migration 增加自然语言草稿的独立生成租约、阶段进度 revision 与最终回复栅栏；前十八份继续覆盖领域状态、投影、Inbox、模板、Edge、IM 命令、重启、编辑、人员分工、恢复卡、canonical 动作、首反馈指标和事务后通知。通知不携带业务状态，凭据侧默认验证上限仍为 24 次，结构化日志中的非零 `exhausted` 必须告警。
