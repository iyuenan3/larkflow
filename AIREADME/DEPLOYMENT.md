...
- 本次只重启 `larkflow-target-console.service`。Runtime、Draft Generation Worker、Projection、两个 Interactive、凭据侧入站、领域侧入站、Edge、Console 和 legacy 消费者共十个 Python 服务均回读 `active / running / NRestarts=0`。5432、8765 与 8780 都只监听 `127.0.0.1`，公网仍只有 SSH。Caddy 因 ICP 边界继续保持 `disabled / inactive`。
