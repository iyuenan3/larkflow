...
- 长期开发库已应用 `0001_workflow` 到 `0005_template_lifecycle`。第五份 migration 为 Template aggregate 增加乐观并发版本，并增加不可变模板审计事件。Inbox 以飞书 event ID 作为 tenant 内幂等键，分开记录校验阶段与领域处理阶段。
...
- 正式模板 CLI 已用合成输入依次完成模板创建、启用、从模板创建草稿、只读预览和确认。实例 `template_entry_20260801_213749` 保存 `target_agent_review:1` 完整快照，首节点为 `waiting_human` 且只生成一条 Task Projection。该流程仍是开发验证，不含用户业务数据。
