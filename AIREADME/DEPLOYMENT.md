...
- 当前 Target 与 legacy 已安装包含企业共享正文与 ContextBundle 的 wheel，SHA-256 为 `16e1833fe05579661bfec72ccbd2fcfeee6fa8b86f7fe48eeb80fa3e60be00c4`。两个 venv 的 `pip check` 通过，安装态 `knowledge_context.py`、`console_admin_knowledge.py`、`knowledge/blob.py` 和 `0026_enterprise_knowledge_content_authorization.sql` 与本地源码哈希一致。长期库已增量应用并重入 0026，ledger 为 `26 / 0026_enterprise_knowledge_content_authorization`。一次性真实 PostgreSQL 从空库运行 26 份 migration 并重入，`tests/test_workflow_postgres.py` 为 `30 passed`；另一个一次性数据库验证强制失败后 ledger 仍为 25 且 0026 表不存在，随后正常应用 0026 并重入。两个测试库均已精确删除并回读不存在。
...
- 开发环境使用持久、非 symlink 的企业资料目录，并只为 Console、Draft Generation Worker 与 Target Runtime 配置相同正文根和显式 allow 外发策略。synthetic/public 正文探针确认 planning 与 Agent 各获得一个安全来源，能力精确为 `context.read.enterprise_knowledge`；撤销后新 planning 不再获得来源，新 Agent ContextBundle 以 `EnterpriseKnowledgeContextRejected` 失败。PostgreSQL 回读一个 revoked 版本、一个正文绑定授权证明与 published、revoked 两条追加审计；安全表和运行证据不含正文、object key 或路径。本轮未创建或修改飞书资源，真实管理员浏览器发布与真实内部资料验收仍待手工完成。
- 上一版 metadata-only 回滚 wheel 对应提交 `fc2fc2e73ff0e4001d952f6cba7e4a20e6b04005`，保留于 `/srv/larkflow/target/releases/knowledge_api_20260819_005038_fc2fc2e/larkflow-0.0.2-py3-none-any.whl`，SHA-256 为 `c608eef5e6bd5ae85a95797ea61b2ca0d0dce5f87a31b10b1130f0d9f1e1c0e9`。本轮依照此前明确的不备份边界没有创建新的数据库备份；0026 为追加型事务 migration，旧 wheel 与旧 release 继续作为应用回滚点。
