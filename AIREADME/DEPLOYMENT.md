...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、十个 IM 窄命令、人员选择卡、未来区域编辑、节点与完整实例安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前代码发布 wheel SHA-256 为 `ab18ddf5a2cf42084129893a9e2e16640ed1b769fb4bc089aa361128588688e0`，保存在 `releases/20260804_012909_card_valid_19ea7be/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `19ea7be`；升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260804T012922+0800.dump` 已读回为成功、`0600` 且 115223 bytes，长期库已应用十四份 migration。代码发布后的六个 Python 服务均回读 active、`NRestarts=0`，部署窗口无 warning 级日志。安装使用现有 root 管理的虚拟环境；一次以服务用户替换旧 root-owned metadata 失败时，六个服务仍保持停止，随后使用统一 root 安装完成，pip 遗留目录被移入对应发布目录的可恢复备份，不存在混合运行窗口。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
