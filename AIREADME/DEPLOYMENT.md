...
- 当前开发发布件对应内容提交 `b7e589ba4af0398573ec995254dd61e9b1a4508c`，包含裸 `/larkflow draft` 的 Card 2.0 引导、有界重生成、独立 Draft Generation Worker、阶段 revision 栅栏、按消息 ID 的多阶段卡片更新，以及来源约束型材料复核。长期库为十九份 migration，八个 Target 服务与一个 legacy 事件消费者均回读 `active / running / NRestarts=0`。
