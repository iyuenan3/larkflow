...
- 当前 Target 应用 wheel 对应内容提交 `b60cbbd8beb98742cc80082df78ac185274e3a8a`，管理员 allowlist 运维工具对应 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS；Console 继续只监听 `127.0.0.1:8780`。九个 Target 服务、legacy 消费者与 Caddy 均回读 `active / NRestarts=0`。飞书 OAuth 应用凭证、模型凭据与管理员 allowlist 继续只保存在受限 env；Console 本身不持有模型或飞书 bot profile。
- 当前发布件为 `/srv/larkflow/target/releases/20260809_175848_console_canvas_b60cbbd/larkflow-0.0.2-py3-none-any.whl`，权限为 `0640 root:lf_target_dev`，大小为 990208 bytes，SHA-256 为 `e558f75a2e495d7d1e79e52a1b36458fb55c76ed6eca608e365dc94d43f97221`。升级前备份服务回读 `Result=success / ExecMainStatus=0`；migration runner 返回 `versions=[]`，ledger 为 `23 / 0023_console_draft_requests`，现有有效 Console 会话保持 1 条。首次以应用用户重装时，pip 因历史卸载备份目录中的 root-owned `direct_url.json` 无法清理而返回失败；部署随后只删除两个已解析的 `~arkflow` 遗留备份目录，并以应用用户完整重装，最终 `pip check` 无断裂依赖且不再有 invalid distribution。安装资源、公网 JavaScript 与 CSS 哈希一致；公网工作台为 200，未登录图编辑接口为 401，安全响应头齐全。此次只重启 Console，十个 Python 服务和 Caddy 均为 `active / NRestarts=0`，部署窗口 warning 为 0。
