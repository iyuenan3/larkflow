...
- 当前 Target 开发发布件对应内容提交 `ee2fa9439594d765cd08f2caa0f7ecb20d30d78b`，在既有具体退回意见与表单绑定能力上新增 Owner 只读中央控制台。wheel SHA-256 为 `58b27648ccaf3f863cf4bb0ca820b3e2209523b58b0574af626aa303c0e4ff5c`，位于 `/srv/larkflow/target/releases/20260806_162126_console_ee2fa94/`。完整离线套件为 `922 passed, 18 skipped`，控制台页面资源已从 wheel 回读。legacy 事件消费者不需要本次只读控制台代码，继续使用原虚拟环境。
- 升级前 PostgreSQL custom-format 备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260806T162137+0800.dump` 为 211967 bytes、`0600 lf_target_dev:lf_target_dev`，备份服务回读 `Result=success / ExecMainStatus=0`。migration runner 返回 `versions=[]`，长期库保持 `19 / 0019_draft_generation_progress`，本次不增加 schema。
- Runtime、Draft Generation Worker、Projection、两个 Interactive、凭据侧入站、领域侧入站、Edge、Console 和 legacy 消费者共十个 Python 服务，统一重启后均回读 `active / NRestarts=0`；部署窗口 warning 为 0。5432、8765 与 8780 都只监听 `127.0.0.1`。Caddy 因 ICP 边界继续保持 `disabled / inactive`。
- `larkflow-target-console.service` 以 `lf_target_dev` 运行，env 为 `0640 root:lf_target_dev`，静态 token 为 64 字符且不进入命令行或日志。真实 API 验收返回 30 条当前 Owner 流程、运行中实例 4 个节点与 16 条审计；另一个 Owner 的真实实例统一返回 404。SSH 隧道浏览器验收覆盖运行中 DAG、Attempt 1/2、审计时间线、草稿 0/3、无 error 或 warning 日志，以及点击“锁定”立即回到令牌页。隧道、两端临时凭据和上传件已删除，服务保持 loopback 常驻。
...
- 当前 Target 开发发布件对应内容提交 `ee2fa9439594d765cd08f2caa0f7ecb20d30d78b`，包含裸 `/larkflow draft` 的 Card 2.0 引导、有界重生成、独立 Draft Generation Worker、阶段 revision 栅栏、按消息 ID 的多阶段卡片更新、来源约束型材料复核、必填退回意见、目标 Attempt 返工上下文与 Owner 只读中央控制台。长期库为十九份 migration，九个 Target 服务与一个 legacy 事件消费者均回读 `active / NRestarts=0`。
