...
- 当前 Target 与 legacy 应用 wheel 对应内容提交 `6e6e3895ac9bb355f40c017e4b5ffe395f4ddca4` 和测试修正 `3a93fd2af9cfecdc00cada822ab8705232018205`，包含 Phase 2A、Phase 2B Agent Attempt 附件上下文、薄 Python 豆包 `SearchProvider` 和原生飞书 Docx 交付物。发布件位于 `/srv/larkflow/target/releases/phase2b_20260818_234507_3a93fd2/larkflow-0.0.2-py3-none-any.whl`，SHA-256 为 `dda518748ff93be4e70e8e040ebff5edfd80bddddb9129c9be14b21c9c135edd`。两个 venv 的 `pip check` 通过，安装态 `agent_context.py`、搜索适配器和交付物 adapter 与本地源码哈希一致。十个 larkflow Python 服务均为 `active / running / NRestarts=0`，部署窗口 warning 及以上为 0。
- 用户明确选择不备份旧测试数据，开发数据库 `larkflow_target_dev` 曾按同名、同 owner 重建为空库。完整 migration chain 与重入均通过，当前 ledger 保持 `24 / 0024_console_project_attachments`。Phase 2B 一次性真实 PostgreSQL 从空库运行 24 份 migration 并重入，`tests/test_workflow_postgres.py` 为 `27 passed`；promotion fixture 先创建外键要求的真实 Instance，没有放松生产约束。安装态 synthetic Runtime 探针回读 Node/Attempt 绑定的能力信封，并确认正文、object key 和 claim 没有持久化；一次性数据库随后精确删除并回读不存在。
