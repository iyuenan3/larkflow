...
- 当前开发发布件对应内容提交 `50d3d7136160f4208421ff194f6929200103f141`，包含失败自动节点的 Owner 恢复卡、重试、人工接管、migration `0015_recovery_cards`，以及对 lark-cli 真实回调中字符串化 `action_value`、缺失 `action_name` 和微秒时间戳的归一化。完整离线套件为 `776 passed, 13 skipped`；长期库 migration、六服务部署和真实飞书恢复闭环均已验收。
...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、十个 IM 窄命令、人员选择卡、失败恢复卡、未来区域编辑、节点与完整实例安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前代码发布 wheel SHA-256 为 `5000b1ebdc42524cc7f709ebc7f2fc723d2fd969e9d234540b132001f958c56b`，保存在 `releases/20260804_050057_callback_time_50d3d71/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `50d3d7136160f4208421ff194f6929200103f141`；恢复升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260804T035234+0800.dump` 已读回为成功、`0600`、`lf_target_dev` 所有且 115831 bytes，长期库已应用十五份 migration。代码发布后的六个 Python 服务均回读 active、`NRestarts=0`。安装使用现有 root 管理的虚拟环境；一次以服务用户替换旧 root-owned metadata 失败时，六个服务仍保持停止，随后使用统一 root 安装完成，pip 遗留目录被移入对应发布目录的可恢复备份，不存在混合运行窗口。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
...
- 开发交互延迟配置已与内容提交 `409167d` 对齐：Runtime 的 `LARKFLOW_TARGET_IDLE_MAX_SECONDS` 和 Projection 的 `LARKFLOW_TARGET_PROJECTION_IDLE_MAX_SECONDS` 均由 5 秒收紧到 1 秒，只重启这两个服务后均为 active、`NRestarts=0`。原 env 已分别备份为 `/etc/larkflow-target.env.bak-20260804T014440+0800-idle5` 与 `/etc/larkflow-target-projection.env.bak-20260804T014440+0800-idle5`，所有者和权限保持不变。真实人员分工卡片回调的服务端总耗时从 8.881 秒降至 3.272 秒，用户观察约 4 秒；这不改变 PostgreSQL 耐久状态机和幂等边界。长期方案仍应评估 PostgreSQL `LISTEN/NOTIFY` 或等价唤醒机制，避免以更高空轮询频率换取低延迟。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的十四份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
