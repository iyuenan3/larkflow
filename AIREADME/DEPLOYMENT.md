...
- Owner Console 当前对应内容提交 `8ba0ab9d93554b7958a650492e0282ad40db0d2e`，systemd 出站边界对应 `bc961b6`。Caddy 通过公网 IP 的 80 / 443 端口提供 HTTPS，Console 继续只监听 `127.0.0.1:8780`；两项服务均为 `active / NRestarts=0`，公网与 loopback `/console/` 均返回 200。飞书 OAuth 应用凭证已由官方令牌端点验证，受限 env 保持 `0640 root:lf_target_dev`，管理员 allowlist 只保存于该受限 env。
- 飞书应用已发布网页应用与机器人两种能力，网页应用作为工作台默认入口。至少两名真实成员已完成网页授权登录、本人 Owner 数据读取与跨 Owner 隔离验证，机器人命令入口继续可用。完成登录后的会话摘要已保存到 PostgreSQL；真实会话在 Console 重启后仍有效，用户直接刷新保持登录。服务端 allowlist 命中的成员可读取当前企业管理员聚合，并通过五分钟耐久预览和显式确认撤销其他浏览器会话；当前会话只能注销，普通成员访问管理路由返回 404。该证据关闭开发环境单进程会话、最小管理员观察面和单会话撤销缺口，不等于生产发布；入口仍使用公网 IP，也没有 allowlist 自助管理、批量撤销、正式域名或跨区域容灾。
- 当前 Target 开发发布件对应内容提交 `8ba0ab9d93554b7958a650492e0282ad40db0d2e`，在既有生命周期、来源约束复核、Owner Console、待处理中心、PostgreSQL 耐久会话和管理员只读概览上增加受审计的其他会话撤销。wheel SHA-256 为 `b2cff677a419f7151f6ceb6dc8986fcd061999406cbd8212ac2cdde7504fecc8`，位于 `/srv/larkflow/target/releases/20260807_212230_session_gov_8ba0ab9/`。完整离线结果为 `965 passed, 21 skipped`；真实 PostgreSQL 与 HTTP 验收覆盖二十一份 migration、管理员授权、普通成员 404、当前会话保护、预览确认、双连接竞争、撤销失效、幂等回放和追加型审计。Target 虚拟环境已替换为该 wheel；legacy 事件消费者未因本次 Console 会话治理变化重装。
...
- 当前 Target 开发发布件对应内容提交 `8ba0ab9d93554b7958a650492e0282ad40db0d2e`，在既有自然语言草稿、来源约束复核、返工上下文、生命周期、Owner Console、待处理中心、PostgreSQL 耐久会话和管理员概览基础上增加受审计的其他会话撤销。长期库为二十一份 migration；升级前数据库备份服务返回 `Result=success / ExecMainStatus=0`，备份为 `/var/backups/larkflow-postgres/larkflow_target_dev-20260807T212320+0800.dump`，大小 230122 bytes，模式 `0600`。本次只重启 Console，十个 Python 服务与 Caddy 均回读 `active / running / NRestarts=0`。真实 HTTP 验收回读会话列表 200、当前会话撤销拒绝 409、预览 201、确认 200、幂等回放一致、已撤销会话 401、普通成员 404 和审计一条；短期验收会话已清理，原有真实登录会话仍为一条。新会话治理面板的真实登录浏览器视觉验收仍待完成；既有待处理中心查询、生命周期竞争与真实飞书命令证据继续有效。
