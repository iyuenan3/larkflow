...
- 当前开发发布件对应内容提交 `dc77faad92e5d45f0271e45747bcbede3dd2ac02`，包含所有可操作卡片的耐久后即时视觉反馈、人员选择卡单动作约束、历史重复回调无损保留，以及既有失败恢复能力。完整离线套件为 `779 passed, 14 skipped`；长期库第十六份 migration、六服务部署和真实飞书人员选择卡最终状态读回均已验收。
...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、十个 IM 窄命令、人员选择卡、失败恢复卡、未来区域编辑、节点与完整实例安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前代码发布 wheel SHA-256 为 `f4a830387ab058af19bc465789f050daca6d6f0d25ab139b427de9ebc04babbb`，保存在 `releases/20260804_150033_card_feedback_dc77faa/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `dc77faad92e5d45f0271e45747bcbede3dd2ac02`；升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260804T150052+0800.dump` 已读回为成功、`0600`、`lf_target_dev` 所有且 122949 bytes，长期库已应用十六份 migration。代码发布后的六个 Python 服务均回读 `active / running / NRestarts=0`。安装使用现有 root 管理的虚拟环境；Target 与 legacy 在同一停机窗口统一安装同一 wheel，migration 成功后才统一启动，不存在混合运行窗口。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
...
- 开发交互延迟配置已与内容提交 `409167d` 对齐：Runtime 的 `LARKFLOW_TARGET_IDLE_MAX_SECONDS` 和 Projection 的 `LARKFLOW_TARGET_PROJECTION_IDLE_MAX_SECONDS` 均由 5 秒收紧到 1 秒，只重启这两个服务后均为 active、`NRestarts=0`。原 env 已分别备份为 `/etc/larkflow-target.env.bak-20260804T014440+0800-idle5` 与 `/etc/larkflow-target-projection.env.bak-20260804T014440+0800-idle5`，所有者和权限保持不变。既有真实人员分工卡片回调的服务端总耗时从 8.881 秒降至 3.272 秒，用户观察约 4 秒。本轮新增耐久落库后的直接“处理中”更新，最长等待 3 秒，并以 10 秒可认领延后作为崩溃兜底；真实点击的领域处理在入站后 3.393 秒完成，最终回复在 5.793 秒完成。用户忘记记录瞬态视觉耗时，因此本轮只确认最终卡片服务端读回，不把“即时”写成已测得毫秒数。长期方案仍应评估 PostgreSQL `LISTEN/NOTIFY` 或等价唤醒机制，避免以更高空轮询频率换取低延迟。
- 即时反馈版本部署后，Owner 在测试组织创建新的 `collaborative_agent_review` 人员选择卡并提交分工。长期库只保存一个 canonical 动作，动作进入 `processed / draft_created`、回复进入 `sent`，同卡动作数、canonical 动作数和草稿数均为 1，持久化错误数为 0。机器人身份从飞书服务端读回原消息为 `interactive / updated`，内容包含“人员分工已确认”，不含“处理中”或“确认人员分工”按钮标签。该验收确认最终卡片和幂等结果，不替代下一次带计时的瞬态视觉验收。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的十六份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
