...
- 当前 Target 应用 wheel 对应 Phase 2A 主体提交 `b2a13ff1eff796723774d42ca5d04556814a38c2` 与 PostgreSQL 合同收口提交 `07de190db49839d8195cfa26967241fad7d975f6`。发布件位于 `/srv/larkflow/target/releases/phase2a_deploy_20260815_030407_07de190/larkflow-0.0.2-py3-none-any.whl`，大小为 1039998 bytes，SHA-256 为 `44f7c870000f7b7b74d1ecefba7ce02904630072956fade9d646bb74d2743df7`，安装态必需文件与 wheel 逐项匹配，`pip check` 无断裂依赖。十个 larkflow Python 服务均为 `active / running / NRestarts=0`，部署窗口 warning 为 0。
- 用户明确选择不备份旧测试数据，开发数据库 `larkflow_target_dev` 已按同名、同 owner 重建为空库。完整 migration chain 与重入均通过，当前 ledger 为 `24 / 0024_console_project_attachments`，Instance、Console Session、草稿请求和附件元数据均为 0。一次性真实 PostgreSQL 执行 `tests/test_workflow_postgres.py` 为 `26 passed`；补充验证关闭了跨请求并发 tenant 配额、逻辑撤销保留、Owner 隔离、manifest round-trip、promotion 幂等、删除保护和 migration 事务回滚。两座一次性数据库已精确删除并回读不存在。
- Console 与 Draft Generation Worker 使用同一个持久绝对附件目录，且只对这两个服务启用 `LARKFLOW_TARGET_ATTACHMENT_MODEL_EGRESS=allow`；systemd `ProtectSystem=strict` 通过精确 `ReadWritePaths` 只放行该目录。Caddy 2.11.4 候选经过真实 validate/adapt，live 配置 SHA-256 为 `748175bb5f85551fdae805f1f78b7c8d2e42e76ef00ffea5c5d510e334f70989`。公网实测普通 70000 字节 POST 返回 413，精确附件 70000 字节 POST 穿透到应用授权层返回 401，270000 字节附件 POST 返回 413；loopback 与公网 Console 为 200，三个受保护 API 均为 401，附件能力标志为 true，公网 JavaScript 与 CSS 哈希和安装态一致。数据库重建后没有合法 Owner 登录会话，因此没有伪造 cookie 或会话记录；真实 Owner 浏览器上传、列表与显式生成仍待 Maxwell 手工验收。本条只证明开发环境，不代表生产就绪。
