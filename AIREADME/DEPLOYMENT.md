...
- Projection 启动全量对账和 `reconcile-projections` 运维命令已部署。启动与手动命令均回读 `instances_scanned=1`、`nodes_scanned=2`、`unchanged=2`、`failed=0`，没有创建或重建 Task；数据库回读 8 条 Task Projection 均有 external ID，7 条已完成，1 个 Human 节点仍在等待。一次性 PostgreSQL 已验证分页枚举、缺失补建、确认删除后的受控换绑和重入；真实飞书 Task 删除后的重建尚未执行。
