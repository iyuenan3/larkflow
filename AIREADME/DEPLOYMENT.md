...
- 当前开发发布件对应内容提交 `c1d8fe510805cbe209a6275c4e4b3d8311b6692c`，包含所有可操作卡片的耐久后即时视觉反馈、人员选择卡单动作约束、首个服务端反馈耐久指标，以及既有失败恢复能力。完整离线套件为 `780 passed, 14 skipped`；长期库第十七份 migration、六服务部署、真实人员选择卡和失败恢复卡的反馈耗时及终态读回均已验收。
...
- 长期开发库已应用 `0001_workflow` 到 `0017_card_feedback_metrics`。第六份 migration 允许 Inbox 进入不可再认领的 `exhausted` 终态；第七份增加一次性配对、可撤销设备和追加型 Edge 审计；第八份增加耐久 IM 命令、发送者验证和回复状态；第九份增加 Owner 最近实例列表所需的复合索引；第十份增加短期 RestartPreview、消费版本和未消费预览部分索引；第十一份为 RestartPreview 增加显式 scope、可空节点键和作用域约束；第十二份增加耐久 GraphEditPreview、候选 Snapshot 哈希、消费版本与未消费预览索引；第十三份保存认证 mention；第十四份保存人员选择卡候选快照、回调、处理和回复状态；第十五份为恢复卡片增加耐久更新 token；第十六份为同卡回调增加 canonical 标记与部分唯一索引；第十七份在 `workflow_im_commands` 与 `workflow_role_binding_actions` 上增加 `feedback_status`、`feedback_elapsed_ms` 和 `feedback_completed_at`，并用完整性约束拒绝部分写入。迁移前真实库已有一组五条历史同卡动作，迁移后最早一条保持 canonical，其余四条保留为非 canonical 审计，canonical 重复组为零。凭据侧默认验证上限为 24 次，结构化日志中的非零 `exhausted` 必须告警。
...
- 包含严格字段校验、Template Service、正式模板 CLI、`llm.generate`、`content.check`、十个 IM 窄命令、人员选择卡、失败恢复卡、未来区域编辑、节点与完整实例安全重启、完成投影、有限入站验证重试、Projection 启动全量对账、Human Task 完成状态轮询和 Personal Edge 的 wheel 已安装到 Target 与 legacy 开发虚拟环境。当前代码发布 wheel SHA-256 为 `779990ca33771e0eb2ece2fa30bc8c1d4d2062625e4ded0f08e90d951d403204`，保存在 `releases/20260804_162012_card_metrics_c1d8fe5/larkflow-0.0.1-py3-none-any.whl`，对应内容提交 `c1d8fe510805cbe209a6275c4e4b3d8311b6692c`；升级前备份 `/var/backups/larkflow-postgres/larkflow_target_dev-20260804T162043+0800.dump` 已读回为成功、`0600`、`lf_target_dev` 所有且 124574 bytes，长期库已应用十七份 migration。代码发布后的六个 Python 服务均回读 `active / running / NRestarts=0`，部署窗口 warning 级日志数为 0。安装使用现有 root 管理的虚拟环境；Target 与 legacy 在同一停机窗口统一安装同一 wheel，migration 成功后才统一启动，不存在混合运行窗口。`/etc/larkflow-target.env` 已在明确授权下启用开发用 OpenAI 兼容主路由和确定性内容检查，不配置备用线路；env 保持 `0640 root:lf_target_dev`，路由真值不进入仓库或日志。
...
- 开发交互延迟配置已与内容提交 `409167d` 对齐：Runtime 的 `LARKFLOW_TARGET_IDLE_MAX_SECONDS` 和 Projection 的 `LARKFLOW_TARGET_PROJECTION_IDLE_MAX_SECONDS` 均由 5 秒收紧到 1 秒，只重启这两个服务后均为 active、`NRestarts=0`。原 env 已分别备份为 `/etc/larkflow-target.env.bak-20260804T014440+0800-idle5` 与 `/etc/larkflow-target-projection.env.bak-20260804T014440+0800-idle5`，所有者和权限保持不变。内容提交 `c1d8fe5` 把首个反馈的服务端区间改为耐久指标：单调时钟从有效回调被接受开始，覆盖动作插入和直接卡片更新，直到该更新调用返回；该指标不包含用户物理点击到服务端接收的网络时间，也不包含飞书客户端渲染时间。本轮人员选择卡测得 1.264 秒，失败恢复卡测得 0.990 秒。长期方案仍应评估 PostgreSQL `LISTEN/NOTIFY` 或等价唤醒机制，避免以更高空轮询频率换取低延迟。
...
- migration：由目标应用启动入口调用 package-data migration runner。长期库的十七份 migration 已落地，后续不得手工改 schema 后跳过 migration ledger。
