...
- 企业资料上下文候选配置：Console、Draft Generation Worker 与 Target Runtime 必须共享同一持久绝对 `LARKFLOW_TARGET_ENTERPRISE_KNOWLEDGE_ROOT`，并显式设置 `LARKFLOW_TARGET_ENTERPRISE_KNOWLEDGE_MODEL_EGRESS=allow` 才能发布或读取正文。缺失、deny、撤销、版本漂移、proof 不匹配、哈希或大小异常、预算超限和临时 Blob 故障都不能静默忽略资料继续运行。当前代码已提交但未部署，systemd 还需在部署时给 Console 和 Runtime 最小目录权限并回读 realpath、owner、mode 与非 symlink 边界。
