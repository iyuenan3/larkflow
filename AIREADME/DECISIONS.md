...
- 顺带记录（**未解**）：`lark-cli event _bus` 是它自己的**共享单例**，不随我们的 consume 子进程一起走，干净停机后仍然存活并被下一个 daemon 复用。实测见过一个 **3 小时 12 分**的旧 bus 被新起的 consumer 挂上去。ADR-039 那个「`task.task.update_user_access_v2` 根本不推送」的谜，以及 ADR-038 那次「10 小时 48 分零事件」，都可能与这个跨重启复用的旧 bus 有关。**这是线索不是结论**，没有验证过。
