...
>
> 文档状态：2026-08-04 Phase 1 中央工作流基础实现，飞书 IM 窄命令、Human-Agent-Tool-Human、完成投影、Owner 查询、两类重启、运行中未来区域编辑和自动节点失败恢复均已在开发环境闭环。失败恢复真实验收覆盖两个连续重试、新 Attempt 历史保护、人工接管、飞书 Task 完成、完成文档与最终通知。跨人员分工已同时完成群聊 `role=@成员` 和单聊 Card 2.0 人员选择的开发真栈验收。可操作卡片现统一在动作耐久落库后先尝试显示无按钮的“处理中”，再收口为无按钮的成功或拒绝状态；人员选择卡与失败恢复卡均已耐久记录首个服务端反馈耗时。四类 Target 常驻 Worker 已使用 PostgreSQL `LISTEN/NOTIFY` 在事务提交后唤醒，通知不承载业务状态，原有有界轮询继续作为可靠性兜底。逐项完成时间缺陷已在 `a506e7d` 修正并经真实 PostgreSQL 与五次飞书卡片验收；首反馈 P50 / P95 为 0.991 / 1.274 秒，突发样本显示后续串行外部调用存在队头阻塞。Personal Agent Edge Proof v0 已完成 loopback 开发部署与 SSH 隧道跨机验收，公网设备链路受 ICP 接入备案阻断，Caddy 已停止。`Target` 表示目标产品契约，`As-built` 表示当前代码事实，两者不得混写。
>
> last-synced: a506e7df078cbb6d8fa5f359272505ba62cac241 · 2026-08-04
...
| ARCHITECTURE | ✅ | Target 模块化单体、中央 Worker、PostgreSQL 通知唤醒与轮询兜底、逐项完成计时、飞书投影、失败恢复、Agent / Tool adapter、Edge Proof 和剩余差距 |
| RELATIONS | ✅ | Target 飞书、mention 与人员选择卡身份边界、中央 lark-cli、Edge HTTPS、Node Runner 与 LangGraph 边界 |
| ROADMAP | ✅ | Phase 1 已落码跨人员分工、飞书窄命令、逐项完成计时、数据库通知唤醒、受控变化、失败恢复与完成通知，下一步验证有界并发 |
| SPEC | ✅ | legacy 契约、Target CLI、数据库通知唤醒、十个飞书窄命令、人员选择与失败恢复卡、逐项反馈与完成指标、Task 入站、受控变化、完成投影与私有 Edge v1 HTTP |
| DEPLOYMENT | ✅ | Legacy ECS 与 Target 六服务、十八份 migration、四条监听连接、五次卡片批次验收、逐项延迟实测、PostgreSQL、备份和恢复实录 |
| CONVENTIONS | ✅ | Target 与 As-built 的命名、状态、安全和文档约定 |
| DECISIONS | ✅ | Append-only ADR 历史，最新记录批次 Worker 必须逐项结算完成时间 |
| CHANGELOG | ✅ | Append-only 已实现变更，最新为逐项完成计时修正与五次真实卡片验收 |
| MEMORY | ⚑ | Append-only 经验，仍含语义占位，已记录回调漂移、即时反馈竞态、数据库通知与批次计时边界 |
