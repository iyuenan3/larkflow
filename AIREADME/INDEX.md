...
>
> P0 结构能力已由内容提交 `6d2d9fa22b7e6926cfe5a1bcf714ccccd073b6d3` 和装配修复 `c7e7d901237a3b72d3d265382e1e2239a0626abd` 提交、推送并部署：节点交付物合同、Human 结构化提交、飞书 Task 深链与完成旁路阻断、服务端内部 key、插入前重连，以及旅游规划深流程与受控 `web.search` Tool 均已进入开发真栈。完整离线套件为 `1058 passed, 24 skipped`。真实苏州样本生成 6 节点图，完成结构化需求、景点/美食/交通三项联网研究和综合 Agent；飞书需求待办从服务端回读为 `done`。最终 Human 真实退回结果，因为部分引用返回 404 或只重定向到首页，正文却作绝对真实性声明。结构与交付物流转门槛已关闭，来源可访问性、证据一致性和网页决定后飞书卡片收口仍阻断邀请测试。契约见 [PRD.md](PRD.md)、[SPEC.md](SPEC.md) 与 [ARCHITECTURE.md](ARCHITECTURE.md)，进度边界见 [ROADMAP.md](ROADMAP.md) 和 [CHANGELOG.md](CHANGELOG.md)。
>
...
>
> last-synced: c7e7d901237a3b72d3d265382e1e2239a0626abd · 2026-08-10
