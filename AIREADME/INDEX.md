...
>
> last-synced: 4c587cc20c49aa668941206d1d0c4080bcd416b9 · 2026-08-07
...
| ARCHITECTURE | ✅ | Target 模块化单体、飞书 OAuth 与 PostgreSQL 耐久会话的 Owner 工作台、服务端 allowlist 管理员聚合与受审计会话撤销、独立凭据侧 Interactive 双副本、无凭据 Draft Generation Worker、来源契约检查、带返工上下文的人类决定卡、PostgreSQL 通知唤醒与轮询兜底、飞书投影、失败恢复、Edge Proof、macOS 版本化安装与剩余差距 |
| RELATIONS | ✅ | Target 飞书、mention 与人员选择卡身份边界、中央 lark-cli、Edge HTTPS、Node Runner 与 LangGraph 边界 |
| ROADMAP | ✅ | 首批三项真实工作小样本已建立，员工工作台身份、管理员聚合和受审计会话撤销已部署，下一门槛为新面板真实目视、生产限流、安全响应头及 Edge 正式分发 |
| SPEC | ✅ | legacy 契约、Target CLI、Owner Console HTTP、OAuth v3、PostgreSQL 耐久会话、服务端管理员 allowlist、聚合与受审计会话撤销、独立 interact 与 draft generation Worker、数据库通知唤醒、来源声明与确定性检查、必填退回意见的人类决定卡、十五个飞书窄命令、模板与无模板草稿、暂停继续取消、Task 入站、受控变化、完成投影与私有 Edge v1 HTTP、前台客户端、doctor 及 macOS manager |
| DEPLOYMENT | ✅ | Legacy ECS 与当前 Target 九服务、二十一份 migration、七条通知连接、公网 IP Console、真实飞书登录、耐久会话、管理员聚合和受审计会话撤销、暂停继续取消的真实飞书收口、独立草稿生成、Edge serve、macOS 安装、备份与回滚实录 |
| CONVENTIONS | ✅ | Target 与 As-built 的命名、状态、安全和文档约定 |
| DECISIONS | ✅ | Append-only ADR 历史，最新为首个管理员写操作只撤销其他浏览器会话 |
| CHANGELOG | ✅ | Append-only 已实现变更，最新为管理员会话治理的开发部署、竞争与权限隔离验收 |
