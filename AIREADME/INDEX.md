...
| ARCHITECTURE | ✅ | Target 模块化单体、中央 Worker、PostgreSQL 通知唤醒与轮询兜底、飞书投影、卡片反馈耐久观测、失败恢复、Agent / Tool adapter、Edge Proof 和剩余差距 |
| RELATIONS | ✅ | Target 飞书、mention 与人员选择卡身份边界、中央 lark-cli、Edge HTTPS、Node Runner 与 LangGraph 边界 |
| ROADMAP | ✅ | Phase 1 已落码跨人员分工、飞书窄命令、卡片反馈观测、数据库通知唤醒、受控变化、失败恢复与完成通知，Edge 公网设备链路仍受备案阻断 |
| SPEC | ✅ | legacy 契约、Target CLI、数据库通知唤醒、十个飞书窄命令、人员选择与失败恢复卡、卡片反馈指标、Task 入站、受控变化、完成投影与私有 Edge v1 HTTP |
| DEPLOYMENT | ✅ | Legacy ECS 与 Target 六服务、十八份 migration、四条监听连接、卡片反馈实测、飞书真实闭环、PostgreSQL、备份和恢复实录 |
| CONVENTIONS | ✅ | Target 与 As-built 的命名、状态、安全和文档约定 |
| DECISIONS | ✅ | Append-only ADR 历史，最新记录 PostgreSQL 通知只负责唤醒、队列表与轮询继续保证可靠性 |
| CHANGELOG | ✅ | Append-only 已实现变更，最新为 PostgreSQL Worker 唤醒与真实飞书验收 |
| MEMORY | ⚑ | Append-only 经验，仍含语义占位，已记录回调漂移、即时反馈竞态、人工计时与数据库通知边界 |
