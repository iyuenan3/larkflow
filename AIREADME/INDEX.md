...
| ROADMAP | ✅ | Phase 1 已部署凭据侧双 Interactive 副本，下一步验收员工设备前台 Edge serve |
| SPEC | ✅ | legacy 契约、Target CLI、独立 interact Worker、数据库通知唤醒、十个飞书窄命令、人员选择与失败恢复卡、Task 入站、受控变化、完成投影与私有 Edge v1 HTTP 及前台客户端 |
| DEPLOYMENT | ✅ | Legacy ECS 与 Target 八服务、十八份 migration、六条监听连接、双 Interactive 副本、PostgreSQL、备份、回滚实录及未部署 Edge 候选边界 |
| CONVENTIONS | ✅ | Target 与 As-built 的命名、状态、安全和文档约定 |
| DECISIONS | ✅ | Append-only ADR 历史，最新记录 Edge 使用用户启动的前台持续会话 |
| CHANGELOG | ✅ | Append-only 已实现变更，最新为前台 Personal Agent Edge serve |
