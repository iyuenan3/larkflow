...
>
> last-synced: 29c16c763aa3fdb5dbbc6ee9e27a566095e9e895 · 2026-08-07
...
| PRD | ✅ | Target 单层 DAG MVP、飞书应用内本人工作台身份与耐久会话边界、来源约束型结果、带具体意见的人类决定与 Edge Proof 功能契约 |
| DAG_TEMPLATE_SPEC | ✅ | v0.2 模板、mention 角色绑定、草稿预览、未来区域编辑和两类重启已实现 |
| ARCHITECTURE | ✅ | Target 模块化单体、飞书 OAuth 与 PostgreSQL 耐久会话的 Owner 只读工作台、独立凭据侧 Interactive 双副本、无凭据 Draft Generation Worker、来源契约检查、带返工上下文的人类决定卡、PostgreSQL 通知唤醒与轮询兜底、飞书投影、失败恢复、Edge Proof、macOS 版本化安装与剩余差距 |
| RELATIONS | ✅ | Target 飞书、mention 与人员选择卡身份边界、中央 lark-cli、Edge HTTPS、Node Runner 与 LangGraph 边界 |
| ROADMAP | ✅ | 首批三项真实工作小样本已建立，员工工作台 OAuth、多用户隔离与耐久会话已完成，下一门槛为最小管理员后台、正式域名、生产限流及 Edge 正式分发门禁 |
| SPEC | ✅ | legacy 契约、Target CLI、Owner 只读 Console HTTP、OAuth v3、PostgreSQL 耐久会话与双鉴权模式、独立 interact 与 draft generation Worker、数据库通知唤醒、来源声明与确定性检查、必填退回意见的人类决定卡、十五个飞书窄命令、模板与无模板草稿、暂停继续取消、Task 入站、受控变化、完成投影与私有 Edge v1 HTTP、前台客户端、doctor 及 macOS manager |
| DEPLOYMENT | ✅ | Legacy ECS 与当前 Target 九服务、二十份 migration、七条通知连接、公网 IP Console、真实飞书登录与重启后会话保持、暂停继续取消的真实飞书收口、独立草稿生成、Edge serve、macOS 安装、备份与回滚实录 |
| CONVENTIONS | ✅ | Target 与 As-built 的命名、状态、安全和文档约定 |
| DECISIONS | ✅ | Append-only ADR 历史，最新为员工工作台已完成会话只存 PostgreSQL 摘要并跨 Console 重启保持 |
| CHANGELOG | ✅ | Append-only 已实现变更，最新为 PostgreSQL 耐久 Console 会话部署与真实重启回归 |
