...
typed 豆包搜索现在从 NodeSpec 明确声明消费的已确认直接依赖编译实际 query，不再只发送通用静态指令。景点与交通字段按确定性优先级压缩到 100 字，未授权字段不进入查询；交通枢纽只从同一交通语义组内解析，能够从“上海往返乌鲁木齐航班”提取乌鲁木齐，不会跨分号借用前一个景点实体。明显与目的地和关键实体完全无交集的来源以 `search_evidence_missing` 拒绝。实际 query 同时保存在 Tool result 和完成审计。实例 `console_draft_cc16a99b9b79e282488ec8453658e8e2` 已通过两个正式节点 restart 重新执行景点与交通搜索及下游，并由真实参与者在飞书端接受 Human Attempt 3；数据库回读为 `done / version 20 / 5 of 5`，审计以 `node.human_decision_accepted` 和 `instance.completed` 收口，19 条关联 outbox 全部 published。最终 Docx revision 5 可读，旧 Attempt 和两条 restart 审计继续保留；本条证明该真实流程闭环，不外推为生产就绪。

last-synced: 83d6cfa71d7aa0d01227cedbce6989fe630b3548 · 2026-08-21
