...
>
> 文档状态：2026-08-10 Phase 1 首次成功路径与可用性收口。开发真栈已经覆盖中央状态、飞书投影、自然语言草稿、人工任务、版本绑定的接受或退回，以及受控流程画板，但当前产品仍不适合邀请测试或推荐使用。内容提交 `64424f1de429f051af58739962f504e62337755d` 已把普通任务的提交或转交、决定任务的接受或退回统一到同一有界任务面，并增加“描述目标、检查流程、推进工作”三步引导和业务化术语；完整离线套件为 `1037 passed, 24 skipped`。候选 wheel SHA-256 为 `a79d55caa8398806c826acf6ede4f0a625d48637de3a525e6bbc2d57c1f44ed0`。该变更尚未完成开发部署和真实登录可见验收，历史测试流程、默认工作区信息层级和首次独立成功路径也仍需收口。飞书继续承担通知、待办和备用决定卡，PostgreSQL 继续是唯一业务真相。多人实时协同、通用自由白板与 Personal Agent Edge 扩张均不属于当前优先级。正式域名、生产容量和异机容灾仍缺。现有 Personal Agent Edge 只保留已验证的开发 Proof，正式分发仍为 No-Go。`Target` 表示目标产品契约，`As-built` 表示当前代码事实，两者不得混写。
>
