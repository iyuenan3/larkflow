...
## 状态
引擎核心 + 通用性收口 + **服务层**都已落码（v1 节点契约 + tool 数据化能力库 + 交付物层 + 选择性重算 + 打回预算与 `blocked` 出口 + 打回权限层 / 应答权 + 受控活图 + `larkflow serve` 常驻与 CLI + 三张模板 + 真实栈代码，**329 测绿**）。**新增业务场景 = 只加一个 yaml，零 Python**（见 `templates/hiring.yaml`）。
