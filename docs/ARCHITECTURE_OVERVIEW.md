...
1. 先判断当前用户、tenant、项目和 Attempt 有权使用哪些资料。
2. 再读取并校验这些资料的不可变正文快照。
3. 需要检索时，只在已经授权的资料范围内检索。
4. 再把有界结果交给 Planner 或 Agent。

不能先从整个企业知识库检索，然后让模型自己“不要泄露无权限内容”。模型提示词不是权限系统，向量数据库也不是权限系统。

系统会给 Planner 或显式声明附件输入的 Agent 一份有界的 `ContextBundle`，里面包含：
...
当前 `web.search` 属于第二类。它可以通过一个薄的 Python 豆包搜索适配器调用 Coding Plan 的 Custom API，先做静态 capability preflight，再把标题、摘要、来源 URL、发布时间或“时间不明”以及安全用量字段规范化为 Tool 交付物。搜索服务只负责检索，不能修改 DAG、写 PostgreSQL 或持有飞书凭据；信息综合仍由显式下游 Agent 完成。任何结果缺少可验证 URL 都会失败，不能把模型自行生成的文字冒充联网证据。未开启部署开关、缺少 API Key 或配置不完整时，能力保持不可用。
...
面向人的文本交付物默认物化为原生飞书 Docx 云文档，而不是上传一个本地 Markdown 文件。配置中的文件夹 token 只决定云文档放在哪个项目文件夹，不改变交付物类型；图片、压缩包等二进制对象才走普通云盘文件。PostgreSQL 仍只登记文档 handle、URL、版本关联和审计，不把飞书文档正文当成中央业务状态。
