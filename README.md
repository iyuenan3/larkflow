...
Personal Agent Edge 的 macOS 客户端现已接入登录 Keychain：设备密钥只写入系统钥匙串，`0600` 元数据文件只保存服务器地址和设备 ID。合成凭证的真实创建、回读和删除已在本机通过，正式 Edge 槽位仍为空；这不是正式员工配对或生产安全验收。
...
- 当前完整离线套件为 `816 passed, 17 skipped`；跳过项是需要显式外部环境的集成验证，不会在默认测试中访问网络、凭据或真实飞书。
