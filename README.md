...
- 当前完整离线套件结果为 `972 passed, 21 skipped`；跳过项是需要显式外部环境的集成验证，不会在默认测试中访问网络、凭据或真实飞书。受沙箱 `ps` 限制的进程树测试已在清除代理并取得所需本机权限后随完整套件通过。
