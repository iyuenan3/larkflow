...
- **员工工作台**：开发期 Owner 只读页面已提供本人流程列表、真实 DAG、跨轮次 Attempt、追加型审计、返工摘要和待处理提示。待处理项只从现有 PostgreSQL 状态派生，当前只建议或复制已存在的飞书命令，不在浏览器执行领域写操作。飞书 OAuth v3、PKCE、tenant 显式映射和服务端不透明 HttpOnly 会话已通过公网 IP HTTPS 部署；至少两名真实成员已从飞书工作台网页入口完成授权登录、本人 Owner 可见性和跨 Owner 隔离验证，机器人会话入口继续可用。静态 Bearer token 只保留为 loopback 开发回退；飞书会话已使用 PostgreSQL 耐久保存凭据摘要，并通过真实 Console 重启后的浏览器登录态回归。服务端 allowlist 中的成员还会看到当前企业的只读管理概览，普通成员无法枚举该接口；该管理面不包含会话撤销、配置修改或流程写操作。
...
- migration SQL 已进入 wheel，仓库与长期开发库均为二十份。`0013_im_command_mentions` 到 `0019_draft_generation_progress` 依次覆盖 mention、人员选择卡、恢复卡、canonical 动作、首反馈指标、通知唤醒和独立草稿生成进度；`0020_console_sessions` 保存飞书工作台不透明凭据的 SHA-256 摘要、服务端主体与有效期。migration 19、真实 PostgreSQL 双副本竞争和通知连接，以及 migration 20 的重建、注销、过期与重启后登录态均已验收；既有节点重启、完整实例重启和未来区域编辑预览继续保持恰好一路执行、一路幂等回放。
- 当前完整离线套件结果为 `960 passed, 20 skipped`；跳过项是需要显式外部环境的集成验证，不会在默认测试中访问网络、凭据或真实飞书。受沙箱 `ps` 限制的进程树测试已在清除代理并取得所需本机权限后随完整套件通过。
...
- `alicloud-sh` 的长期 Target 开发库只接受本机 peer authentication，已应用二十份 migration。九个 Target systemd 服务与一个 legacy 事件消费者组成十个 Python 服务；本次只重启 Console，十个 Python 服务与 Caddy 均回读 `active / running / NRestarts=0`。Edge Gateway 与 Owner Console 分别只监听 `127.0.0.1:8765` 和 `127.0.0.1:8780`；七条 PostgreSQL 通知连接分别由凭据侧持有四条、领域侧持有三条。两个 Interactive 副本固定 `claim_limit=1`，Draft Generation Worker 不持有飞书 profile。队列表仍是业务权威，通知失败时继续使用有界轮询。当前中央开发发布件对应内容提交 `e15f47942fcc01bc85ecbbfa822acd00558c06f0`，包含既有流程、Owner 只读 Console、PostgreSQL 耐久会话和服务端授权的管理员只读聚合。既有接受路径、带具体意见的退回到 Agent 重做路径、真实登录态跨 Console 重启和管理员与普通成员的 HTTP 隔离均已通过。既有小样本延迟只描述开发环境，不能外推生产容量。
