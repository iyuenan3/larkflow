...
- **员工工作台**：开发期 Owner 页面已提供本人流程列表、真实 DAG、跨轮次 Attempt、追加型审计、返工摘要和待处理提示。待处理项只从现有 PostgreSQL 状态派生，不保存第二套业务状态。草稿确认、暂停和继续可以直接执行；取消与节点或完整实例重启必须先查看服务端影响预览，再明确确认。Human 正文和决定仍在飞书责任入口完成。飞书 OAuth v3、PKCE、tenant 显式映射和服务端不透明 HttpOnly 会话已通过公网 IP HTTPS 部署；至少两名真实成员已从飞书工作台网页入口完成授权登录、本人 Owner 可见性和跨 Owner 隔离验证，机器人会话入口继续可用。静态 Bearer token 只保留为 loopback 开发回退；飞书会话已使用 PostgreSQL 耐久保存凭据摘要，并通过真实 Console 重启后的浏览器登录态回归。服务端 allowlist 中的成员还能看到当前企业聚合，并通过预览、确认和追加型审计撤销其他浏览器会话；当前会话必须注销，普通成员无法枚举管理接口。公网入口现已具备应用层有界令牌桶、可信代理来源覆盖、Caddy 超时与完整浏览器安全响应头。该管理面仍不包含 allowlist 自助修改、队列处置、配置修改、运行中图编辑或 Human 正文提交。
...
- 当前完整离线套件等价结果为 `988 passed, 21 skipped`；跳过项是需要显式外部环境的集成验证，不会在默认测试中访问网络、凭据或真实飞书。受沙箱 `ps` 限制的单个进程树测试已在沙箱外单独通过。
...
- `alicloud-sh` 的长期 Target 开发库只接受本机 peer authentication，已应用二十一份 migration。九个 Target systemd 服务与一个 legacy 事件消费者组成十个 Python 服务；十个 Python 服务与 Caddy 均回读 `active / running`，九个本次重启的 Target 服务均为 `NRestarts=0`。Edge Gateway 与 Owner Console 分别只监听 `127.0.0.1:8765` 和 `127.0.0.1:8780`；七条 PostgreSQL 通知连接分别由凭据侧持有四条、领域侧持有三条。两个 Interactive 副本固定 `claim_limit=1`，Draft Generation Worker 不持有飞书 profile。队列表仍是业务权威，通知失败时继续使用有界轮询。当前中央应用 wheel 对应内容提交 `db7651228e26055eb1229ae9f451e3e87c31df38`，SHA-256 为 `54a4bbf4c96834d7d69a3434d01b083d2467f5df6dd129c9ac6e35876efb49ff`；管理员 allowlist 运维工具对应内容提交 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`。既有接受路径、带具体意见的退回到 Agent 重做路径、来源约束决策路径、真实登录态跨 Console 重启、管理员与普通成员的 HTTP 隔离、会话撤销竞争与审计、伪造来源头拒绝、429 响应、无变化 allowlist 确认和唯一管理员保护均已通过。既有小样本与限流预算只描述开发环境，不能外推生产容量。
...
员工工作台 v1 是独立的 Owner 入口，用于查看本人发起的最近流程、DAG 状态、历史 Attempt 结果和审计时间线，并执行本人实例的受控操作。内容提交 `623b9b6228caa52b4680eb30ad2fee723e8921b6` 修正了首版把所有节点画成线性链的问题：浏览器现在依据服务端 `deps` 计算拓扑层级、绘制真实依赖箭头，并在节点上标注直接依赖。内容提交 `da94891f5e6d01ecee6082a98bab6148abba12ee` 又增加草稿确认、暂停、继续、取消预览确认和节点或完整实例重启预览确认；所有写入都复用中央领域服务，不复用 Personal Agent Edge API，也不在浏览器提交可信身份或状态。Human 正文与决定继续使用飞书责任入口。服务强制绑定 loopback，并提供两种互斥鉴权模式：`static` 把至少 32 字符的开发 Bearer token 映射到固定 tenant 与 person；`feishu` 使用 OAuth v3 授权码、PKCE 和显式 tenant_key 映射取得当前用户 open_id，再签发最长一天的服务端不透明 HttpOnly 会话。登录只读取身份所需字段，不请求用户业务 scope，不保存 user_access_token 或 refresh_token。该模式现已通过公网 IP HTTPS、飞书网页应用工作台入口和至少两名真实成员完成授权登录、本人 Owner 可见性与跨 Owner 隔离验证；消息中的机器人入口仍保持原有命令能力。会话凭据摘要由 migration `0020_console_sessions` 保存到 PostgreSQL，真实浏览器会话通过 Console 重启后刷新仍保持登录。管理员能力复用同一会话与服务端 allowlist：除当前企业只读聚合外，migration `0021_console_session_governance` 允许管理员经耐久预览和显式确认撤销其他会话，并追加不可变审计；当前会话只能注销，普通成员与不存在路由同样返回 404。开发公网入口还通过 Caddy 覆盖来源头，由 Console 使用不保存原始 IP 的有界令牌桶实施读取、认证、管理员写入、Owner 工作流写入和全局预算，并设置请求大小、超时和浏览器安全响应头。allowlist 变更只通过 root 侧预览确认工具执行，不进入员工 Console，也不允许浏览器自行提权。该结果关闭开发环境中的复制命令绕行、单进程会话、最小集中撤销、受控 allowlist 运维和基础公网滥用防护缺口，但不代表正式域名、allowlist 自助管理、批量会话治理、运行中图编辑、分布式限流、生产容量或生产上线已经完成。
