...
内容提交 `0dc5359e990635c7b6aa16ec0bcd798eb8df39d0` 修复受控内部试用暴露的返工上下文缺口。决定卡继续允许一键接受，但退回必须填写不超过 1000 字的具体意见；服务端重新校验、裁剪空白，并把意见写入 Human Attempt 结果、质量证据和追加型审计。Instance Owner 确认 `reject_target` 节点重启后，意见只进入该目标节点的新 Attempt 输入快照，节点真正激活时仍会保留并交给 Agent；影响集合之外的上游节点、旧 Attempt 和冻结 Instance Snapshot 均不改写。接受路径忽略客户端额外提交的意见。完整离线套件为 `908 passed, 18 skipped`，该实现复用既有 JSONB 字段，不增加 migration。本段只描述已推送代码与离线证据，开发服务器部署和真实飞书表单返工验收仍待执行。
