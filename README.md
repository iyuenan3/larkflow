...
内容提交 `b60cbbd8beb98742cc80082df78ac185274e3a8a` 的受控 DAG 画板已完成真实登录 Owner 公网验收。纯合成实例 `console_draft_2cd1ec1ad73e84abf9292ae0835c4fcc` 先完成未来 Agent 节点修改和从根节点发起的三节点返工，Graph 从 r1 进入 r2，旧 Attempt 全部保留为 canceled，新 Attempt 进入 2；随后从画板增加依赖最终复核节点的 Human 节点“归档复核结论”，Graph 进入 r3，实例为 `running / version 6 / 4 nodes`。再为未来 Agent 节点选择下游复核节点作为上游依赖时，页面明确提示循环依赖并拒绝生成预览；PostgreSQL 回读仍为 Graph r3、version 6，只有两条合法 `instance.graph_edited` 和一条节点重启审计。五个相关服务均为 `active / NRestarts=0`，验收窗口无 warning。该证据关闭开发环境中的真实登录画板增改、返工、DAG 拒绝和审计门槛，不代表通用自由白板、多人协同编辑、生产容量或生产发布已经完成。

内容提交 `482c280cf9007951fb117b835086a4b19eb1f932` 修正飞书 Task 描述超过 3000 字符或 10000 字节后整条投影失败的问题。任务描述会保留目标、前部上下文、明确截断提示和流程定位尾注，完整结果继续以 PostgreSQL 流程记录为准。完整离线套件为 `1016 passed, 23 skipped`。wheel SHA-256 为 `9d1cbf7cd1a0880632cdabb2dc31a757e29230d63bf89afc24ccfa3e5e2f08af`，已部署到 `/srv/larkflow/target/releases/20260809_164706_task_desc_482c280/`；十个 Python 服务均为 `active / NRestarts=0`。真实公开材料试用实例 `console_draft_d939e35738afd959f72e80c8e1cff3d4` 的失败 outbox 以原幂等键在第 9 次尝试恢复，两个飞书 Task 最终均完成，实例为 `done / version 7`。Owner 对内容做出退回判断，要求补齐官方逐条来源和真实运行约束，因此该结果证明真实链路和缺陷恢复，不证明草稿内容可直接用于升级或生产决策。

内容提交 `432fea77c210e7a2cfa5344054eb30d01706bf87` 已把受控流程输入直接接入员工工作台。员工填写目标、可选背景和可选协作者后，请求先写入 PostgreSQL 耐久队列，再由不持有飞书凭据的中央草稿 Worker 生成并确定性校验候选 DAG；浏览器不会模拟机器人消息，也不会直接创建运行中流程。生成结果始终先保存为草稿，只有本人再次点击“确认并启动”才进入既有运行时。请求 ID 保证重复提交幂等，生成租约支持崩溃接管，已保存候选在接管时保持冻结，失败最多尝试五次后进入保留历史的终态。完整离线套件为 `1015 passed, 23 skipped`。wheel SHA-256 为 `6b320b22804c02eaa2840d9a101bcf1b4ffe75287509816486727588ccdc0198`，已部署到 `/srv/larkflow/target/releases/20260809_0357_console_drafts_432fea7/`；长期库应用第二十三份 migration。真实 PostgreSQL 双 Worker 竞争只有一路领取，测试记录已清理。十个 Python 服务与 Caddy 均为 `active / NRestarts=0`，公网工作台返回 200，未登录草稿 API 返回 401，安全响应头、安装资源哈希和部署窗口零 warning 均已读回。同一真实成员主体签发的五分钟临时会话又用纯合成文本调用同一草稿 API 和真实中央模型，一次领取生成三节点草稿 `console_draft_0dc8215ed17b411288ca451be615a074`；独立数据库回读为 `draft / version 0 / template_version_id NULL / 0 NodeInstance / 0 Attempt / 0 Projection`，临时会话已注销。随后真实 Chrome 会话从公网工作台填写纯合成目标与背景，点击后按钮立即进入“生成中”，页面自动跳转三节点草稿 `console_draft_a11c6bb9d2ae071d78b10f802f567119`。PostgreSQL 独立回读为 `draft / version 0 / template_version_id NULL / confirmed_at NULL / 3 nodes / 0 NodeInstance / 0 Attempt / 0 Projection`，请求为 `ready / attempt 1`；Console 与草稿 Worker 均为 `active / NRestarts=0`，验收窗口无 warning。本轮没有确认启动，没有创建飞书待办，也没有发送消息。Chrome 可见草稿生成验收已经通过。
...
macOS 开发试用版使用 `deploy/larkflow-edge-manager.py` 管理独立版本目录，不修改 Homebrew 或系统 Python。推荐由发布方先构建哈希锁定的离线 bundle。构建器从 clean commit 的主 wheel 中只提取 `edge_contract.py`、`edge_client.py`、`edge_agent.py` 和 `edge_cli.py`，生成独立 `larkflow-personal-edge` wheel，不带中央控制面、PostgreSQL、飞书或 Console 模块。构建时需要联网，员工安装时不联网；manifest 固定目标 Mac 与 Python、完整 source commit、主 artifact、manager、全部 wheel 的包名、版本、大小和 SHA-256，并绑定 `requirements.lock`、`sbom.spdx.json` 与 `build-proof.json`。员工必须从独立可信渠道取得 manifest SHA-256：
...
默认版本目录是 `~/Library/Application Support/larkflow-edge/releases/`。直接 wheel 安装以 wheel 摘要标识 release，离线安装以覆盖全部依赖、manager 与构建证据的 manifest 摘要标识 release，因此相同主 wheel 的不同依赖集合不会误用旧环境。安装器先验证精确文件集、wheel 清单、哈希 lock、SPDX SBOM 和构建证明，再离线安装 bundle 内哈希锁定且已修复 `CVE-2026-8643` 的 pip，并用 `--require-hashes --no-index --only-binary=:all:` 安装锁定依赖；只有 `pip check` 和 CLI 启动校验通过才原子切换，旧版本保存在 `previous`。安装器拒绝不匹配目标、文件、wheel 元数据、证据、符号链接目录、额外文件和已有的无关同名命令，也不读取或迁移 Keychain，不注册后台服务。

当前 bundle 仍未代码签名或公证，manifest SHA-256 也尚未通过正式可信渠道发布。它只适合明确批准的开发试用，不能作为正式员工分发。详细结论和放行门禁见 [`research/edge-distribution-security-review.md`](research/edge-distribution-security-review.md)。直接 `--wheel <path> --sha256 <hex>` 的旧开发入口仍保留，但可能携带中央依赖并联网解析，不属于推荐的离线交付路径。
