...
- 当前完整离线套件等价结果为 `955 passed, 19 skipped`；跳过项是需要显式外部环境的集成验证，不会在默认测试中访问网络、凭据或真实飞书。唯一受沙箱 `ps` 限制的进程树测试已在沙箱外单独通过。
...
- `alicloud-sh` 的长期 Target 开发库只接受本机 peer authentication，已应用二十份 migration。九个 Target systemd 服务与一个 legacy 事件消费者组成十个 Python 服务；本次只重启 Console，Console 与 Caddy 均回读 `active / running / NRestarts=0`。Edge Gateway 与 Owner Console 分别只监听 `127.0.0.1:8765` 和 `127.0.0.1:8780`；七条 PostgreSQL 通知连接分别由凭据侧持有四条、领域侧持有三条。两个 Interactive 副本固定 `claim_limit=1`，Draft Generation Worker 不持有飞书 profile。队列表仍是业务权威，通知失败时继续使用有界轮询。当前中央开发发布件对应内容提交 `a6f5babb07623590e9be2a2b8c523857cce56ff7`，包含既有流程与 Owner 只读 Console 能力，以及 PostgreSQL 耐久会话。既有接受路径、带具体意见的退回到 Agent 重做路径和真实登录态跨 Console 重启均已通过。既有小样本延迟只描述开发环境，不能外推生产容量。
