...
- **员工工作台**：开发期 Owner 页面已提供本人流程列表、真实 DAG、跨轮次 Attempt、追加型审计、返工摘要和待处理提示。Owner 领域资源仍只读，待处理项只从现有 PostgreSQL 状态派生，当前只建议或复制已存在的飞书命令，不在浏览器执行领域写操作。飞书 OAuth v3、PKCE、tenant 显式映射和服务端不透明 HttpOnly 会话已通过公网 IP HTTPS 部署；至少两名真实成员已从飞书工作台网页入口完成授权登录、本人 Owner 可见性和跨 Owner 隔离验证，机器人会话入口继续可用。静态 Bearer token 只保留为 loopback 开发回退；飞书会话已使用 PostgreSQL 耐久保存凭据摘要，并通过真实 Console 重启后的浏览器登录态回归。服务端 allowlist 中的成员还能看到当前企业聚合，并通过预览、确认和追加型审计撤销其他浏览器会话；当前会话必须注销，普通成员无法枚举管理接口。该管理面不包含 allowlist 修改、队列处置、配置修改或流程写操作。
...
- migration SQL 已进入 wheel，仓库与长期开发库均为二十一份。`0013_im_command_mentions` 到 `0019_draft_generation_progress` 依次覆盖 mention、人员选择卡、恢复卡、canonical 动作、首反馈指标、通知唤醒和独立草稿生成进度；`0020_console_sessions` 保存飞书工作台不透明凭据的 SHA-256 摘要、服务端主体与有效期；`0021_console_session_governance` 增加安全会话 ID、耐久撤销预览和追加型审计。migration 19 的真实 PostgreSQL 双副本竞争和通知连接、migration 20 的重建与重启后登录态，以及 migration 21 的双连接确认竞争、幂等和不可变审计均已验收；既有节点重启、完整实例重启和未来区域编辑预览继续保持恰好一路执行、一路幂等回放。
- 当前完整离线套件结果为 `965 passed, 21 skipped`；跳过项是需要显式外部环境的集成验证，不会在默认测试中访问网络、凭据或真实飞书。受沙箱 `ps` 限制的进程树测试已在清除代理并取得所需本机权限后随完整套件通过。
...
- `alicloud-sh` 的长期 Target 开发库只接受本机 peer authentication，已应用二十一份 migration。九个 Target systemd 服务与一个 legacy 事件消费者组成十个 Python 服务；本次只重启 Console，十个 Python 服务与 Caddy 均回读 `active / running / NRestarts=0`。Edge Gateway 与 Owner Console 分别只监听 `127.0.0.1:8765` 和 `127.0.0.1:8780`；七条 PostgreSQL 通知连接分别由凭据侧持有四条、领域侧持有三条。两个 Interactive 副本固定 `claim_limit=1`，Draft Generation Worker 不持有飞书 profile。队列表仍是业务权威，通知失败时继续使用有界轮询。当前中央开发发布件对应内容提交 `8ba0ab9d93554b7958a650492e0282ad40db0d2e`，包含既有流程、Owner 领域只读 Console、PostgreSQL 耐久会话、管理员聚合和受审计的其他会话撤销。既有接受路径、带具体意见的退回到 Agent 重做路径、真实登录态跨 Console 重启、管理员与普通成员的 HTTP 隔离，以及会话撤销竞争与审计均已通过。既有小样本延迟只描述开发环境，不能外推生产容量。
...
员工工作台 v0 是独立的 Owner 入口，用于查看本人发起的最近流程、DAG 状态、历史 Attempt 结果和审计时间线。内容提交 `623b9b6228caa52b4680eb30ad2fee723e8921b6` 修正了首版把所有节点画成线性链的问题：浏览器现在依据服务端 `deps` 计算拓扑层级、绘制真实依赖箭头，并在节点上标注直接依赖。Owner 领域资源仍不提供确认、重启或改图操作，也不复用 Personal Agent Edge API。服务继续强制绑定 loopback，并提供两种互斥鉴权模式：`static` 把至少 32 字符的开发 Bearer token 映射到固定 tenant 与 person；`feishu` 使用 OAuth v3 授权码、PKCE 和显式 tenant_key 映射取得当前用户 open_id，再签发最长一天的服务端不透明 HttpOnly 会话。登录只读取身份所需字段，不请求用户业务 scope，不保存 user_access_token 或 refresh_token。该模式现已通过公网 IP HTTPS、飞书网页应用工作台入口和至少两名真实成员完成授权登录、本人 Owner 可见性与跨 Owner 隔离验证；消息中的机器人入口仍保持原有命令能力。会话凭据摘要由 migration `0020_console_sessions` 保存到 PostgreSQL，真实浏览器会话通过 Console 重启后刷新仍保持登录。管理员能力复用同一会话与服务端 allowlist：除当前企业只读聚合外，migration `0021_console_session_governance` 允许管理员经耐久预览和显式确认撤销其他会话，并追加不可变审计；当前会话只能注销，普通成员与不存在路由同样返回 404。该结果关闭开发环境单进程会话和最小集中撤销缺口，但不代表正式域名、allowlist 管理、批量会话治理、流程可写控制、生产限流或生产上线已经完成。
