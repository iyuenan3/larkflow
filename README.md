...
- 当前完整离线套件结果为 `982 passed, 21 skipped`；跳过项是需要显式外部环境的集成验证，不会在默认测试中访问网络、凭据或真实飞书。受沙箱 `ps` 限制的进程树测试已在清除代理并取得所需本机权限后随完整套件通过。
...
- `alicloud-sh` 的长期 Target 开发库只接受本机 peer authentication，已应用二十一份 migration。九个 Target systemd 服务与一个 legacy 事件消费者组成十个 Python 服务；十个 Python 服务与 Caddy 均回读 `active / running / NRestarts=0`。Edge Gateway 与 Owner Console 分别只监听 `127.0.0.1:8765` 和 `127.0.0.1:8780`；七条 PostgreSQL 通知连接分别由凭据侧持有四条、领域侧持有三条。两个 Interactive 副本固定 `claim_limit=1`，Draft Generation Worker 不持有飞书 profile。队列表仍是业务权威，通知失败时继续使用有界轮询。当前中央应用 wheel 对应内容提交 `66b2c12d3ea27a61e5a1cdc21332ed03adb516ac`；管理员 allowlist 运维工具对应内容提交 `00b3c8f920e6b856d11d9d4a91678959de3da6a5`。既有接受路径、带具体意见的退回到 Agent 重做路径、真实登录态跨 Console 重启、管理员与普通成员的 HTTP 隔离、会话撤销竞争与审计、伪造来源头拒绝、429 响应、无变化 allowlist 确认和唯一管理员保护均已通过。既有小样本与限流预算只描述开发环境，不能外推生产容量。
